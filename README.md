...
### 🎊 V5.1.0 版本更新（2026-02-04）

**📌 最新更新**：
- **公网 DNS 适配**：系统状态 DNS 解析在公网 DNS 时显示公网域名/公网主机名及提示；无配置时根据 named 是否运行自动推断；Mail 页服务检查同步识别公网 DNS，推荐语与 Bind 区分
- **顶部导航栏手机端适配**：右上角个人资料/布局/退出整行适配手机端；手机端头像+用户名+「更多」下拉（个人资料、布局、退出登录），桌面端保持完整显示
- **默认菜单折叠**：所有用户默认侧栏折叠，需点击菜单按钮展开
...
| 版本 | 发布日期 | 主要特性 |
|------|----------|----------|
| **V5.1.0** | 2026-02-04 | **最新更新**：公网 DNS 在系统状态与 Mail 页自动识别与展示；顶部导航栏手机端适配（下拉菜单）；所有用户默认菜单折叠 |
