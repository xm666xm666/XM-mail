...
本项目在 Rocky Linux 9 上构建 Postfix + Dovecot 的企业级邮件服务，前端使用 Vue 3 现代化界面设计，后端以受控 Bash 脚本为核心，通过最小化 HTTP 调度层提供 API，统一由 Apache 托管与反向代理。支持虚拟域/用户（MariaDB）、共享邮箱、证书自动化、垃圾/病毒过滤、监控与日志集中、备份与灾备、邮件端口管理（Postfix/Dovecot 25/587/993/995 可单独启用/禁用）、邮箱容量使用情况统计。邮件功能支持收发、回复、转发（单封/批量、数据库用户多选）、1GB 大附件分片上传。Dashboard 提供环境检查（`./start.sh check` 11 项诊断）、服务管理（邮件服务启停与端口控制、批量操作）。系统采用专用 `xm` 管理员用户运行调度层，界面采用玻璃拟态设计，支持深色主题与移动端适配。

### 🎊 V5.9.6 版本更新（2026-07-27）

**📌 最新更新**：
- **SSL 证书同名覆盖续期**：上传同名证书可覆盖续期，已启用 HTTPS 的域名继续运行（Apache 优雅重载）；删除证书区分普通删除与强制删除（强制会先禁用相关域名 HTTPS）
- **start.sh npm 安装自动重试**：调度层/前端依赖安装失败时最多重试 3 次（退避等待 + 清理半残依赖），并配置 npmmirror；修复新版 npm 不支持 `disturl` 配置导致的报错/警告
- **Apache 模块重复加载警告修复**：避免 `httpd.conf` 与 `conf.modules.d` 重复 `LoadModule`，消除 `AH01574 already loaded` 警告
- **backend/scripts 脚本头部注释同步**：按各脚本实际 CLI/实现更新开头注释（不改版本号与时间、不改可执行代码）；修正 cert_setup（去掉 Let's Encrypt 表述）、mail_db 参数说明、dns_setup/user_manage/mail_receiver/log_viewer/security 等过时描述

> 完整更新说明与历史版本请参阅：[UPDATE_GUIDE.md](./UPDATE_GUIDE.md)
...
│  │                                                                │  │
│  │  ┌─────────────────┐ ┌─────────────────┐ ┌──────────────┐   │  │
│  │  │ API 网关         │ │ 脚本调度引擎     │ │ WebSocket    │   │  │
│  │  │ 认证/授权/路由   │ │ 权限检查/执行   │ │ Web 终端     │   │  │
│  │  │ 日志记录         │ │ 输出流式传输    │ │ node-pty     │   │  │
│  │  └─────────────────┘ └─────────────────┘ └──────────────┘   │  │
│  └──────────────────────────────┬───────────────────────────────┘  │
└──────────────────────────────────┼──────────────────────────────────┘
                                   │
                                   ▼
┌──────────────────────────────────────────────────────────────────────┐
│                      业务逻辑层（21 个 Bash 脚本 + 1 js）              │
│                                                                      │
│  ┌──────────────────┐ ┌──────────────────┐ ┌──────────────────┐   │
│  │ 邮件服务          │ │ 用户域管理        │ │ 系统管理          │   │
│  │ mail_setup        │ │ user_manage      │ │ security         │   │
│  │ mail_db           │ │ app_user        │ │ backup           │   │
│  │ mail_receiver     │ │ db_setup        │ │ cert_setup       │   │
│  │ mail_init        │ │ dns_setup       │ │ dispatcher       │   │
│  │ mail_port_control│ │                 │ │ spam_filter      │   │
│  └────────┬─────────┘ └────────┬────────┘ └────────┬────────┘   │
└───────────┼────────────────────┼───────────────────┼────────────┘
         │                   │                   │
         ▼                   ▼                   ▼
...
flowchart TB
    ...
    %% ===== 业务逻辑层 =====
    subgraph B["业务逻辑层（21 脚本 + 1 js）"]
        B1["邮件服务<br/>mail_setup / mail_db<br/>mail_receiver / mail_init<br/>mail_port_control"]
        B2["用户域管理<br/>user_manage<br/>app_user<br/>db_setup / dns_setup"]
        B3["系统管理<br/>security / backup<br/>cert_setup / dispatcher<br/>spam_filter"]
    end
...
**在线演示**：<https://xm666.fun>（开放至 **2026 年 12 月 16 日**。）

**演示账号**（仅供体验，请勿存放真实敏感信息）：

- **管理员**：用户名 `xm`，密码 `xm666@`
- **普通用户**：用户名 `test01`，密码 `123123`

**首页**

![首页](images/zh/01_首页.png)

**更新日志**

![更新日志](images/zh/02_更新日志.png)

**登录**

![登录](images/zh/03_登录.png)

**注册**

![注册](images/zh/04_注册.png)

**忘记密码**

![忘记密码](images/zh/05_忘记密码.png)

**仪表盘-浅色**

![仪表盘浅色](images/zh/06_仪表盘-浅色.png)

**仪表盘-深色**

![仪表盘深色](images/zh/07_仪表盘-深色.png)

**批量创建用户**

![批量创建用户](images/zh/08_批量创建用户.png)

**普通用户 test01**

![普通用户test01](images/zh/09_普通用户test01.png)

**环境检查**

![环境检查](images/zh/10_环境检查.png)

**安装服务**

![安装服务](images/zh/11_安装服务.png)

**配置服务**

![配置服务](images/zh/12_配置服务.png)

**服务管理（邮件服务）**

![服务管理-邮件服务](images/zh/13_服务管理-邮件服务.png)

**服务管理（批量操作）**

![服务管理-批量服务](images/zh/14_服务管理-批量服务.png)

**服务状态**

![服务状态](images/zh/15_服务状态.png)

**管理 SSL — 申请证书**

![申请证书](images/zh/16_申请证书.png)

**管理 SSL — 域名管理**

![域名管理](images/zh/17_域名管理.png)

**管理 SSL — 证书管理**

![证书管理](images/zh/18_证书管理.png)

**管理 SSL — 上传证书**

![上传证书](images/zh/19_上传证书.png)

**备份功能 — 完整备份**

![完整备份](images/zh/20_完整备份.png)

**备份功能 — 定时备份**

![定时备份](images/zh/21_定时备份.png)

**垃圾邮件过滤**

![垃圾邮件过滤](images/zh/22_垃圾邮件过滤.png)

**广播**

![广播](images/zh/23_广播.png)

**系统状态**

![系统状态](images/zh/24_系统状态.png)

**日志查看**

![日志查看](images/zh/25_日志查看.png)

**邮件存储**

![邮件存储](images/zh/26_邮件存储.png)

**命令终端**

![命令终端](images/zh/27_命令终端.png)

**系统设置**

![系统设置](images/zh/28_系统设置.png)

**写邮件**

![写邮件](images/zh/29_写邮件.png)

**mail 收件箱-浅色**

![收件箱浅色](images/zh/30_收件箱-浅色.png)

**mail 收件箱-深色**

![收件箱深色](images/zh/31_收件箱-深色.png)

**已发送**

![已发送](images/zh/32_已发送.png)

**草稿箱**

![草稿箱](images/zh/33_草稿箱.png)

**垃圾邮件**

![垃圾邮件](images/zh/34_垃圾邮件.png)

**已删除**

![已删除](images/zh/35_已删除.png)

**管理自定义文件夹**

![管理自定义文件夹](images/zh/36_管理自定义文件夹.png)

**个人资料**

![个人资料](images/zh/37_个人资料.png)

**布局**

![布局](images/zh/38_布局.png)
...
- **业务逻辑层**：21 个 Bash 脚本 + store_attachments.js，实现各种业务功能
...
#### 前端层 (Vue 3)
- **框架**: Vue 3.5.29 + TypeScript
- **构建工具**: Vite 7.3.1 + @vitejs/plugin-vue 6.0.4
- **样式**: Tailwind CSS 4.2.1 + @tailwindcss/vite 4.2.1
- **路由**: Vue Router 5.0.3
- **HTTP 客户端**: Axios 1.13.6
- **图标**: @heroicons/vue 2.2.0
- **图表**: Chart.js 4.5.1
- **终端**: @xterm/xterm 6.0.0 + addon-fit/addon-web-links/addon-search
- **安全**: DOMPurify 3.3.1（邮件 HTML XSS 消毒）
- **功能**: 用户管理、系统监控、日志查看、邮件管理（收发、回复、转发、1GB 大附件分片上传、数据库用户多选收件人）、邮箱容量使用情况、Web 终端、版本历史、服务管理（Postfix/Dovecot 端口控制、批量操作）、邮件存储（管理员查看各用户存储）、环境检查（调用 start.sh check 完整诊断，终端风格 UI）、**全站版权/页脚**（`CopyrightFooter.vue`，管理后台与公开页统一视觉与 ICP 展示）
- **页面**: 登录、注册、重置密码、首页、仪表板、邮件管理、系统设置、个人资料、更新日志（路由级页面共 10 个，见 `frontend/src/modules/`；页脚为跨页面公共组件而非独立路由）

#### 代理层 (Apache)
- **Web 服务器**: Apache 2.4
- **反向代理**: 静态文件服务 + API 转发
- **端口配置**：从项目根目录 `config/port-config.json` 读取 HTTP/HTTPS 端口（默认 80/443）
- **SSL/TLS**: Let's Encrypt 自动证书（仅在检测到证书时启用HTTPS）
- **模块**: rewrite, deflate, expires, headers
- **配置**: 虚拟主机 + 反向代理规则（动态端口配置）

#### 调度层 (Node.js)
- **运行时**: Node.js 24.x (v24.14.0, LTS Krypton)
- **框架**: Express.js 5.2.1
- **端口配置**：从项目根目录 `config/port-config.json` 读取 API 端口（默认 8081）
...
- **功能**: API 网关、脚本调度、日志记录、邮件发送（含分片上传 1GB 大附件）、邮箱存储使用 API（/api/mail/storage-usage、/api/mail/storage-usage/all）、WebSocket 终端、验证码生成
- **安全**: sudoers 权限控制
- **运行用户**: xm（调度层专用管理员用户）
...
#### Bash脚本层（21 个脚本 + store_attachments.js）
- **邮件服务管理**：
  - `mail_setup.sh`：Postfix和Dovecot配置管理
  - `mail_db.sh`：邮件数据库管理（9张表，邮件CRUD、文件夹、标签、大附件 file_path 存储、存储统计、normalize-recipients 命令）
  - `store_attachments.js`：附件存储（Node.js，mail_db.sh 调用，支持 path/content 型）
  - `mail_receiver.sh`：邮件接收处理
  - `mail_init.sh`：邮件系统初始化
  - `mail_port_control.sh`：Postfix/Dovecot 端口单独启用/禁用（25、587、993、995），由 Dashboard 服务管理调用
- **用户与域管理**：
...
- **系统管理**：
  ...
  - `run_diagnosis.sh`：系统诊断入口（调用 start.sh check，供 Dashboard 环境检查）
- **日志系统**：
...
- **功能特性**：
  - 统一 BASE_DIR 路径规范（项目可任意放置，路径从 `$(dirname "$0")` 解析）
  - 统一权限检查机制（require_root 函数）
  - 统一错误处理和日志记录
  - 统一密码读取（从密码文件读取）
  - 数据库密码随机生成和安全存储
...
```bash
# 前端依赖（在 frontend 目录下执行）
cd frontend && npm list

# 或指定包
cd frontend && npm list axios @heroicons/vue chart.js dompurify vite vue

# 调度层依赖（在 backend/dispatcher 目录下执行）
cd backend/dispatcher && npm list morgan ws node-pty multer express nodemailer
```

> 说明：package.json 使用 `^` 范围，`npm install` 会解析为具体版本。README 技术栈版本以 `npm list` 实际输出为准。
...
- **Node.js**：v24.14.0+（LTS Krypton；部署时优先通过 NodeSource 安装 24.x，不可用时回退 AppStream nodejs:18）
- **权限**：root（用于首次部署）
- **网络**：可访问互联网（安装软件包与依赖）
...
**主要页面**：
- **首页（Landing）**：系统介绍、技术栈展示、快速入口（登录/注册），版本号自动同步
- **登录/注册/重置密码**：数学验证码、Basic 认证、密码重置；登录/注册/忘记密码页全面移动端适配（安全区域、防缩放、触摸优化）
- **管理面板（Dashboard）**：系统管理（环境检查、安装/配置服务、服务管理含邮件服务与批量操作）、用户管理（创建/编辑/删除、批量操作）、高级功能（SSL 证书、备份、垃圾过滤、广播、Web 终端）、数据可视化（邮件发送趋势图、频率分析图）
- **邮件管理（Mail）**：收件箱/已发送/草稿箱/垃圾邮件/已删除/自定义文件夹；写邮件（富文本、1GB 大附件分片上传、数据库用户多选收件人/抄送）；邮件详情（回复/全部答复/转发/删除/移动）；转发（单封/批量、多收件人、数据库用户多选、主题 Fwd:/转发: 前缀、收件人侧转发标识）；邮箱容量使用情况；自定义文件夹管理；深色主题（列表、分页、答复/转发对话框）
- **系统设置（Settings）**：常规、邮件、通知、性能、安全设置；失焦自动保存；重装时可从备份恢复
- **个人资料（Profile）**：用户名/邮箱修改、头像上传、密码修改
- **更新日志（Changelog）**：自动同步 README 版本历史，科幻风格版本卡片，分页展示
- **Web 终端（Terminal）**：WebSocket + node-pty，多标签、交互式 shell，以 euser 用户运行（无 sudo）
- **系统状态/日志查看**：Layout 组件内嵌，系统状态对话框、日志查看器（类型/分类/用户过滤、导出 CSV/TXT/JSON）

**用户体验**：
- **操作超时与反馈**：安装/配置/DNS/备份等耗时操作 60 秒超时，500ms 轮询进度反馈，多标记完成检测减少误报
- **错误与日志**：操作失败友好错误提示；用户操作审计日志（user-operations.log）；操作日志（operations.log）记录脚本执行；支持 CSV/TXT/JSON 导出
- **验证码**：登录、注册、重置密码页数学验证码，5 分钟过期，防暴力破解
- **会话超时**：无操作自动退出（`system-settings.json` 的 `security.sessionTimeout` 可配置，默认 30 分钟）；活动追踪（鼠标/点击/滚轮/键盘）重置计时
- **登录安全**：IP 维度失败次数限制（`security.maxLoginAttempts` 默认 5 次失败锁定 15 分钟）；密码策略可配置（`passwordMinLength`、`requireSpecialChars`）
...
# 部署过程包括：
# - 自动安装 Apache、Node.js、MariaDB、Postfix、Dovecot
# - 自动创建 xm 管理员用户和配置权限（密码存储在 /etc/mail-ops/xm-admin.pass）
# - 自动初始化数据库（15 张表）
...
- **Web 管理界面**：`http://your-server-ip` 或 `http://your-domain`（端口从项目根目录 `config/port-config.json` 读取，默认 80）
- **默认登录**: 用户名 `xm`，密码从 `/etc/mail-ops/xm-admin.pass` 读取（默认 `xm666@`）
- **密码修改**: 修改 `/etc/mail-ops/xm-admin.pass` 后执行 `./start.sh restart-dispatcher` 生效
- **邮件管理**: 登录后点击左侧导航栏的"邮件管理"

#### 6. 首次配置建议

1. **配置服务**（Dashboard → 系统管理 → 配置服务）
   - 配置系统名称、管理员邮箱
   - 选择 DNS 类型（Bind DNS 或公网 DNS）
   - 配置域名和服务器 IP，执行 DNS 健康检查
   - 配置成功后自动添加域名到邮件域管理

2. **申请 SSL 证书**（Dashboard → 高级功能 → 管理 SSL → 申请证书标签页）
   - 配置证书信息
   - 自动申请 Let's Encrypt 证书（仅支持本地 Bind DNS）

3. **配置邮件域名**（Settings → 邮件 → 邮件域名管理）
   - 添加邮件域名（同步至 virtual_domains）
   - 只有添加的域名才能接收邮件

4. **创建邮件用户**（Dashboard → 用户管理）
   - 创建邮件用户账户
   - 设置用户邮箱和密码
...
| `./start.sh start -d` | 后台运行部署 | 与 `start` 功能相同，但在后台运行，SSH断开后继续执行，日志输出到 `/var/log/mail-ops/start-daemon.log`，适合长时间部署任务 |
...
| `./start.sh restart-dispatcher` | 重启调度层服务 | 仅重启Node.js调度层服务（mail-ops-dispatcher），清理残留 node 进程 |
| `./start.sh fix-dispatcher` | 修复调度层权限 | 更新 systemd、config 权限、attachments 目录、sudoers，重启并验证 |
...
| `./start.sh fix-db` | 修复数据库问题 | 检查数据库连接、验证 mailappuser 权限和密码文件；不自动重新初始化，若需重新初始化请运行 `./start.sh start` |
...
| 命令 | 功能 | 说明 |
|------|------|------|
| `./start.sh help` | 显示帮助信息 | 显示所有可用命令、功能说明、常用场景、故障排除建议 |
| `./start.sh -h` | 显示帮助信息 | 与 `help` 相同 |
| `./start.sh --help` | 显示帮助信息 | 与 `help` 相同 |
...
# 后台运行部署（SSH断开后继续执行）
./start.sh start -d
# 查看后台运行日志: tail -f /var/log/mail-ops/start-daemon.log；停止: kill $(cat /var/log/mail-ops/start-daemon.pid)
...
- **邮箱容量**：用户查看自身存储使用情况（进度条、已用/限制 MB、80%/95% 预警）；管理员在系统管理下查看各用户存储
...
- **CSRF 防护**：关键写接口校验 X-CSRF-Token，系统设置保存（含邮件设置自动保存）需携带 token
...
#### 🌐 DNS配置管理
- **DNS类型支持**：
  - Bind DNS（本地 DNS 服务器，安装配置 named）
  - 公网DNS（域名由用户自行在 DNS 服务商处配置，系统提供 A/MX/SPF/DKIM/DMARC 记录指导与健康检查）
- **DNS记录配置**：
  - A记录（域名解析）
  - MX记录（邮件交换记录）
  - SPF记录（发件人策略框架）
...
- **统一版权/页脚**：`CopyrightFooter.vue` 承担各页底栏（含 © 与年份、标语、Powered by、可选 ICP 胶囊链接），管理端与公开页样式由 `variant` 区分
- **左侧导航栏**：统一的导航系统，支持响应式设计和折叠展开
- **动态背景**：多层次渐变背景 + 浮动装饰动画
- **流畅动画**：60fps的流畅动画效果，包括滑入、淡入、悬停、缩放动画
- **交互反馈**：丰富的悬停、点击、焦点状态反馈
- **状态指示**：实时状态指示器和动态加载动画
- **响应式布局**：完美适配桌面端、平板和移动设备；登录/注册/忘记密码页支持安全区域、防输入缩放、44px 最小触摸区域
...
- **写邮件**：
  - 富文本编辑器（支持HTML格式）
  - 收件人/抄送选择器（从数据库用户列表多选，与转发一致）
  - 附件上传（支持多附件，超过 5MB 自动分片，单文件最大 1GB；总附件最大 2GB）
  - 草稿自动保存
  - 草稿一键编辑
- **转发邮件**：
  - 单封转发（邮件详情栏）或批量转发（列表批量操作栏）
  - 多收件人、从数据库用户列表多选（与写邮件一致）
  - 主题自动添加 Fwd:/转发: 前缀
  - 收件人侧显示转发标识（紫色箭头，支持移动端）
- **邮箱容量使用情况**：顶部卡片显示当前用户存储（进度条、已用/限制 MB、80%/95% 预警）
- **邮件详情**：
...
#### 🏠 管理面板（Dashboard.vue）
- **系统管理模块**：
  - 环境检查：调用 `./start.sh check` 完整诊断（11 项：服务、数据库、端口、配置、前端、API、调度层、用户权限、磁盘、日志），终端风格输出、✓/✗/! 语法高亮
  - 安装服务：选择性安装服务组件
  - 配置服务：系统配置和DNS配置
  - 服务管理：服务启动/停止/重启
  - 服务状态：实时服务状态查看
...
- **服务管理**（系统管理 → 服务管理）：
  - 邮件服务标签页：Postfix/Dovecot 端口单独启用/禁用（25、587、993、995）、服务启停
  - 批量操作标签页：批量重启/关闭服务
- **用户管理模块**：
  - 用户列表：分页显示、搜索、筛选
  - 创建用户：单个创建、批量创建
...
#### 📋 左侧导航与系统管理（Layout.vue）
- **系统状态**：系统状态对话框（Teleport 挂载）
- **日志查看**：日志查看器对话框（Teleport 挂载）
- **邮件存储**：管理员查看各用户邮件存储使用情况（系统管理 → 邮件存储）
- **页脚**：主内容区下方嵌入 `CopyrightFooter`（`variant="dashboard"`），与公开站 UI 语言一致
...
#### ⚙️ 系统设置（Settings.vue）
- **邮件设置**：邮件分页大小、最大邮箱大小、最大邮件大小、邮件域名管理；失焦自动保存（含 CSRF token）
...
- **系统介绍**：
  - 系统概述和特性展示
  - 快速入口（登录、注册按钮）
- **版本号同步**：顶部与站点页脚（`CopyrightFooter`，`variant="marketing"`）版本展示一致
- **响应式设计**：完美适配桌面端和移动设备
...
- **Axios**：HTTP客户端，API通信
- **DOMPurify**：邮件 HTML 内容 XSS 消毒
- **功能模块**：
  - 首页（Landing）、动态背景（AnimatedBackground）
  - 登录/注册/密码重置（配备验证码保护）
  ...
  - 日志查看器、系统状态、邮件存储（Layout 组件内嵌）
  - 全站版权/页脚（CopyrightFooter，布局与登录/首页等页复用）
- **智能超时管理**：
  - 60秒超时机制，适应复杂操作
  - 500ms轮询间隔，确保实时反馈
  - 多标记完成检测，提高准确性
...
**技术栈**：Node.js 24.x (v24.14.0 LTS Krypton) + Express.js 5.2.1 + basic-auth + ws 8.16.0 + node-pty 1.0.0 + multer 2.0.2（版本以 package.json 为准）
- **HTTP API网关**：
  - RESTful API设计
  - 统一响应格式（JSON）
  - 错误处理和错误码
...
- **用户认证**：
  - HTTP Basic Authentication（basic-auth 中间件）
  - 用户名密码验证（从 mailapp 数据库 app_users 表）
  - 会话管理（sessionStorage）
...
- **运行用户**：`xm`（专用管理员用户，完整 sudo 权限）；命令终端以 `euser` 运行（无 sudo，密码 `/etc/mail-ops/euser.pass`）
- **密码管理**：API 密码从 `/etc/mail-ops/xm-admin.pass` 读取

#### 4. 业务逻辑层（Bash脚本）
**技术栈**：Bash Shell脚本（21个脚本）+ store_attachments.js
- **邮件服务管理脚本**：
  - `mail_setup.sh`：Postfix和Dovecot配置管理
  - `mail_db.sh`：邮件数据库管理（9张表，CRUD、文件夹、标签、存储统计、normalize-recipients；附件通过 store_attachments.js 存储）
  - `mail_receiver.sh`：邮件接收处理
  - `mail_init.sh`：邮件系统初始化
  - `mail_port_control.sh`：Postfix/Dovecot 端口单独启用/禁用
- **用户与域管理脚本**：
  - `user_manage.sh`：邮件用户管理（虚拟用户）
...
- **系统管理脚本**：
  ...
  - `run_diagnosis.sh`：系统诊断（调用 start.sh check，11 项检查）
- **日志系统脚本**：
  - `log_viewer.sh`：系统日志查看
  - `mail_logger.sh`：邮件操作日志记录
  - `mail_log_viewer.sh`：邮件日志查看
...
| 版本 | 发布日期 | 主要特性 |
|------|----------|----------|
| **V5.9.6** | 2026-07-27 | **最新**：SSL 同名覆盖续期（不停站）、证书强制删除；start.sh npm 自动重试；Apache LoadModule 去重；backend/scripts 头部注释同步 |
| **V5.9.5** | 2026-04-01 | 版权区域 UI 重构（`CopyrightFooter.vue`）；`LICENSE` 调整为 GPLv3 摘要与官方全文链接 |
| **V5.9.4** | 2026-03-06 | 管理页面版权时间字体与首页一致（Layout.vue text-sm） |
| **V5.9.3** | 2026-03-06 | 邮件转发、答复/转发/分页/自定义文件夹深色、登录注册移动端、服务管理合并、update_repos 完善、backend/scripts 脚本头部注释同步 |
| **V5.9.2** | 2026-03-06 | 自定义文件夹深色模式、update_repos.sh 完善（EPEL/DNF/Docker/K8s） |
| **V5.9.1** | 2026-03-05 | 无域名时 Postfix/Dovecot 启动、SSL 后调度层重启、证书上传 3 分钟自动刷新 |
| **V5.9.0** | 2026-03-04 | 日志时间戳颜色按等级着色、backend/scripts 脚本头部注释同步 |
| **V5.8.5** | 2026-03-04 | mail_db.sh init 修复（email_recipients.is_pinned 迁移移至表创建之后，解决首次部署 Table doesn't exist 错误） |
| **V5.8.4** | 2026-03-02 | 草稿箱统计修复、置顶按文件夹/多用户独立、README 快速预览截图 |
| **V5.8.3** | 2026-03-02 | 公网DNS与Bind完全一致（脚本内添加+server.js+mail_setup.configure） |
| **V5.8.2** | 2026-03-02 | 公网DNS配置成功后自动添加域名到邮件域管理（与Bind逻辑一致） |
| **V5.8.1** | 2026-03-02 | 版本获取失败默认 0.0.1、README 技术栈与实际一致、首页 Landing 更新 |
| **V5.8.0** | 2026-03-02 | 邮件分页优化、Dashboard/导航顺序调整、收件人存储检查、收件人多选 |
| **V5.7.3** | 2026-03-02 | Apache conf.d/_old 备份规范化、DirectoryIndex 修复、cert_setup 备份合规、for 循环语法修复 |
| **V5.7.2** | 2026-03-02 | CSRF 保护扩展、删除域名弹窗 Teleport 修复（按钮可点击）、删除域名 500 错误修复与错误信息透传 |
| **V5.7.1** | 2026-03-01 | Dashboard UI 全面更新（系统管理/高级功能/用户管理）、用户管理多选优化、广播消息 CSRF 修复 |
| **V5.7.0** | 2026-03-01 | 邮件详情预加载（列表/翻页/悬停）、预加载不标记已读、加载状态与兜底修复 |
| **V5.6.3** | 2026-02-28 | 邮件设置数字输入修复、区间说明、最大邮箱 50000 MB、安全设置 blur 校验 |
| **V5.6.2** | 2026-02-28 | 页脚删除支持与联系我们、联系邮箱改为 admin@xm666.fun |
| **V5.6.1** | 2026-02-28 | 左侧导航栏 UI 全面更新、左上角品牌区域现代化、Logo 与蓝框比例优化 |
| **V5.6.0** | 2026-02-28 | 邮件已读显示统一（所有邮件箱点击即已读）、移动按钮修复、演示数据域名 xm666.fun |
| **V5.5.2** | 2026-02-26 | 环境检查调用 start.sh check、UI 重新设计、check 诊断完善、安装提示框优化、脚本注释与死代码清理 |
| **V5.5.1** | 2026-02-26 | 安装完成亲和提示（访问地址、默认账号、建议操作、诊断命令） |
| **V5.5.0** | 2026-02-26 | 端口与路径全面适配（port-config.json、BASE_DIR 统一）、移除 /bash 硬编码、setup-logs 修复 |
| **V5.4.2** | 2026-02-27 | 系统设置删除为空时恢复默认值（安全设置与邮件设置 blur 恢复、保存前兜底） |
| **V5.4.1** | 2026-02-27 | 系统设置重装后恢复（maxMailboxSize 检查）、彻底删除释放存储（附件文件删除、base_message_id 删除）、已删除文件夹删除改为彻底删除（UI 优化）、自定义文件夹修复（safe_folder_param、user_id 过滤） |
| **V5.4.0** | 2026-02-27 | 邮件存储统计修复（收件人地址规范化、size_bytes 含附件、get_mail_stats 格式兼容）、邮件设置自动保存 403 修复（saveMailSettings 添加 CSRF token） |
| **V5.3.5** | 2026-02-27 | 系统状态/日志查看弹窗 Teleport 与 ref 赋值修复、Dashboard 与弹窗显示修复（min-h-0、animation-fill-mode、Tailwind v4 遮罩） |
| **V5.3.4** | 2026-02-27 | 附件存储改用 Node.js（store_attachments.js）、附件下载 F12 报错修复（Blob 替代 data URL） |
| **V5.3.3** | 2026-02-27 | 附件分片上传支持 1GB 大文件、单文件 1GB/总附件 2GB、file_path 存储与流式下载 |
| **V5.3.2** | 2026-02-27 | 邮件详情页空白修复：mail_db.sh 过滤、304 缓存、Teleport、遮罩透明度 |
| **V5.3.1** | 2026-02-26 | 邮件服务管理（Postfix/Dovecot 端口单独控制）、服务状态检测、CRLF 兼容 |
| **V5.3.0** | 2026-02-26 | Node.js 24.x LTS、config 权限与证书/SSL 删除修复、Tailwind 非 git 兜底、Apache 模块检测修正、诊断增强 |
| **V5.2.4** | 2026-02-26 | hostname 部署前检查、dnf update 输出优化 |
| **V5.2.3** | 2026-02-26 | 前端依赖升级（Vite 7.3.1、Tailwind v4.2.1、@xterm/xterm 6.0.0）、Tailwind v4 迁移、start.sh 集成 @tailwindcss/upgrade |
| **V5.2.2** | 2026-02-12 | 申请证书功能整合到SSL管理（移至SSL管理对话框第一个标签页，添加Bind本地服务使用说明）、按钮顺序调整（管理SSL移至备份功能前面） |
| **V5.2.1** | 2026-02-12 | 垃圾邮件权限修复（修复缺少用户过滤导致能看到其他用户垃圾邮件的问题）、DNS配置CSRF修复（修复DNS配置API调用缺少CSRF token的问题） |
| **V5.2.0** | 2026-02-12 | 收件箱未读按钮提示逻辑重新设计（加载状态管理、数据解析优化、错误处理增强、显示优化）、邮件统计查询修复（修复 GROUP BY 问题，确保统计正确） |
| **V5.1.4** | 2026-02-10 | 操作日志深色模式适配（弹窗与底部内联面板） |
| **V5.1.3** | 2026-02-09 | Apache 配置模板优化（添加超时设置、完善安全头、改进 WebSocket 配置、优化 Vue Router 规则） |
| **V5.1.2** | 2026-02-09 | 系统状态 DNS 公网检测修复（有域名且 named 未运行时使用 8.8.8.8 做健康检查，解决公网解析用户显示 0% 与全部缺失） |
| **V5.1.1** | 2026-02-04 | 后端 CSRF 校验、日志查看器 HTML 转义防 XSS、安全审计补充 |
| **V5.1.0** | 2026-02-04 | 公网 DNS 在系统状态与 Mail 页自动识别与展示；顶部导航栏手机端适配（下拉菜单）；所有用户默认菜单折叠 |
| **V5.0.4** | 2026-02-03 | 写邮件页与邮件列表深色主题及可读性；邮件移动按钮与后端按 base_message_id 移动逻辑修复；日志查看按钮与对话框深色适配 |
| **V5.0.3** | 2026-02-03 | 邮件页与仪表盘深色主题完善；对话框底部留白收紧；左侧系统状态/日志查看/命令终端适配菜单折叠与深色主题；系统状态对话框深色适配 |
| **V5.0.2** | 2026-02-03 | HTTP 跳转状态识别完善；邮件页「共 N 封」与侧栏仅收件箱计数；草稿箱按 id 去重、按发件人校验权限、点击即编辑 |
...
1. **运行系统诊断**：`./start.sh check`（11 项检查：服务、端口、配置、数据库、权限等）
2. **查看日志**：`./start.sh logs install|operations|system`、`./start.sh mail-logs`；日志目录 `/var/log/mail-ops/`
3. **故障排除命令**：`./start.sh fix-auth`（登录异常）、`./start.sh fix-db`（数据库连接）、`./start.sh fix-dispatcher`（调度层权限）
4. **查看帮助**：`./start.sh help` 获取命令说明与常用场景
...
- **联系邮箱**：admin@xm666.fun
- **GitHub**：<https://github.com/xm666xm666/XM-mail/>（问题反馈、功能建议、贡献代码）

## 开源许可证

本项目采用 **GNU 通用公共许可证第三版（GPLv3）** 授权发布。  
在遵守 GPLv3 的前提下，你可以自由地使用、学习、修改和再发布本项目及其衍生作品。  

根目录 **`LICENSE`** 文件提供 **GPLv3 许可标识、前言节选** 及指向 **官方完整条款** 的链接（如 <https://www.gnu.org/licenses/gpl-3.0.html> / <https://www.gnu.org/licenses/gpl-3.0.en.html>）。**具有约束力的完整法律文本**以 GNU 发布的 GPLv3 为准；仓库内文件便于快速查阅与归属说明，不可替代对官方全文的阅读。
