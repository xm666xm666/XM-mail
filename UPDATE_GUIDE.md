# XM邮件管理系统 - 更新操作指南 V5.9.6

## 概述

本文档详细记录了XM邮件管理系统的所有更新操作、版本升级步骤和系统维护指南。适用于系统管理员进行版本升级、功能更新和系统维护。

## 🎉 最新版本 - V5.9.6 (2026-07-27) - SSL 证书续期不停站与部署安装加固

### 🎊 版本亮点

**V5.9.6 优化 SSL 证书运维逻辑（同名覆盖续期保持域名 HTTPS 运行），加固 `start.sh` 依赖安装与 Apache 模块去重，并同步 `backend/scripts` 脚本头部注释与实际实现一致。**

### 📋 最新更新

- **SSL 证书同名覆盖续期**：单传/批量上传同名证书时备份后覆盖，已启用 HTTPS 时执行 Apache 优雅重载，域名可继续运行
- **SSL 证书删除逻辑调整**：未启用 HTTPS 可直接删除并解除关联；已启用 HTTPS 需确认强制删除（先禁用相关域名 HTTPS）；前端引导优先使用同名上传续期
- **start.sh npm 安装自动重试**：`npm_install_with_retry` 最多 3 次（退避等待、清理 `node-pty`/半残 `node_modules`，yarn 兜底）；调度层与前端安装统一接入
- **npm 配置兼容**：移除新版 npm 不支持的 `disturl` 配置，改用 `NODEJS_ORG_MIRROR` 等环境变量供 node-gyp 使用
- **Apache AH01574 修复**：避免 `httpd.conf` 与 `conf.modules.d` 重复加载 `proxy_*` 等模块
- **backend/scripts 脚本头部注释同步**：按实际 CLI/实现更新开头注释（不改版本号与时间、不改可执行代码）

### 📋 主要更新内容

- **backend/dispatcher/server.js**：证书删除（`force`）、单传/批量同名覆盖、覆盖后 `reload` Apache；证书名支持域名格式
- **frontend/src/modules/Dashboard.vue**：删除确认与强制删除二次确认；上传提示「同名覆盖续期」
- **start.sh**：`configure_npm_install_env` / `npm_install_with_retry`；`remove_duplicate_httpd_loadmodules`；WebSocket 模块启用逻辑去重
- **backend/scripts 头部注释**：`app_user`/`backup`/`cert_setup`/`db_setup`/`dispatcher`/`dns_setup`/`log_viewer`/`mail_db`/`mail_init`/`mail_logger`/`mail_log_viewer`/`mail_port_control`/`mail_receiver`/`mail_service_logger`/`mail_setup`/`run_diagnosis`/`security`/`test_spam_filter`/`update_repos`/`user_manage`/`store_attachments.js` 等与实现对齐；重点修正：cert_setup 去掉 Let's Encrypt 表述、mail_db 的 `sending-trends`/`frequency-analysis`/`delete_domain`/`mark_read` 参数、dns_setup 须用 `configure-public`、user_manage 为直接 mysql+Maildir、mail_receiver 标注 spam_filter 未集成、log_viewer 无 search/stats、security 含 stop 且 install-cert 为 certbot 占位

### 🔄 升级步骤

- 含前后端、部署脚本与脚本注释变更；建议重建前端并重启调度层
- 仅脚本头部注释变更本身无需重建；若同时拉取 SSL/start.sh 相关代码，仍按下列步骤升级
...
若调度层依赖曾因 `node-pty` 编译失败而不完整，可清理后重装：

```bash
cd /path/to/project/backend/dispatcher
rm -rf node_modules package-lock.json
npm config delete disturl 2>/dev/null || true
cd /path/to/project
# 重新执行部署中的依赖安装步骤，或手动 npm install 后再 restart-dispatcher
```
...
### V5.9.5 (2026-04-01) - 版权页 UI 重构与 LICENSE 更新

- **版权区域 UI 重构**：新增 `CopyrightFooter.vue`，管理后台（Layout）、登录/注册/重置、首页与更新日志统一引用；极光分隔线、渐变品牌标识、ICP 备案胶囊、版本号芯片、标语排版
- **LICENSE 更新**：根目录 `LICENSE` 调整为 GPLv3 标识、前言摘要与官方完整条款链接，**授权仍为 GNU GPLv3**
- **CopyrightFooter.vue**：`variant` 区分 dashboard（管理端底栏）、auth（登录/注册/重置固定底栏）、marketing（首页/Changelog 大页脚内嵌）；去除各页重复页脚相关 CSS
- **涉及文件**：`Layout.vue`、`Login.vue`、`Register.vue`、`Reset.vue`、`Landing.vue`、`Changelog.vue`
- **升级步骤**：前端变更，需重新构建界面；`./start.sh rebuild`

---

### V5.9.4 (2026-03-06) - 管理页面版权时间字体与首页一致

- **管理页面版权时间字体与首页一致**：Layout.vue 版权行（© 2024-2026 XM.）使用 text-sm，与首页 Landing 风格统一
- **Layout.vue**：版权行容器添加 text-sm，移除 © 符号的 text-base，使管理页面（仪表板、邮件管理、系统设置、个人资料等）版权时间字体与首页一致
- **升级步骤**：无需重启或重建，仅前端样式更新；`./start.sh rebuild`

---

### V5.9.3 (2026-03-06) - backend/scripts 脚本头部注释同步

- **backend/scripts 脚本头部注释同步**：mail_db.sh、mail_setup.sh、update_repos.sh、dns_setup.sh、cert_setup.sh、app_user.sh、user_manage.sh、run_diagnosis.sh、dispatcher.sh、mail_receiver.sh、mail_init.sh、test_spam_filter.sh、spam_filter.sh、mail_service_logger.sh、mail_logger.sh、mail_log_viewer.sh、security.sh、backup.sh、log_viewer.sh、db_setup.sh、mail_port_control.sh 等 21 个脚本头部注释按实际实现更新
- **升级步骤**：无需重启或重建，仅文档与脚本注释更新

---

### V5.9.2 (2026-03-06) - 自定义文件夹深色模式、update_repos.sh 完善

- **自定义文件夹页面深色模式**：Mail.vue 自定义文件夹顶部栏、加载状态、空状态全面支持深色主题，与整体深色模式一致
- **update_repos.sh 完善**：EPEL 阿里云镜像（epel*.repo）；DNF 性能优化（max_parallel_downloads=10、fastestmirror=True）；Docker CE 官方源自动替换为阿里云；Kubernetes 更新至 v1.35；timeout 不存在时兼容执行
- **升级步骤**：`./start.sh rebuild` 与 `./start.sh restart`（前端样式变更需重新构建）

---

### V5.9.1 (2026-03-05) - 邮件服务启动修复、SSL 后调度层重启、证书上传自动刷新

- **Postfix/Dovecot 无域名时也启动**：start.sh 在未检测到有效域名时仍调用 mail_setup.sh configure，确保邮件服务正常启动
- **SSL 配置后调度层重启**：enable-ssl 成功后延迟 3 秒重启 mail-ops-dispatcher，避免 ERR_CONNECTION_REFUSED
- **证书上传 3 分钟自动刷新**：批量上传证书后 3 分钟自动刷新页面；点击上传时提示「系统将在 3 分钟后自动刷新页面」
- **升级步骤**：`./start.sh restart`

---

### V5.9.0 (2026-03-04) - 日志时间戳颜色优化、脚本头部注释同步

- **日志时间戳颜色优化**：start.sh format_log_message 根据日志等级为时间戳设置不同颜色；INFO/SUCCESS 绿色、WARN 黄色、ERROR 红色、DEBUG 橙色
- **backend/scripts 脚本头部注释同步**：mail_db.sh 补充 search 命令说明；全目录脚本头部注释按实际实现更新
- **升级步骤**：`./start.sh restart`

---

### V5.8.5 (2026-03-04) - mail_db.sh 初始化修复

- **mail_db.sh 初始化修复**：修复首次部署时 `email_recipients.is_pinned` 在表创建之前执行 ALTER 导致「Table 'maildb.email_recipients' doesn't exist」错误；将 is_pinned 字段迁移逻辑移至 CREATE TABLE email_recipients 之后
- **升级步骤**：`./start.sh restart`；首次部署无需额外操作

---

### V5.8.4 (2026-03-02) - 草稿箱统计修复、置顶独立、README 快速预览

- **草稿箱统计修复**：mail_db.sh 修复 draft_user_emails 变量名拼写；stats 与 list 查询逻辑统一（使用 draft_user_emails IN）；Mail.vue 草稿箱视图使用 totalEmails 确保「共 N 封」与内容一致
- **置顶按文件夹独立**：pin_email 仅更新当前 folder_id 内同封邮件；收件箱置顶不影响已发送箱
- **多用户置顶独立**：email_recipients 新增 is_pinned 字段；收件箱/垃圾邮件按收件人独立置顶；已发送/草稿箱/已删除仍按 emails.is_pinned
- **README 快速预览**：添加 images 目录下多张截图（首页、登录、仪表盘、邮件、系统管理、高级功能、左导航栏等）
- **升级步骤**：`./start.sh restart`；首次升级需运行 `mail_db.sh init` 以添加 email_recipients.is_pinned 字段

---

### V5.8.3 (2026-03-02) - 公网DNS与Bind逻辑完全一致

- **dns_setup.sh 脚本内添加**：公网 public 分支增加与 Bind 相同的 add_domain 逻辑（localhost + 公网域名）
- **server.js 回调增强**：使用 `cd ROOT_DIR` 确保脚本工作目录；与 Bind 相同的 mailDbScript 路径；添加 `mail_setup.sh configure` 配置邮件系统
- **默认域名与邮件收发**：dns.public.domain 已保存，新域名自动显示为默认域名；mail_setup configure 配置 Postfix/Dovecot 支持收发
- **升级步骤**：`./start.sh restart`

---

### V5.8.2 (2026-03-02) - 公网DNS域名自动添加到邮件域管理

- **公网DNS域名自动添加**：`/api/dns/public-configure` 配置成功后，调用 `mail_db.sh add_domain` 将 localhost 与公网域名写入 virtual_domains 表
- **与Bind逻辑一致**：先确保 localhost 存在，再添加公网域名；mail_db.sh 内部检查存在则跳过、不存在则插入
- **升级步骤**：`./start.sh restart`

---

### V5.8.1 (2026-03-02) - 版本默认值与文档更新

- **版本获取失败默认值**：versionManager.ts、server.js 返回 0.0.1；各页面初始/失败时显示 V0.0.1
- **README 技术栈版本**：与实际 npm list 一致（Axios、Chart.js、Morgan、ws、node-pty 等）
- **首页 Landing 更新**：核心脚本 21、业务逻辑层描述、版本默认 V0.0.1
- **升级步骤**：`./start.sh rebuild` 与 `./start.sh restart`

---

### V5.8.0 (2026-03-02) - 邮件分页优化、收件人存储检查与多选

- **邮件分页优化**：Mail.vue 分页切换动画缩短、去掉 stagger；分页时自动滚动到顶部
- **Dashboard 按钮顺序**：Dashboard.vue「邮件服务管理」移至「广播」前面
- **左侧导航顺序**：Layout.vue「邮件存储」移至「命令终端」上方
- **收件人存储检查**：server.js 发送前检查收件人存储空间；主收件人任一满则整封拒发；抄送满则仅跳过该人
- **收件人多选**：Mail.vue 选择收件人弹窗支持多选、复选框、确认按钮、已选标签展示
- **升级步骤**：`./start.sh rebuild` 与 `./start.sh restart`

---

### V5.7.3 (2026-03-02) - Apache 配置备份规范化与 DirectoryIndex 修复

- **Apache 配置备份规范化**：创建 `/etc/httpd/conf.d/_old`，将 `*.backup.*`、`*.bak.*` 等备份移入 _old 避免被 `IncludeOptional conf.d/*.conf` 加载；_old 保留最近 10 个，自动清理过旧备份
- **禁用标记文件优化**：welcome/autoindex/userdir/ssl.conf.disabled 含注释 `# Disabled by XM mail-ops`，避免空文件歧义
- **DirectoryIndex 修复**：httpd-vhost.conf、cert_setup.sh、dns_setup.sh 的 `<Directory /var/www/mail-frontend>` 添加 `DirectoryIndex index.html`，修复「No matching DirectoryIndex (index.html) found」错误
- **cert_setup.sh 备份合规**：mailmgmt.conf 及旧格式 HTTP 配置备份写入 conf.d/_old
- **for 循环语法修复**：移除 for 循环 in 列表中错误的 `2>/dev/null`，修复「syntax error near unexpected token `2'」」
- **升级步骤**：`./start.sh restart`

---

### V5.7.2 (2026-03-02) - CSRF 保护扩展与删除域名修复

- **CSRF 保护扩展**：公网 DNS 配置、邮件标签删除、测试系统通知、终端命令执行等接口补充 csrfProtection
- **删除域名弹窗修复**：Settings.vue Teleport 挂载到 body、z-index 9999、body overflow 控制
- **删除域名 500 修复**：脚本路径统一、ID 匹配修复、错误信息透传
- **升级步骤**：`./start.sh restart`

---

### V5.7.1 (2026-03-01) - Dashboard UI 全面更新与用户管理优化

- **系统管理按钮 UI 重新设计**：左栏风格、顶部色条、图标+描述，统一管理面板卡片
- **高级功能、用户管理 UI 更新**：与系统管理统一风格；高级功能采用磁贴风格（顶部色条、居中图标）与系统管理区分
- **用户管理多选优化**：全选/取消全选逻辑修复（selectableUsers 判断）；移动端全选条、复选框 44px 触摸区域
- **添加新用户表单 UI 更新**：卡片化、翠绿主题、输入框与按钮样式统一
- **广播消息 403 修复**：saveBroadcast、confirmClearBroadcast 添加 csrfHeader()
- **升级步骤**：`./start.sh rebuild` 与 `./start.sh restart`

---

### V5.7.0 (2026-03-01) - 邮件详情预加载与已读逻辑优化

- **邮件详情预加载**：列表加载后预取当前页、翻页时预取新页、悬停约 120ms 预取，点击时若已缓存则秒开
- **预加载不标记已读**：预取时传 `prefetch=1`，后端 get_email_detail 支持 skip_mark_read，仅用户实际点击打开时才标记已读
- **邮件详情加载优化**：emailDetailLoading 状态、有正文时不显示加载中、移除兜底块
- **升级步骤**：`./start.sh rebuild` 与 `./start.sh restart`

---

### V5.6.3 (2026-02-28) - 邮件设置数字输入修复与区间说明

- **邮件设置数字输入修复**：type="text" 替代 type="number"、onMailNumericInput、blur 校验
- **邮件设置区间说明**：（5-25 条、100-50000 MB、1-1024 MB）
- **最大邮箱大小上限**：50000 MB
- **安全设置输入优化**：移除 @change
- **升级步骤**：`./start.sh rebuild` 与 `./start.sh restart`
...
### V5.6.2 (2026-02-28) - 页脚精简与联系邮箱更新

- **页脚精简**：Landing.vue、Changelog.vue 删除「支持」和「联系我们」链接
- **联系邮箱更新**：页脚邮箱地址由 xm@localhost 改为 admin@xm666.fun
- **升级步骤**：`./start.sh rebuild` 与 `./start.sh restart`

---

### V5.6.1 (2026-02-28) - 左侧导航栏 UI 全面更新

- **左侧导航栏 UI 全面更新**：Layout.vue 玻璃拟态、统一导航项样式、分组标题与分隔线、底部用户信息美化
- **左上角品牌区域现代化**：Logo 蓝紫渐变发光、副标题「邮件管理平台」、背景装饰
- **Logo 图标与蓝框比例优化**：Logo 图片放大、蓝框尺寸与发光减弱
- **升级步骤**：`./start.sh rebuild` 与 `./start.sh restart`

---

### V5.6.0 (2026-02-28) - 邮件已读显示统一与移动按钮修复

- **邮件已读显示统一**：收件箱、已发送、草稿箱、垃圾邮件、已删除、自定义文件夹点击后立即显示已读；mail_db.sh get_email_detail、mark_email_read 支持各文件夹
- **移动按钮修复**：Mail.vue 移动下拉 window 防护、getAvailableFolders 防御、openMoveDropdown try-catch
- **演示数据**：demo-mail-data.sql 域名 skills.com → xm666.fun
- **升级步骤**：`./start.sh rebuild` 与 `./start.sh restart`

---

### V5.5.2 (2026-02-26) - 环境检查与诊断完善

- **环境检查调用 start.sh check**：run_diagnosis.sh、Dashboard 环境检查按钮、终端风格 UI、11 项诊断
- **安装完成提示框优化**：框宽自动计算、右边界对齐
- **脚本与代码清理**：dns_setup.sh 删除第三方 DNS API 死代码、app_user.sh fix-email-domains
- **升级步骤**：`./start.sh rebuild` 与 `./start.sh restart`

---

### V5.5.1 (2026-02-26) - 安装完成亲和提示

- **安装完成亲和提示**：start.sh 部署完成后显示友好提示框，包含访问地址（自动获取服务器 IP）、默认用户名/密码、建议操作、诊断命令
- **升级步骤**：拉取代码后 `./start.sh restart`

---

### V5.5.0 (2026-02-26) - 端口与路径全面适配

- **端口与路径全面适配**：端口从 config/port-config.json 读取；路径统一使用 ${BASE_DIR}；移除 /bash 硬编码；setup-logs 改为 mail_service_logger.sh install
- **升级步骤**：拉取代码后 `./start.sh fix-dispatcher` 与 `./start.sh restart`

---

### V5.4.2 (2026-02-27) - 系统设置删除为空时恢复默认值

- **系统设置删除为空时恢复默认值**：Settings.vue 安全设置（会话超时、最大登录尝试次数、密码最小长度）和邮件设置（最大邮箱大小、最大邮件大小）在 blur/change 时若为空或无效则恢复默认值；executeSaveSystemSettings 与 saveMailSettings 保存前兜底
- **升级步骤**：拉取代码后 `./start.sh rebuild` 与 `./start.sh restart`（仅前端变更）

---

### V5.4.1 (2026-02-27) - 系统设置恢复、彻底删除释放存储、自定义文件夹修复

- **系统设置重装后恢复**：start.sh 备份恢复增加 maxMailboxSize 检查
- **彻底删除释放存储**：mail_db.sh hard_delete_email 删除附件文件、base_message_id 删除；Mail.vue 彻底删除后刷新
- **已删除文件夹删除按钮**：详情中 view=trash 时改为彻底删除，新增确认对话框
- **自定义文件夹修复**：server.js safeMailFolder、mail_db.sh safe_folder_param、get_emails user_id 过滤
- **升级步骤**：拉取代码后 `./start.sh restart-dispatcher`

---

### V5.4.0 (2026-02-27) - 邮件存储统计修复与邮件设置自动保存 403 修复

- **邮件存储使用情况统计修复**：mail_db.sh store_email 收件人规范化、size_bytes 含附件、get_mail_stats 格式兼容、normalize-recipients 命令
- **邮件设置自动保存 403 修复**：Settings.vue saveMailSettings 添加 csrfHeader()
- **升级步骤**：拉取代码后 `./start.sh rebuild` 与 `./start.sh restart`

---

### V5.3.5 (2026-02-27) - 系统状态与日志查看弹窗修复

- **系统状态与日志查看弹窗修复**：Layout.vue 系统状态、日志查看对话框使用 Teleport 挂载到 body；关闭逻辑改用 closeSystemStatusDialog/closeLogViewerDialog 函数，修复 ref 赋值导致的 `Cannot create property 'value' on boolean 'true'` 错误
- **Dashboard 与弹窗显示修复**：Layout main 添加 min-h-0；Dashboard 动画添加 animation-fill-mode: forwards；弹窗遮罩改用 Tailwind v4 语法
- **升级步骤**：拉取代码后 `./start.sh rebuild` 与 `./start.sh restart`

---

### V5.3.4 (2026-02-27) - 附件存储 Node.js 化与下载修复

- **附件存储改用 Node.js**：新增 store_attachments.js，mail_db.sh 调用替代 Python；支持 path 型与 content 型
- **附件下载 F12 报错修复**：Mail.vue downloadAttachment 改用 Blob + URL.createObjectURL
- **升级步骤**：拉取代码后 `./start.sh rebuild` 与 `./start.sh restart`

---

### V5.3.3 (2026-02-27) - 附件分片上传支持 1GB 大文件

- **分片上传 API**：POST /api/mail/upload-chunk，单分片 10MB，单文件最大 1GB；分片写入 `/tmp/mail-uploads/{uploadId}/`，全部到齐后合并为 `final_{filename}`
- **发送 API 增强**：支持 `{ name, uploadId }` 与 `{ name, content }` 两种附件格式；单文件 1GB、总附件 2GB
- **大附件下载**：GET /api/mail/attachment/:emailId/:attachmentId 校验权限后从 file_path 流式返回
- **mail_db.sh**：附件存储支持 `{ name, path, type, size }`，复制至 `/var/mail-ops/attachments/` 并写入 file_path
- **前端 Mail.vue**：超过 5MB 自动分片上传；file_path 附件通过 API 流式下载
- **升级步骤**：拉取代码后执行 `./start.sh fix-dispatcher` 创建附件目录，再 `./start.sh rebuild` 与 `./start.sh restart`

---

### V5.3.2 (2026-02-27) - 邮件详情页与模态框修复

- **邮件详情页空白修复**：mail_db.sh 过滤逻辑、304 缓存、Teleport、遮罩透明度；选择收件人、文件夹管理、编辑文件夹、邮件详情均使用 Teleport 挂载
- **升级步骤**：拉取代码后 `./start.sh rebuild` 与 `./start.sh restart`

---

### V5.3.1 (2026-02-26) - 邮件服务端口管理

- **邮件服务管理**：Dashboard 高级功能新增「邮件服务管理」按钮（紫色主题），弹窗可单独启用/禁用 Postfix 端口 25（SMTP）、587（Submission）与 Dovecot 端口 993（IMAPS）、995（POP3S）
- **服务状态检测**：启动服务/停止服务按钮根据 postfix、dovecot 运行状态自动禁用，避免重复点击
- **mail_port_control.sh**：新增脚本，通过修改 Postfix master.cf 与 Dovecot 10-master.conf 实现按端口控制
- **CRLF 兼容**：API 执行脚本前通过 `sed 's/\r$//'` 去除 Windows 换行符，解决 `$'\r': 未找到命令` 等报错
- **start.sh**：部署 mail-service-control sudoers（含 mail_port_control.sh 执行权限）
- **升级步骤**：拉取代码后 `./start.sh fix-dispatcher` 或 `./start.sh start` 部署 sudoers

---

### V5.3.0 (2026-02-26) - Node.js 24 LTS 与权限/删除修复

- **Node.js 24.x LTS**：NodeSource setup_24.x，目标 v24.14.0 (Krypton)，支持至 2028 年
- **config 目录权限**：xm 用户创建后再次 chown config；fix-dispatcher 增加 config 权限修复
- **证书与 SSL 删除**：删除证书文件、Apache 配置文件改用 `sudo rm -f`
- **require is not defined 修复**：移除 ES 模块中的 `require('child_process')`
- **Tailwind 升级**：`@tailwindcss/upgrade --force`；非 git 环境执行 `git init` 兜底
- **Apache 模块检测**：grep 模式改为 `xxx_module`
- **升级步骤**：拉取后 `./start.sh fix-dispatcher` 或 `./start.sh start`

---

### V5.2.4 (2026-02-26) - hostname 部署前检查与 dnf 输出优化

- **hostname 部署前检查**：require_root 后调用 check_hostname，禁止空/localhost/纯数字
- **dnf update 输出优化**：PYTHONUNBUFFERED、tr 转 \r
- **升级步骤**：拉取后无需额外操作

---

### V5.2.3 (2026-02-26) - 前端依赖升级与 Tailwind v4 迁移

- **版本号与文档**：README 与 UPDATE_GUIDE 统一更新至 5.2.3
- **前端依赖升级**：Vite 7.3.1、@vitejs/plugin-vue 6.0.4、Tailwind CSS 4.2.1、@tailwindcss/vite 4.2.1、@xterm/xterm 6.0.0
- **Tailwind v4 迁移**：移除 tailwind.config.js 与 postcss.config.js，使用 @theme 在 CSS 中配置
- **start.sh 集成**：部署与 rebuild 流程中自动执行 `npx @tailwindcss/upgrade`
- **升级步骤**：若已部署，拉取代码后 `./start.sh rebuild` 与 `./start.sh restart`

---

### V5.2.2 (2026-02-12) - 申请证书功能整合与按钮顺序调整

- **版本号与文档**：README 与 UPDATE_GUIDE 统一更新至 5.2.2
- **申请证书功能整合**：将Dashboard独立的"申请证书"按钮移除，功能整合到SSL管理对话框中，作为第一个标签页
- **按钮顺序调整**：将"管理SSL"按钮移动到"备份功能"按钮前面
- **升级步骤**：若已部署，拉取代码后 `./start.sh rebuild` 与 `./start.sh restart`

---

### V5.2.1 (2026-02-12) - 垃圾邮件权限修复与DNS配置CSRF修复

- **版本号与文档**：README 与 UPDATE_GUIDE 统一更新至 5.2.1；README 仅保留最新版本与版本列表，UPDATE_GUIDE 保留历史版本并更新最新版本
- **垃圾邮件权限修复**：
  - **问题**：`get_emails()` 函数中缺少垃圾邮件文件夹（spam）的用户过滤处理，导致会显示所有用户的垃圾邮件
  - **修复**：在 `get_emails()` 函数中添加垃圾邮件文件夹的处理分支，与收件箱逻辑一致，只显示收件人是当前用户的垃圾邮件
  - **实现**：支持完整邮箱地址、xm用户、普通用户名的匹配，使用SQL转义防止注入
- **DNS配置CSRF修复**：
  - **问题**：DNS配置API调用（本地Bind和公网DNS）缺少CSRF token，导致"Missing CSRF token"错误
  - **修复**：在 `Dashboard.vue` 中为 `/api/dns/configure` 和 `/api/dns/public-configure` 的 `fetch` 请求添加 `csrfHeader()`
  - **实现**：在请求头中添加 `X-CSRF-Token`，与后端 `csrfProtection` 中间件要求一致
- **升级步骤**：若已部署，拉取代码后 `./start.sh rebuild` 与 `./start.sh restart`

---

### V5.2.0 (2026-02-12) - 收件箱未读按钮提示逻辑重新设计

- **版本号与文档**：README 与 UPDATE_GUIDE 统一更新至 5.2.0；README 仅保留最新版本与版本列表，UPDATE_GUIDE 保留历史版本并更新最新版本
- **收件箱未读按钮提示逻辑重新设计**：
  - **加载状态管理**：添加 `unreadCountLoading` 状态，加载中显示旋转动画，加载完成后显示未读数量
  - **数据解析优化**：确保未读数量正确解析为数字（处理字符串/数字类型），添加类型转换和 NaN 检查
  - **错误处理增强**：添加完整的错误处理和调试日志，请求失败或发生错误时重置为 0
  - **显示优化**：未读数量 > 0 时显示红色渐变背景和脉冲动画，未读数量 = 0 时显示灰色背景
  - **文件夹逻辑优化**：确保只有收件箱显示未读数量，其他文件夹（已发送、草稿箱、垃圾邮件、已删除）不显示未读数量
- **邮件统计查询修复**：
  - **收件箱统计修复**：修复 `GROUP BY SUBSTRING_INDEX(message_id, '_', 1)` 问题，改为按完整 `message_id` 分组
  - **已发送统计修复**：移除 `SUBSTRING_INDEX`，直接统计
  - **已删除统计修复**：改为按完整 `message_id` 分组
  - **垃圾邮件统计修复**：改为按完整 `message_id` 分组，优化邮箱匹配逻辑（与列表查询保持一致）
  - **草稿箱统计修复**：改为按完整 `message_id` 分组
  - **文件夹统计修复**：改为按完整 `message_id` 分组
  - **搜索查询修复**：改为按完整 `message_id` 分组
- **升级步骤**：若已部署，拉取代码后 `./start.sh rebuild` 与 `./start.sh restart`

---

### V5.1.4 (2026-02-10) - 操作日志深色模式适配

- **版本号与文档**：README 与 UPDATE_GUIDE 统一更新至 5.1.4；README 仅保留最新版本与版本列表，UPDATE_GUIDE 保留历史版本并更新最新版本
- **操作日志深色模式适配**：仪表盘操作日志弹窗（容器、标题栏、状态标签、日志内容区、关闭按钮）与运行命令后底部「状态显示」内联操作日志面板（容器、加载/错误块、标题、状态标签、日志内容区）全面增加 `dark:` 样式，与现有深色主题统一
- **主要更新内容**：
  - **README.md**：标题与「版本更新」章节更新为 V5.1.4；版本历史表新增 V5.1.4 行
  - **UPDATE_GUIDE.md**：标题更新为 V5.1.4；新增「最新版本 V5.1.4」章节；原 V5.1.3 内容移入「版本历史记录」
  - **frontend/src/modules/Dashboard.vue**：操作日志弹窗与底部内联状态/操作日志面板增加深色模式类（如 dark:bg-gray-800、dark:border-gray-600、dark:text-gray-100、状态标签与日志区 dark 变体等）

---

### V5.1.3 (2026-02-09) - Apache 配置模板优化

- **版本号与文档**：README 与 UPDATE_GUIDE 统一更新至 5.1.3；README 仅保留最新版本与版本列表，UPDATE_GUIDE 保留历史版本并更新最新版本
- **Apache 配置模板优化**：`backend/apache/httpd-vhost.conf` 配置结构优化，添加全局超时设置（`ProxyTimeout 60`、`Timeout 60`），完善安全头配置（`X-Frame-Options SAMEORIGIN`、`X-Content-Type-Options nosniff`、`X-XSS-Protection "1; mode=block"`），改进 WebSocket 代理配置（使用 `<Location>` 块 + `upgrade=websocket` 参数，保留回退方案），优化 Vue Router 规则（在 LocationMatch 中排除 `/uploads/` 路径），确保所有 `<Location>` 和 `<Directory>` 块包含认证禁用设置（`AuthType None`、`Satisfy Any`）
- **升级步骤**：若已部署，拉取代码后 `./start.sh start` 或手动替换配置后 `systemctl restart httpd`

---

### V5.1.2 (2026-02-09) - 系统状态 DNS 公网检测修复

- **版本号与文档**：README 与 UPDATE_GUIDE 统一更新至 5.1.2
- **系统状态 DNS 公网检测修复**：`server.js` 中 `/api/system-status` 增加兜底逻辑，有域名且 named 未运行则强制使用 8.8.8.8 做健康检查

---

### V5.1.1 (2026-02-04) - CSRF 防护与前端 XSS 加固

- **版本号与文档**：README 与 UPDATE_GUIDE 统一更新至 5.1.1
- **CSRF 防护落地**：`server.js` 新增 `csrfProtection` 中间件，对关键写接口校验 `X-CSRF-Token`，缺失返回 403
- **日志查看 XSS 加固**：`Layout.vue` 日志查看器对动态内容统一 `escapeHtml` 后渲染
- **安全审计补充**：文档补充与整理

---

### V5.1.0 (2026-02-04) - 公网 DNS 适配、顶部栏手机端与默认菜单折叠
...
- **调度层**：Node.js 24.x (v24.14.0 LTS Krypton) + Express.js 5.2.1
...
**如果前端构建失败**：
- 检查 Node.js 版本是否符合要求（Node.js 24.x，推荐 v24.14.0 LTS Krypton）
