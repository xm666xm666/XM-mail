...
#   app_user.sh init                          - 列出 MySQL 所有用户信息（mysql.user）
#   app_user.sh schema                        - 初始化应用用户数据库表结构（app_users, app_accounts）
#   app_user.sh register <用户名> <邮箱> <密码> - 注册新用户（同时创建app_users、mail_users、virtual_users和Maildir）
#   app_user.sh login <用户名> <密码>         - 用户登录验证（支持用户名或邮箱登录）
#   app_user.sh reset <用户名> <新密码>        - 重置用户密码
#   app_user.sh update <原用户名> [新用户名] [邮箱] [密码] [头像URL] - 更新用户信息（字段可选，不存在则创建）
#   app_user.sh query-users                   - 查询应用用户列表（JSON格式）
#   app_user.sh delete-user <邮箱>            - 删除应用用户（同时删除mail_users、virtual_users、Maildir和邮件记录）
#   app_user.sh check-user-exists <用户名>    - 检查用户是否存在（检查app_users和mail_users表）
#   app_user.sh fix-email-domains [目标域名]  - 批量修复用户邮箱域名（localhost 等替换为目标域名）
#
# 功能描述:
#   - 用户注册：创建新的应用用户账户（app_users表），同时同步到mail_users、virtual_users和Maildir
#   - 用户登录：验证用户凭据（支持用户名或邮箱登录），兼容app_users和app_accounts表
#   - 密码管理：密码使用SHA512哈希存储，不存储明文
#   - 用户查询：查询用户信息和状态，支持JSON格式输出
#   - 用户删除：删除应用用户时同步删除mail_users、virtual_users、Maildir和相关邮件记录
...
#   - virtual_users(id, domain_id, email, password, active) - Postfix虚拟用户表（注册时同步，maildb数据库）
#
# 数据库来源说明:
...
#   - maildb数据库：由mail_db.sh init命令创建邮件系统核心表（emails, email_folders等9张表）
#
# 依赖关系:
#   - MariaDB数据库（mailapp和maildb数据库）
#   - user_manage.sh（注册/删除时同步 virtual_users 与 Maildir：user-add / user-del）
...
register_user() {
  ...
  # 同步创建 virtual_users 和 Maildir（Postfix/Dovecot 收发邮件必需）
  local user_manage_script="${BASE_DIR}/backend/scripts/user_manage.sh"
  if [[ -x "$user_manage_script" || -f "$user_manage_script" ]]; then
    if bash "$user_manage_script" user-add "$email" "$password" 2>/dev/null; then
      log "已同步创建邮件账户与 Maildir: $email"
    else
      echo "警告: 无法同步创建 virtual_users/Maildir，用户可能无法通过 SMTP/IMAP 收发邮件" >&2
    fi
  else
    echo "警告: 未找到 user_manage.sh，跳过 virtual_users 与 Maildir 创建" >&2
  fi
...
delete_app_user() {
  ...
  if [[ $? -eq 0 ]]; then
    ...
    # 同时删除 virtual_users 和 Maildir（Postfix/Dovecot 邮件账户）
    local user_manage_script="${BASE_DIR}/backend/scripts/user_manage.sh"
    if [[ -f "$user_manage_script" ]]; then
      if bash "$user_manage_script" user-del "$email" 2>/dev/null; then
        echo "已同步删除邮件账户与 Maildir: $email" >&1
      else
        echo "警告: 无法同步删除 virtual_users/Maildir" >&2
      fi
    fi
...
case "${1:-help}" in
  ...
  check-user-exists)
    ...
    check_user_exists "$2" ;;
  fix-email-domains)
    fix_user_email_domains "${2:-}" ;;
  *)
    echo 'usage: app_user.sh {init(查看MySQL用户)|query-users(查询应用用户)|schema(建表)|register|login|reset|update|check-user-exists|fix-email-domains}' >&2; exit 2 ;;
esac
