...
# ============================================================================
# 脚本名称: mail_init.sh
# 工作职责: 邮件系统初始化脚本 - 通过 mail_db.sh 初始化 MariaDB 邮件表并导入示例数据
# 系统组件: XM邮件管理系统 - 初始化模块
# ============================================================================
# 用法说明:
#   mail_init.sh <action>
#   mail_init.sh init                     - 初始化邮件数据库和表结构（调用 mail_db.sh init，MariaDB）
#   mail_init.sh sample                   - 导入示例邮件数据用于测试
#   mail_init.sh services                 - 启动并启用 Postfix 和 Dovecot 服务
#   mail_init.sh monitor                  - 配置邮件监控 systemd 服务
#   mail_init.sh all                      - 执行所有操作（init + sample + services + monitor）
#
# 功能描述:
#   - 数据库初始化：调用 mail_db.sh init 创建 MariaDB 邮件表（非 SQLite）
#   - 示例数据导入：添加示例邮件用于测试
#   - 服务管理：确保 Postfix 和 Dovecot 运行并启用开机自启
#   - 监控配置：创建 mail-monitor systemd 服务
#   - 日志记录：/var/log/mail-ops/mail-init.log
#   - 无 reset 子命令
#
# 数据库来源说明:
#   - maildb：virtual_* 由 db_setup.sh init 创建；邮件核心表由 mail_db.sh init 创建
#
# 依赖关系:
#   - mail_db.sh（MariaDB 邮件数据库管理）
#   - MariaDB 数据库
