...
#   mail_db.sh store <邮件ID> <发件人> <收件人> <主题> <正文> <HTML> <文件夹> <大小> <抄送> <附件> <头部> - 存储邮件到数据库（附件通过 store_attachments.js 处理，支持 path 型与 content 型；收件人规范化存储）
#   mail_db.sh list <用户邮箱> <文件夹> [每页数量] [偏移量] - 查询用户邮件列表（文件夹 lookup 增加 user_id 与 is_active 过滤，支持自定义文件夹）
#   mail_db.sh search <用户邮箱> <搜索关键词> [文件夹] [每页数量] [偏移量] - 搜索用户邮件（支持关键词搜索，folder 默认 all）
...
#   mail_db.sh hard-delete <邮件ID> <用户邮箱> - 彻底删除邮件（删除前先删除磁盘附件文件，按 base_message_id 删除所有相关记录，不可恢复）
#   mail_db.sh fix-file-paths - 修复数据库中 body/html_body 存储了文件路径的邮件
#   mail_db.sh normalize-recipients - 一次性修复历史数据中 "Name <email>" 格式的 email_recipients
#   mail_db.sh move <邮件ID> <文件夹> <用户邮箱> - 移动邮件到指定文件夹（移动到已删除文件夹时会自动记录原文件夹）
#   mail_db.sh pin <邮件ID> [0|1] <用户邮箱> - 置顶(1)或取消置顶(0)邮件，默认1
#   mail_db.sh stats <用户邮箱>          - 获取用户邮件统计信息（含各文件夹邮件数、未读数、存储使用，get_mail_stats 支持收件人格式兼容）
#   mail_db.sh cleanup [天数]            - 清理过期邮件（默认30天）
#   mail_db.sh mark_read <邮件ID> [用户邮箱] - 标记邮件为已读
#
#   用户管理:
#   mail_db.sh add-user <用户名> <邮箱> [显示名] - 添加邮件用户
#   mail_db.sh list-users               - 列出所有邮件用户
#   mail_db.sh list_users               - 列出所有邮件用户（JSON 格式，供 API 使用）
...
#   mail_db.sh delete_domain <域名ID>   - 删除域名（按 virtual_domains.id，非域名名称）
#
#   文件夹管理:
#   mail_db.sh folders [用户ID]         - 获取文件夹列表（包括系统文件夹和自定义文件夹，用户ID为 mail_users 表的 id；safe_folder_param 支持自定义文件夹名）
...
#
#   图表统计:
#   mail_db.sh sending-trends [hour|day|week] - 邮件发送趋势（按时间周期，默认 day）
#   mail_db.sh frequency-analysis [user|day] - 发送频率分析（group_by，默认 user）
#
# 功能描述:
#   邮件管理功能:
#   - 邮件存储：将接收到的邮件存储到数据库，支持多附件、多收件人
#   - 邮件检索：根据条件查询和检索邮件，支持 list 列表、search 关键词搜索、分页、文件夹过滤、标签过滤
...
# 安全审计：SQL/参数转义与校验（防止注入）
# 对传入脚本的 user/folder 等参数进行转义或白名单校验后再参与 SQL 或子进程
safe_sql_escape() {
  echo "$1" | sed "s/'/''/g"
}
# 文件夹名白名单：仅允许系统文件夹名或数字 ID，否则返回 inbox
safe_folder_param() {
  local f="$1"
  if [[ "$f" =~ ^[0-9]+$ ]]; then
    echo "$f"
  ...
  case "$f" in
    inbox|sent|drafts|trash|spam|all) echo "$f" ;;
    *)
      # 自定义文件夹：仅允许字母、数字、下划线、连字符（与 add-folder 命名规则一致）
      if [[ "$f" =~ ^[a-zA-Z0-9_-]+$ ]] && [[ ${#f} -le 100 ]]; then
        echo "$f"
      else
        echo "inbox"
      fi
...
# 限制 user 参数为合理格式（用户名或邮箱），去除危险字符后用于 SQL
safe_user_param() {
  local u="$1"
  # 只保留字母、数字、@.-_ 防止 SQL/Shell 注入
  echo "$u" | sed "s/[^a-zA-Z0-9@._-]//g"
}
...
init_mail_db() {
  ...
  # 检查并添加is_pinned字段（置顶功能）
  local has_is_pinned=$(mysql -h "$DB_HOST" -u "$DB_USER" -p"$(cat "$DB_PASS_FILE")" "$DB_NAME" -s -r -e "
    SELECT COUNT(*) FROM information_schema.COLUMNS
    WHERE TABLE_SCHEMA='$DB_NAME'
    AND TABLE_NAME='emails'
    AND COLUMN_NAME='is_pinned';
  " 2>/dev/null | tail -1)
  if [[ "$has_is_pinned" -eq 0 ]]; then
    log "添加is_pinned字段..."
    mysql_connect "ALTER TABLE emails ADD COLUMN is_pinned TINYINT(1) DEFAULT 0 COMMENT '是否置顶' AFTER is_deleted;"
    mysql_connect "CREATE INDEX IF NOT EXISTS idx_emails_pinned ON emails(is_pinned);"
  fi
  ...
  mysql_connect "CREATE TABLE IF NOT EXISTS email_recipients (
    ...
    is_pinned TINYINT(1) DEFAULT 0 COMMENT '该收件人是否置顶',
    is_forwarded TINYINT(1) DEFAULT 0 COMMENT '该收件人收到的邮件是否为转发',
  ...
  # 检查并添加 email_recipients.is_pinned（收件箱/垃圾邮件按收件人独立置顶，必须在表创建之后）
  local has_er_pinned=$(mysql -h "$DB_HOST" -u "$DB_USER" -p"$(cat "$DB_PASS_FILE")" "$DB_NAME" -s -r -e "
    SELECT COUNT(*) FROM information_schema.COLUMNS
    WHERE TABLE_SCHEMA='$DB_NAME'
    AND TABLE_NAME='email_recipients'
    AND COLUMN_NAME='is_pinned';
  " 2>/dev/null | tail -1)
  if [[ "$has_er_pinned" -eq 0 ]]; then
    log "添加 email_recipients.is_pinned 字段（多用户独立置顶）..."
    mysql_connect "ALTER TABLE email_recipients ADD COLUMN is_pinned TINYINT(1) DEFAULT 0 COMMENT '该收件人是否置顶' AFTER is_read;"
  fi

  # 检查并添加 email_recipients.is_forwarded（该收件人收到的邮件是否为转发邮件）
  local has_er_forwarded=$(mysql -h "$DB_HOST" -u "$DB_USER" -p"$(cat "$DB_PASS_FILE")" "$DB_NAME" -s -r -e "
    SELECT COUNT(*) FROM information_schema.COLUMNS
    WHERE TABLE_SCHEMA='$DB_NAME'
    AND TABLE_NAME='email_recipients'
    AND COLUMN_NAME='is_forwarded';
  " 2>/dev/null | tail -1)
  if [[ "$has_er_forwarded" -eq 0 ]]; then
    log "添加 email_recipients.is_forwarded 字段（转发邮件标识）..."
    mysql_connect "ALTER TABLE email_recipients ADD COLUMN is_forwarded TINYINT(1) DEFAULT 0 COMMENT '该收件人收到的邮件是否为转发' AFTER is_pinned;"
  fi
...
store_email() {
  ...
  # 处理附件数据（文件路径时保留不删除，供 Python 直接读取，避免大内容通过 heredoc 传递导致截断）
  local attachments=""
  local attachments_file_path=""  # 供 Python 读取的路径（仅当为文件时有效）
  if [[ -n "$attachments_file" ]]; then
    # 如果attachments_file是JSON字符串（不以/开头），直接使用
    if [[ ! "$attachments_file" =~ ^/ ]]; then
      attachments="$attachments_file"
    # 如果attachments_file是文件路径，仅记录路径供 Python 读取，不读入变量（避免大 base64 截断）
    elif [[ -f "$attachments_file" ]]; then
      attachments_file_path="$attachments_file"
      # 先验证 JSON 格式
      if python3 -m json.tool < "$attachments_file" >/dev/null 2>&1; then
        attachments="[valid]"  # 占位，表示有有效附件，Python 会从文件读取
      else
        log "警告: 附件文件不是有效JSON: $attachments_file"
      fi
    fi
  fi
  ...
  # 对于从文件读取的附件，不在这里记录内容（避免日志过大）
  if [[ -z "$attachments_file_path" ]]; then
    log "附件数据: $attachments"
  else
    log "附件数据: 从文件读取 ($attachments_file_path)"
  fi
  ...
  # 规范化邮箱地址：从 "Name <email@domain.com>" 格式提取纯邮箱，便于存储统计匹配
  normalize_email_addr() {
    local a="$1"
    a=$(echo "$a" | xargs)
    if [[ "$a" == *"<"*">"* ]]; then
      a=$(echo "$a" | sed -n 's/.*<\([^>]*\)>.*/\1/p' | xargs)
    fi
    echo "$a"
  }
  # 处理主收件人（to）
  if [[ -n "$to_addr" ]]; then
    ...
    for addr in "${TO_ADDRESSES[@]}"; do
      addr=$(normalize_email_addr "$addr")
      if [[ -n "$addr" ]]; then
        local escaped_addr=$(echo "$addr" | sed "s/'/''/g")
        # 使用INSERT IGNORE避免重复插入；is_forwarded 用于标识转发邮件
        local insert_result=$(mysql_connect "INSERT IGNORE INTO email_recipients (email_id, recipient_type, email_address, is_forwarded) VALUES ($email_id, 'to', '$escaped_addr', ${is_forwarded:-0});" 2>&1)
        if [[ $? -eq 0 ]]; then
  ...
  if [[ -n "$cc_addr" ]]; then
    ...
    for addr in "${CC_ADDRESSES[@]}"; do
      ...
      if [[ -n "$addr" ]]; then
        local escaped_addr=$(echo "$addr" | sed "s/'/''/g")
        # 使用INSERT IGNORE避免重复插入；is_forwarded 用于标识转发邮件
        local insert_result=$(mysql_connect "INSERT IGNORE INTO email_recipients (email_id, recipient_type, email_address, is_forwarded) VALUES ($email_id, 'cc', '$escaped_addr', ${is_forwarded:-0});" 2>&1)
        if [[ $? -eq 0 ]]; then
  ...
  if [[ -n "$attachments" || -n "$attachments_file_path" ]]; then
    # 验证：文件路径时已验证，内联JSON时再验证
    local do_store_attachments=false
    if [[ -n "$attachments_file_path" ]]; then
      do_store_attachments=true
    elif echo "$attachments" | python3 -m json.tool >/dev/null 2>&1; then
      do_store_attachments=true
    fi
    
    if [[ "$do_store_attachments" == "true" ]]; then
      # 使用 Node.js 解析 JSON 并插入附件（优先从文件读取，避免大内容截断）
      local store_script="$BASE_DIR/backend/scripts/store_attachments.js"
      if [[ -f "$store_script" ]]; then
        ATTACHMENTS_FILE_PATH="$attachments_file_path" ATTACHMENTS_JSON="$attachments" \
        EMAIL_ID="$email_id" DB_HOST="$DB_HOST" DB_USER="$DB_USER" DB_PASS_FILE="$DB_PASS_FILE" DB_NAME="$DB_NAME" \
        node "$store_script" 2>>"$LOG_DIR/mail-db.log" || true
      else
        log "警告: store_attachments.js 不存在，跳过附件存储: $store_script"
      fi
      # 清理附件临时文件（从文件读取时）
      if [[ -n "$attachments_file_path" && -f "$attachments_file_path" ]]; then
        rm -f "$attachments_file_path" 2>/dev/null || true
      fi
    else
...
get_emails() {
  local user_raw="$1"
  local folder_raw="${2:-inbox}"
  local limit="${3:-50}"
  local offset="${4:-0}"
  # 安全：参数校验与转义，防止 SQL/Shell 注入
  local user
  user=$(safe_user_param "$user_raw")
  [[ -z "$user" ]] && user="unknown"
  local user_escaped
  user_escaped=$(safe_sql_escape "$user")
  local folder
  folder=$(safe_folder_param "$folder_raw")
  [[ "$limit" =~ ^[0-9]+$ ]] || limit=50
  [[ "$offset" =~ ^[0-9]+$ ]] || offset=0
  ...
  # 获取文件夹ID（支持向后兼容）
  # 自定义文件夹需按 user_id 过滤，避免多用户时取错文件夹
  local folder_id=""
  local folder_escaped
  folder_escaped=$(safe_sql_escape "$folder")
  if [[ "$folder" =~ ^[0-9]+$ ]]; then
    folder_id="$folder"
  else
    local mail_user_id=""
    mail_user_id=$(mysql -h "$DB_HOST" -u "$DB_USER" -p"$(cat "$DB_PASS_FILE")" "$DB_NAME" -s -r -e "
      SELECT id FROM mail_users WHERE email='$user_escaped' OR username='$user_escaped' LIMIT 1;
    " 2>/dev/null | tail -1)
    local folder_query=""
    if [[ -n "$mail_user_id" && "$mail_user_id" =~ ^[0-9]+$ ]]; then
      folder_query="SELECT id FROM email_folders WHERE name='$folder_escaped' AND is_active=1 AND (folder_type='system' OR user_id=$mail_user_id) LIMIT 1;"
    else
      folder_query="SELECT id FROM email_folders WHERE name='$folder_escaped' AND is_active=1 LIMIT 1;"
    fi
  ...
  if [ "$folder" = "inbox" ] || [ "$folder_id" = "1" ]; then
    ...
    if [[ "$user" == *@* ]]; then
      # 如果传入的是完整邮箱地址，直接精确匹配（使用转义后的值防注入）
      where_clause="WHERE e.folder_id=1 AND e.is_deleted=0 AND EXISTS (
        ...
        AND er.email_address = '$user_escaped'
      )"
    elif [ "$user" = "xm" ]; then
      # 特殊处理xm用户，支持多种邮箱格式
      where_clause="WHERE e.folder_id=1 AND e.is_deleted=0 AND EXISTS (
      ...
      where_clause="WHERE e.folder_id=1 AND e.is_deleted=0 AND EXISTS (
        ...
        WHERE er.email_id = e.id
        AND er.recipient_type IN ('to', 'cc')
        AND (er.email_address = '$user_escaped' OR er.email_address LIKE '%@$user_escaped' OR er.email_address LIKE '$user_escaped@%')
      )"
    fi
    ...
    if [[ "$user" == *@* ]]; then
      ...
      # 也查询数据库中该用户名的所有邮箱地址
      local db_emails=$(mysql -h "$DB_HOST" -u "$DB_USER" -p"$(cat "$DB_PASS_FILE")" "$DB_NAME" -s -r -e "SELECT email FROM mail_users WHERE username='${username_part}' OR email='$user_escaped';" 2>/dev/null | grep -v "^email$" | grep "@" | sed "s/^/'/" | sed "s/$/'/" | tr '\n' ',' | sed 's/,$//')
      if [[ -n "$db_emails" ]]; then
        user_emails="$user_emails, $db_emails"
      fi
    else
      # 如果传入的是用户名，查询该用户的所有邮箱地址
      local db_emails=$(mysql -h "$DB_HOST" -u "$DB_USER" -p"$(cat "$DB_PASS_FILE")" "$DB_NAME" -s -r -e "SELECT email FROM mail_users WHERE username='$user_escaped';" 2>/dev/null | grep -v "^email$" | grep "@" | sed "s/^/'/" | sed "s/$/'/" | tr '\n' ',' | sed 's/,$//')
      if [[ -n "$db_emails" ]]; then
        user_emails="$db_emails, '${user_escaped}@localhost'"
      else
    ...
    if [[ -z "$user_emails" ]]; then
      ...
      user_emails="'${user_escaped}@localhost'"
    fi
    ...
    # 草稿箱：只显示当前用户创建的草稿（发件人=草稿作者），与已发送一致使用用户邮箱列表
    local draft_user_emails=""
    if [[ "$user" == *@* ]]; then
      local username_part=$(echo "$user" | cut -d'@' -f1)
      draft_user_emails="'$user_escaped', '${username_part}@localhost'"
      local db_emails=$(mysql -h "$DB_HOST" -u "$DB_USER" -p"$(cat "$DB_PASS_FILE")" "$DB_NAME" -s -r -e "SELECT email FROM mail_users WHERE username='${username_part}' OR email='$user_escaped';" 2>/dev/null | grep -v "^email$" | grep "@" | sed "s/^/'/" | sed "s/$/'/" | tr '\n' ',' | sed 's/,$//')
      if [[ -n "$db_emails" ]]; then
        draft_user_emails="$draft_user_emails, $db_emails"
      fi
    else
      local db_emails=$(mysql -h "$DB_HOST" -u "$DB_USER" -p"$(cat "$DB_PASS_FILE")" "$DB_NAME" -s -r -e "SELECT email FROM mail_users WHERE username='$user_escaped';" 2>/dev/null | grep -v "^email$" | grep "@" | sed "s/^/'/" | sed "s/$/'/" | tr '\n' ',' | sed 's/,$//')
      if [[ -n "$db_emails" ]]; then
        draft_user_emails="$db_emails, '${user_escaped}@localhost'"
      else
        draft_user_emails="'${user}@localhost'"
      fi
    fi
    if [[ -z "$draft_user_emails" ]]; then
      draft_user_emails="'${user_escaped}@localhost'"
    fi
    where_clause="WHERE e.folder_id=3 AND e.is_deleted=0 AND e.from_addr IN ($draft_user_emails)"
    echo "[mail_db] 草稿箱查询: user=$user, folder_id=3, draft_user_emails=$draft_user_emails" >> "$LOG_DIR/mail-db.log"
  elif [ "$folder" = "trash" ] || [ "$folder_id" = "4" ]; then
    # 已删除文件夹：显示所有已删除的邮件（不检查is_deleted，因为已删除文件夹中的邮件可能is_deleted=1）
    # 根据用户过滤邮件
    if [[ "$user" == *@* ]]; then
      # 如果传入的是完整邮箱地址
      where_clause="WHERE e.folder_id=4 AND (e.from_addr = '$user_escaped' OR EXISTS (
        SELECT 1 FROM email_recipients er
        WHERE er.email_id = e.id
        AND er.recipient_type IN ('to', 'cc')
        AND er.email_address = '$user_escaped'
      ...
      # 普通用户名
      where_clause="WHERE e.folder_id=4 AND (e.from_addr = '$user_escaped' OR e.from_addr LIKE '%@$user_escaped' OR e.from_addr LIKE '$user_escaped@%' OR EXISTS (
        SELECT 1 FROM email_recipients er
        WHERE er.email_id = e.id
        AND er.recipient_type IN ('to', 'cc')
        AND (er.email_address = '$user_escaped' OR er.email_address LIKE '%@$user_escaped' OR er.email_address LIKE '$user_escaped@%')
    ...
    # 垃圾邮件：只显示收件人是当前用户的垃圾邮件（与收件箱逻辑完全一致）
    # 重要：垃圾邮件应该只显示收件人是当前用户的邮件，排除发件人是当前用户的邮件
    # 注意：user参数可能是完整邮箱地址（如xm@skills.com）或用户名（如xm）
    if [[ "$user" == *@* ]]; then
      # 如果传入的是完整邮箱地址，直接精确匹配（使用转义后的值防注入，与收件箱保持一致）
      where_clause="WHERE e.folder_id=5 AND e.is_deleted=0 AND EXISTS (
        SELECT 1 FROM email_recipients er
        WHERE er.email_id = e.id
        AND er.recipient_type IN ('to', 'cc')
        AND er.email_address = '$user_escaped'
  ...
  if [[ "$user" == *@* ]]; then
    # 如果传入的是完整邮箱地址，直接精确匹配（使用转义值防注入）
    user_email_condition="er_user.email_address = '$user_escaped'"
  elif [ "$user" = "xm" ]; then
    # 特殊处理xm用户，支持多种邮箱格式
    ...
    # 普通用户名
    user_email_condition="(er_user.email_address = '$user_escaped' OR er_user.email_address LIKE '%@$user_escaped' OR er_user.email_address LIKE '$user_escaped@%')"
  fi
  ...
  local result=$(mysql -h "$DB_HOST" -u "$DB_USER" -p"$(cat "$DB_PASS_FILE")" "$DB_NAME" -s -r -e "
    SELECT JSON_ARRAYAGG(
      JSON_OBJECT(
        ...
        'is_pinned', CASE WHEN e.folder_id IN (1, 5) THEN COALESCE(er_user.is_pinned, 0) ELSE COALESCE(e.is_pinned, 0) END,
        'is_forwarded', COALESCE(er_user.is_forwarded, 0),
        'labels', COALESCE((
          SELECT JSON_ARRAYAGG(
            JSON_OBJECT(
              'id', el.id,
    ...
    WHERE e.id IN (
      ...
      GROUP BY e2.message_id
    )
    ORDER BY (CASE WHEN e.folder_id IN (1, 5) THEN COALESCE(er_user.is_pinned, 0) ELSE COALESCE(e.is_pinned, 0) END) DESC, e.date_received DESC
    LIMIT $limit OFFSET $offset
  " 2>&1 | tail -1)
...
search_emails() {
  ...
  user_escaped=$(safe_sql_escape "$user")
  local folder
  # all 不在白名单时 safe_folder_param 会返回 inbox；此处允许 all 用于“全部”搜索
  if [[ "$folder_raw" == "all" ]] || [[ "$folder_raw" =~ ^[0-9]+$ ]]; then
    folder="$folder_raw"
  else
    folder=$(safe_folder_param "$folder_raw")
  fi
  ...
  if [[ "$folder" != "all" ]]; then
    if [[ "$folder" =~ ^[0-9]+$ ]]; then
      ...
      local folder_escaped
      folder_escaped=$(safe_sql_escape "$folder")
      local folder_query="SELECT id FROM email_folders WHERE name='$folder_escaped' LIMIT 1;"
      folder_id=$(mysql -h "$DB_HOST" -u "$DB_USER" -p"$(cat "$DB_PASS_FILE")" "$DB_NAME" -s -r -e "$folder_query" 2>/dev/null | tail -1)
      if [[ -z "$folder_id" || ! "$folder_id" =~ ^[0-9]+$ ]]; then
        folder_id=""
      fi
  ...
  if [[ "$folder" == "inbox" || "$folder_id" == "1" ]]; then
    ...
    # 已发送：只搜索当前用户发送的邮件（使用转义值防注入）
    if [ "$user" = "xm" ]; then
      where_clause="$where_clause AND e.from_addr = 'xm@localhost'"
    else
      where_clause="$where_clause AND (e.from_addr = '$user_escaped' OR e.from_addr LIKE '%@$user_escaped' OR e.from_addr LIKE '$user_escaped@%')"
    fi
  elif [[ "$folder" == "drafts" || "$folder_id" == "3" ]]; then
    # 草稿箱：只搜索当前用户的草稿
    if [ "$user" = "xm" ]; then
    ...
    if [ "$user" = "xm" ]; then
      ...
      where_clause="$where_clause AND (e.from_addr = '$user_escaped' OR e.from_addr LIKE '%@$user_escaped' OR e.from_addr LIKE '$user_escaped@%' OR EXISTS (
        SELECT 1 FROM email_recipients er 
        WHERE er.email_id = e.id 
        AND er.recipient_type IN ('to', 'cc') 
        AND (er.email_address = '$user_escaped' OR er.email_address LIKE '%@$user_escaped' OR er.email_address LIKE '$user_escaped@%')
    ...
    # 垃圾邮件：只显示收件人是当前用户的垃圾邮件（与收件箱逻辑完全一致）
    # 重要：垃圾邮件应该只统计收件人是当前用户的邮件，排除发件人是当前用户的邮件
    # 注意：user参数可能是完整邮箱地址（如xm@skills.com）或用户名（如xm）
    if [[ "$user" == *@* ]]; then
      # 如果传入的是完整邮箱地址，直接精确匹配（使用转义后的值防注入，与收件箱保持一致）
      where_clause="$where_clause AND EXISTS (
...
# 获取邮件详情（支持新表结构）
# 第3参数：skip_mark_read=1 时跳过标记已读（用于预加载，避免预加载导致未点击的邮件被标为已读）
get_email_detail() {
  local email_id="$1"
  local user="$2"
  local skip_mark_read="${3:-0}"
  ...
  # 查询附件列表（从email_attachments表，支持 file_path 大文件）
  local attachments_json=$(mysql -h "$DB_HOST" -u "$DB_USER" -p"$(cat "$DB_PASS_FILE")" "$DB_NAME" -s -r -e "
    SELECT JSON_ARRAYAGG(
      JSON_OBJECT(
        ...
        'type', content_type,
        'size', size_bytes,
        'content', COALESCE(content_base64, ''),
...
PYTHON_EOF
  ...
  if [[ "$skip_mark_read" != "1" && "$skip_mark_read" != "true" && -n "$user" && -n "$user_email_condition" && "$user_email_condition" != "1=0" ]]; then
    ...
    if [[ -n "$message_id" ]]; then
      ...
      while IFS= read -r eid; do
        if [[ -n "$eid" && "$eid" =~ ^[0-9]+$ ]]; then
          ...
          if [[ "$folder_id" == "1" ]] || [[ "$folder_id" == "4" ]] || [[ "$folder_id" == "5" ]] || [[ "$folder_id" -ge 6 ]]; then
            # 收件箱(1)、已删除(4)、垃圾邮件(5)、自定义文件夹(>=6)：更新email_recipients表中的is_read状态
            # 这些文件夹中的邮件可能来自收件箱，有email_recipients记录
          ...
          elif [[ "$folder_id" == "2" ]] || [[ "$folder_id" == "3" ]]; then
            # 已发送(2)、草稿箱(3)：更新emails.read_status字段（发件人级别的已读状态）
            local from_match=0
            if [[ "$user" == *@* ]]; then
              if [[ "$from_addr" == "$user" ]]; then from_match=1; fi
            elif [ "$user" = "xm" ]; then
              if [[ "$from_addr" == "xm@localhost" ]] || [[ "$from_addr" == *@xm ]] || [[ "$from_addr" == xm@* ]]; then from_match=1; fi
            else
              if [[ "$from_addr" == "$user" ]] || [[ "$from_addr" == *@$user ]] || [[ "$from_addr" == $user@* ]]; then from_match=1; fi
            fi
            if [[ $from_match -eq 1 ]]; then
              local update_result=$(mysql_connect "UPDATE emails SET read_status = 1 WHERE id = $eid;" 2>&1)
              if [[ $? -eq 0 ]]; then sent_updated_count=$((sent_updated_count + 1)); fi
            fi
          fi
          # 已删除(4)、垃圾(5)、自定义(>=6)：若邮件来自已发送（发件人=当前用户），补充更新emails.read_status
          if [[ "$folder_id" == "4" ]] || [[ "$folder_id" == "5" ]] || [[ "$folder_id" -ge 6 ]]; then
            local from_match=0
            if [[ "$user" == *@* ]]; then
              if [[ "$from_addr" == "$user" ]]; then from_match=1; fi
            elif [ "$user" = "xm" ]; then
            ...
            if [[ $from_match -eq 1 ]]; then
              mysql_connect "UPDATE emails SET read_status = 1 WHERE id = $eid;" 2>/dev/null
            fi
          fi
        fi
      done <<< "$email_ids"
      ...
      if [[ $updated_count -gt 0 ]] || [[ $sent_updated_count -gt 0 ]]; then
        log "邮件详情查询：已标记邮件 $email_id (base_message_id: $base_message_id) 为已读（用户: $user，收件人更新 $updated_count 条，发件人更新 $sent_updated_count 条）"
  else
        # 调试：检查为什么没有更新成功
...
# 彻底删除邮件（硬删除）
# 释放存储：删除前先删除磁盘上的附件文件（file_path），再删除数据库记录
# 与 move_email 一致：按 base_message_id 删除所有相关记录（收件箱+已发送等）
hard_delete_email() {
  local email_id="$1"
  local user="$2"
  ...
  # 1. 获取 base_message_id（与 move_email 一致）
  local message_id
  message_id=$(mysql -h "$DB_HOST" -u "$DB_USER" -p"$(cat "$DB_PASS_FILE")" "$DB_NAME" -s -r -e "
    SELECT message_id FROM emails WHERE id='$email_id' LIMIT 1;
  " 2>/dev/null | tail -1)
  
  if [[ -z "$message_id" ]]; then
    log "错误: 未找到邮件 id=$email_id，可能已被删除"
    return 1
  fi
  
  local base_message_id
  base_message_id=$(echo "$message_id" | cut -d'_' -f1)
  local escaped_base_message_id
  escaped_base_message_id=$(echo "$base_message_id" | sed "s/'/''/g" | sed 's/\\/\\\\/g')
  
  # 2. 删除磁盘上的附件文件（在删除 DB 记录前，否则 cascade 会丢失 file_path）
  local file_paths
  file_paths=$(mysql -h "$DB_HOST" -u "$DB_USER" -p"$(cat "$DB_PASS_FILE")" "$DB_NAME" -s -r -N -e "
    SELECT ea.file_path FROM email_attachments ea
    INNER JOIN emails e ON ea.email_id = e.id
    WHERE SUBSTRING_INDEX(e.message_id, '_', 1) = '$escaped_base_message_id'
    AND ea.file_path IS NOT NULL AND ea.file_path != '';
  " 2>/dev/null)
  ...
  # 3. 删除所有同 base_message_id 的邮件记录（与 move 逻辑一致，彻底释放存储）
  mysql_connect "DELETE FROM emails WHERE SUBSTRING_INDEX(message_id, '_', 1) = '$escaped_base_message_id';"
  
  log "邮件彻底删除成功: id=$email_id, base_message_id=$base_message_id（已释放存储）"
}
...
# 置顶/取消置顶邮件（仅更新当前文件夹内同封邮件，不跨文件夹）
# 收件箱置顶不影响已发送箱，反之亦然：同一封邮件在不同文件夹内可独立置顶
# 收件箱/垃圾邮件：多用户共享同一封邮件，置顶按收件人独立（xm 置顶不影响 test01）
pin_email() {
  local email_id="$1"
  local pinned="${2:-1}"  # 1=置顶，0=取消置顶
  local user="$3"
  ...
  log "置顶邮件: id=$email_id, pinned=$pinned, user=$user"
  
  local user_escaped
  user_escaped=$(safe_sql_escape "$user")
  
  local row=$(mysql -h "$DB_HOST" -u "$DB_USER" -p"$(cat "$DB_PASS_FILE")" "$DB_NAME" -s -r -e "
    SELECT message_id, folder_id FROM emails WHERE id='$email_id' LIMIT 1;
  " 2>/dev/null | tail -1)
  ...
  local message_id=$(echo "$row" | cut -f1)
  local folder_id=$(echo "$row" | cut -f2)
  
  local base_message_id=$(echo "$message_id" | cut -d'_' -f1)
  local escaped_base_message_id=$(echo "$base_message_id" | sed "s/'/''/g" | sed 's/\\/\\\\/g')
  
  if [[ "$folder_id" == "1" || "$folder_id" == "5" ]]; then
    # 收件箱(1)或垃圾邮件(5)：多用户共享同一封邮件，置顶按收件人独立（更新 email_recipients）
    local er_condition=""
    if [[ "$user" == *@* ]]; then
      er_condition="(er.email_address = '$user_escaped' OR er.email_address LIKE '%<$user_escaped>')"
    elif [ "$user" = "xm" ]; then
      er_condition="(er.email_address = 'xm@localhost' OR er.email_address LIKE '%@xm' OR er.email_address LIKE 'xm@%')"
    else
      er_condition="(er.email_address = '$user_escaped' OR er.email_address LIKE '%@$user_escaped' OR er.email_address LIKE '$user_escaped@%')"
    fi
    mysql_connect "UPDATE email_recipients er
      JOIN emails e ON er.email_id = e.id
      SET er.is_pinned = $pinned
      WHERE SUBSTRING_INDEX(e.message_id, '_', 1) = '$escaped_base_message_id'
        AND e.folder_id = $folder_id
        AND er.recipient_type IN ('to', 'cc')
        AND ($er_condition);"
  else
    # 已发送(2)、草稿箱(3)、已删除(4)、自定义文件夹：按邮件行更新 emails.is_pinned
    mysql_connect "UPDATE emails SET is_pinned = $pinned, updated_at = CURRENT_TIMESTAMP WHERE SUBSTRING_INDEX(message_id, '_', 1) = '$escaped_base_message_id' AND folder_id = $folder_id;"
  fi
  
  if [[ $? -ne 0 ]]; then
    log "错误: 置顶操作失败 id=$email_id"
    return 1
  fi
  
  log "邮件置顶成功: id=$email_id, pinned=$pinned, folder_id=$folder_id（folder 1/5 按收件人独立置顶）"
}
...
get_mail_stats() {
  ...
  # 注意：user参数可能是完整邮箱地址（如xm@skills.com）或用户名（如xm）
  # 兼容 email_recipients 中可能存在的 "Name <email>" 格式（历史数据）
  local inbox_stats=""
  if [[ -f "$DB_PASS_FILE" ]]; then
    ...
    if [[ -n "$user" ]]; then
      if [[ "$user" == *@* ]]; then
        # 完整邮箱：精确匹配或 "Name <email>" 格式
        user_email_condition="(er.email_address = '$user_escaped' OR er.email_address LIKE '%<$user_escaped>')"
      elif [ "$user" = "xm" ]; then
        # 特殊处理xm用户，支持多种邮箱格式
  ...
  # 注意：user参数可能是完整邮箱地址（如xm@skills.com）或用户名（如xm）
  # 兼容 from_addr 中可能存在的 "Name <email>" 格式（历史数据）
  local sent_stats=""
  if [[ -f "$DB_PASS_FILE" ]]; then
    # 构建发件人邮箱地址匹配条件
    ...
    if [[ -n "$user" ]]; then
      if [[ "$user" == *@* ]]; then
        # 完整邮箱：精确匹配或 "Name <email>" 格式
        from_addr_condition="(e.from_addr = '$user_escaped' OR e.from_addr LIKE '%<$user_escaped>')"
      elif [ "$user" = "xm" ]; then
        # 特殊处理xm用户，支持多种邮箱格式
    ...
    if [[ -z "$from_addr_condition" ]]; then
      ...
      sent_stats=$(mysql -h "$DB_HOST" -u "$DB_USER" -p"$(cat "$DB_PASS_FILE")" "$DB_NAME" -s -r -e "
        SELECT
          ...
          0 as unread,
          0 as read_count,
          COALESCE(SUM(e.size_bytes), 0) as total_size
          FROM emails e
  ...
  # 重要：与 list 查询保持一致，使用 draft_user_emails IN 列表（来自 mail_users + localhost）
  local drafts_stats=""
  if [[ -f "$DB_PASS_FILE" ]]; then
    local draft_user_emails=""
    if [[ "$user" == *@* ]]; then
      ...
      if [ "$user" = "xm" ]; then
        draft_user_emails="'xm@localhost'"
        local db_emails=$(mysql -h "$DB_HOST" -u "$DB_USER" -p"$(cat "$DB_PASS_FILE")" "$DB_NAME" -s -r -e "SELECT email FROM mail_users WHERE username='xm';" 2>/dev/null | grep -v "^email$" | grep "@" | sed "s/^/'/" | sed "s/$/'/" | tr '\n' ',' | sed 's/,$//')
        if [[ -n "$db_emails" ]]; then
          draft_user_emails="$draft_user_emails, $db_emails"
        fi
      else
    ...
    drafts_stats=$(mysql -h "$DB_HOST" -u "$DB_USER" -p"$(cat "$DB_PASS_FILE")" "$DB_NAME" -s -r -e "
        ...
        FROM (
          SELECT
            e.message_id,
            e.read_status as min_read_status,
            e.size_bytes
          FROM emails e
          WHERE e.folder_id=3 AND e.is_deleted=0 AND e.from_addr IN ($draft_user_emails)
          GROUP BY e.message_id
        ) AS subquery;
      " 2>/dev/null)
  fi
  ...
  local trash_stats=""
  if [[ -f "$DB_PASS_FILE" ]]; then
    local trash_er_condition trash_from_condition
    if [ "$user" = "xm" ]; then
      trash_er_condition="(er.email_address = 'xm@localhost' OR er.email_address LIKE '%@xm' OR er.email_address LIKE 'xm@%')"
      trash_from_condition="e.from_addr = 'xm@localhost'"
    elif [[ "$user" == *@* ]]; then
      trash_er_condition="(er.email_address = '$user_escaped' OR er.email_address LIKE '%<$user_escaped>')"
      trash_from_condition="(e.from_addr = '$user_escaped' OR e.from_addr LIKE '%<$user_escaped>')"
    else
      trash_er_condition="(er.email_address = '$user_escaped' OR er.email_address LIKE '%@$user_escaped' OR er.email_address LIKE '$user_escaped@%')"
      trash_from_condition="(e.from_addr = '$user_escaped' OR e.from_addr LIKE '%@$user_escaped' OR e.from_addr LIKE '$user_escaped@%')"
    fi
        ...
        FROM (
          SELECT
            ...
            COALESCE((
              ...
              WHERE er.email_id = e.id
                AND er.recipient_type IN ('to', 'cc')
                AND ($trash_er_condition)
            ), 0) as user_read_status
          FROM emails e
          WHERE e.folder_id=4 AND e.is_deleted=0
          AND ($trash_from_condition OR EXISTS (
            SELECT 1 FROM email_recipients er 
            WHERE er.email_id = e.id 
            AND er.recipient_type IN ('to', 'cc') 
            AND ($trash_er_condition)
  ...
  local spam_stats=""
  if [[ -f "$DB_PASS_FILE" ]]; then
    # 构建用户邮箱地址匹配条件（与inbox保持一致，兼容 "Name <email>" 格式）
    local spam_er_condition=""
    if [[ -n "$user" ]]; then
      if [ "$user" = "xm" ]; then
        spam_er_condition="(er.email_address = 'xm@localhost' OR er.email_address LIKE '%@xm' OR er.email_address LIKE 'xm@%')"
      elif [[ "$user" == *@* ]]; then
        spam_er_condition="(er.email_address = '$user_escaped' OR er.email_address LIKE '%<$user_escaped>')"
      else
        spam_er_condition="(er.email_address = '$user_escaped' OR er.email_address LIKE '%@$user_escaped' OR er.email_address LIKE '$user_escaped@%')"
      fi
    fi
    
    if [[ -z "$spam_er_condition" ]]; then
      spam_er_condition="1=0"  # 如果为空，使用永远为假的条件
    fi
  ...
  # 去重存储统计：同一封邮件在收件箱+已发送等多处只计一次（单向收到计算，符合用户预期）
  local dedup_size=0
  if [[ -f "$DB_PASS_FILE" ]]; then
    local dedup_where=""
    if [ "$user" = "xm" ]; then
      dedup_where="(
        (e.folder_id=1 AND EXISTS (SELECT 1 FROM email_recipients er WHERE er.email_id=e.id AND er.recipient_type IN ('to','cc') AND (er.email_address='xm@localhost' OR er.email_address LIKE '%@xm' OR er.email_address LIKE 'xm@%')))
        OR (e.folder_id=2 AND (e.from_addr='xm@localhost' OR e.from_addr LIKE '%@xm' OR e.from_addr LIKE 'xm@%'))
        OR (e.folder_id=3 AND (e.from_addr='xm@localhost' OR e.from_addr LIKE '%@xm' OR e.from_addr LIKE 'xm@%'))
        OR (e.folder_id=4 AND ((e.from_addr='xm@localhost' OR e.from_addr LIKE '%@xm' OR e.from_addr LIKE 'xm@%') OR EXISTS (SELECT 1 FROM email_recipients er WHERE er.email_id=e.id AND er.recipient_type IN ('to','cc') AND (er.email_address='xm@localhost' OR er.email_address LIKE '%@xm' OR er.email_address LIKE 'xm@%'))))
        OR (e.folder_id=5 AND EXISTS (SELECT 1 FROM email_recipients er WHERE er.email_id=e.id AND er.recipient_type IN ('to','cc') AND (er.email_address='xm@localhost' OR er.email_address LIKE '%@xm' OR er.email_address LIKE 'xm@%')))
      )"
    elif [[ "$user" == *@* ]]; then
      dedup_where="(
        (e.folder_id=1 AND EXISTS (SELECT 1 FROM email_recipients er WHERE er.email_id=e.id AND er.recipient_type IN ('to','cc') AND (er.email_address = '$user_escaped' OR er.email_address LIKE '%<$user_escaped>')))
        OR (e.folder_id=2 AND (e.from_addr = '$user_escaped' OR e.from_addr LIKE '%<$user_escaped>'))
        OR (e.folder_id=3 AND (e.from_addr = '$user_escaped' OR e.from_addr LIKE '%<$user_escaped>'))
        OR (e.folder_id=4 AND ((e.from_addr = '$user_escaped' OR e.from_addr LIKE '%<$user_escaped>') OR EXISTS (SELECT 1 FROM email_recipients er WHERE er.email_id=e.id AND er.recipient_type IN ('to','cc') AND (er.email_address = '$user_escaped' OR er.email_address LIKE '%<$user_escaped>'))))
        OR (e.folder_id=5 AND EXISTS (SELECT 1 FROM email_recipients er WHERE er.email_id=e.id AND er.recipient_type IN ('to','cc') AND (er.email_address = '$user_escaped' OR er.email_address LIKE '%<$user_escaped>')))
      )"
    else
      dedup_where="(
        (e.folder_id=1 AND EXISTS (SELECT 1 FROM email_recipients er WHERE er.email_id=e.id AND er.recipient_type IN ('to','cc') AND (er.email_address = '$user_escaped' OR er.email_address LIKE '%@$user_escaped' OR er.email_address LIKE '$user_escaped@%')))
        OR (e.folder_id=2 AND (e.from_addr = '$user_escaped' OR e.from_addr LIKE '%@$user_escaped' OR e.from_addr LIKE '$user_escaped@%'))
        OR (e.folder_id=3 AND (e.from_addr = '$user_escaped' OR e.from_addr LIKE '%@$user_escaped' OR e.from_addr LIKE '$user_escaped@%'))
        OR (e.folder_id=4 AND ((e.from_addr = '$user_escaped' OR e.from_addr LIKE '%@$user_escaped' OR e.from_addr LIKE '$user_escaped@%') OR EXISTS (SELECT 1 FROM email_recipients er WHERE er.email_id=e.id AND er.recipient_type IN ('to','cc') AND (er.email_address = '$user_escaped' OR er.email_address LIKE '%@$user_escaped' OR er.email_address LIKE '$user_escaped@%'))))
        OR (e.folder_id=5 AND EXISTS (SELECT 1 FROM email_recipients er WHERE er.email_id=e.id AND er.recipient_type IN ('to','cc') AND (er.email_address = '$user_escaped' OR er.email_address LIKE '%@$user_escaped' OR er.email_address LIKE '$user_escaped@%')))
      )"
    fi
    dedup_size=$(mysql -h "$DB_HOST" -u "$DB_USER" -p"$(cat "$DB_PASS_FILE")" "$DB_NAME" -s -r -e "
      SELECT COALESCE(SUM(size_bytes), 0) FROM (
        SELECT SUBSTRING_INDEX(e.message_id, '_', 1) AS base_msg_id, MAX(e.size_bytes) AS size_bytes
        FROM emails e
        WHERE e.is_deleted=0 AND e.folder_id IN (1,2,3,4,5) AND ($dedup_where)
        GROUP BY base_msg_id
      ) dedup;
    " 2>/dev/null | tail -1 | sed 's/NULL/0/g')
    dedup_size=${dedup_size:-0}
  fi
...
main() {
  ...
  case "$action" in
    "init")
      ...
      ;;
    "store")
      store_email "$2" "$3" "$4" "$5" "$6" "${7:-}" "${8:-inbox}" "${9:-0}" "${10:-}" "${11:-}" "${12:-}" "${13:-0}" "${14:-0}" "${15:-0}"
      ;;
    "list")
...
mark_email_read() {
  ...
  if [[ -n "$user_email_condition" ]]; then
    ...
    while IFS= read -r eid; do
      if [[ -n "$eid" && "$eid" =~ ^[0-9]+$ ]]; then
        ...
        if [[ "$folder_id" == "1" ]] || [[ "$folder_id" == "4" ]] || [[ "$folder_id" == "5" ]] || [[ "$folder_id" -ge 6 ]]; then
          ...
          # 已发送(2)、草稿箱(3)：更新emails.read_status
          local from_match=0
          if [[ "$user" == *@* ]]; then
            if [[ "$from_addr" == "$user" ]]; then from_match=1; fi
          elif [ "$user" = "xm" ]; then
        ...
        # 已删除(4)、垃圾(5)、自定义(>=6)：发件人=当前用户时补充更新emails.read_status
        if [[ "$folder_id" == "4" ]] || [[ "$folder_id" == "5" ]] || [[ "$folder_id" -ge 6 ]]; then
          local from_match=0
          if [[ "$user" == *@* ]]; then
            if [[ "$from_addr" == "$user" ]]; then from_match=1; fi
    ...
    if [[ $updated_count -gt 0 ]] || [[ $sent_updated_count -gt 0 ]]; then
      log "邮件 $email_id 已标记为已读（用户: $user，收件人 $updated_count 条，发件人 $sent_updated_count 条）"
      echo "邮件已标记为已读"
    else
      # 调试：检查为什么没有更新成功
      local debug_info=$(mysql -h "$DB_HOST" -u "$DB_USER" -p"$(cat "$DB_PASS_FILE")" "$DB_NAME" -s -r -e "
...
# 规范化历史数据：将 email_recipients 中 "Name <email>" 格式转为纯邮箱（一次性修复）
normalize_recipients() {
  log "开始规范化 email_recipients 中的邮箱地址格式..."
  if [[ ! -f "$DB_PASS_FILE" ]]; then
    log "数据库密码文件不存在"
    echo "数据库密码文件不存在"
    return 1
  fi
  mysql -h "$DB_HOST" -u "$DB_USER" -p"$(cat "$DB_PASS_FILE")" "$DB_NAME" -e "
    UPDATE email_recipients 
    SET email_address = TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(email_address, '>', 1), '<', -1))
