...
# 工作职责: 邮件服务日志集成脚本 - 配置 Postfix/Dovecot 日志收集与敏感信息掩码
# 系统组件: XM邮件管理系统 - 服务日志集成模块
# ============================================================================
# 用法说明:
#   mail_service_logger.sh <action>
#   mail_service_logger.sh install     - 安装日志记录（rsyslog、logrotate、辅助脚本）
#   mail_service_logger.sh test        - 测试日志记录功能
#   mail_service_logger.sh uninstall   - 卸载日志记录（移除脚本与配置）
#   mail_service_logger.sh help        - 显示帮助信息
#
# 功能描述:
#   - install：配置 Postfix/Dovecot rsyslog 与 logrotate；部署 content_scanner.sh、forward_logger.sh
#   - 敏感信息处理：掩码（masking）邮箱等字段，非加密
#   - test：验证日志记录是否正常
#   - uninstall：删除 /usr/local/bin 下脚本及 rsyslog/logrotate 配置
#
# 依赖关系:
