...
# ============================================================================
# 脚本名称: cert_setup.sh
# 工作职责: SSL/TLS 证书管理脚本 - 基于 OpenSSL 生成自签名证书或私有 CA 证书
#           配置 Apache SSL 虚拟主机与 HTTP 跳转 HTTPS
# 系统组件: XM邮件管理系统 - 证书管理模块
# ============================================================================
# 用法说明:
#   cert_setup.sh <action> [参数...]
#   cert_setup.sh install <domain> [country] [state] [city] [organization] [unit] [common_name] [email] [validity] [san] [ca_country] [ca_state] [ca_city] [ca_organization] [ca_unit] [ca_common_name] [ca_email] [ca_validity]
#     - 完整流程：创建私有 CA、生成服务器证书、配置 Apache SSL、更新系统信任、DNS 建议
#   cert_setup.sh enable-ssl <域名>            - 配置 Apache SSL 虚拟主机（使用 /etc/pki/tls 下已有证书）
#   cert_setup.sh enable-http-redirect [域名]  - 配置 HTTP 自动跳转 HTTPS
#
# 功能描述:
#   - install：OpenSSL 自签名/私有 CA 证书申请（非 certbot/Let's Encrypt）
#   - enable-ssl：使用已有证书文件配置 Apache SSL 虚拟主机（不自动配置 HTTP 跳转）
#   - enable-http-redirect：配置 HTTP 自动跳转 HTTPS（可选域名）
#
# 证书存储:
#   - install 生成：/etc/ssl/<域名>.crt、/etc/ssl/<域名>.key、/etc/pki/CA/cacert.pem
#   - enable-ssl 使用：/etc/pki/tls/<证书名>.crt、/etc/pki/tls/<证书名>.key
#
# 端口配置:
#   - Apache HTTP/HTTPS 端口：从 config/port-config.json 读取（默认 80/443）
#
# 依赖关系:
#   - OpenSSL（证书生成，仅此方式，不含 certbot）
#   - Apache（Web 服务器）
#   - jq（port-config.json 解析）
#
# 注意事项:
#   - 需要 root 权限执行证书操作
#   - 证书文件需要安全权限（600）
#   - SSL 配置时仅配置 SSL 虚拟主机，不自动创建 HTTP 跳转
#   - HTTP 跳转 HTTPS 需通过 enable-http-redirect 单独配置
# ============================================================================
...
if [[ -z "$BASE_DIR" ]]; then
  echo "错误: 无法确定项目根目录，请确保使用绝对路径调用脚本" >&2
  exit 1
fi
...
configure_apache_ssl() {
    ...
    # 备份现有配置到 _old，避免被 IncludeOptional conf.d/*.conf 加载
    if [[ -f "$mailmgmt_conf" ]]; then
        install -d -m 0755 /etc/httpd/conf.d/_old
        cp "$mailmgmt_conf" "/etc/httpd/conf.d/_old/mailmgmt.conf.backup.$(date +%Y%m%d_%H%M%S)"
        log_info "已备份现有邮件管理配置到 _old"
    fi
...
EOF
    ...
    # 启用SSL模块和Rewrite模块（检查 httpd.conf、conf.d、conf.modules.d，避免 AH01574 重复加载警告）
    if ! grep -q "LoadModule ssl_module" /etc/httpd/conf/httpd.conf 2>/dev/null && \
       ! grep -rq "LoadModule ssl_module" /etc/httpd/conf.d/ /etc/httpd/conf.modules.d/ 2>/dev/null; then
        echo "LoadModule ssl_module modules/mod_ssl.so" >> /etc/httpd/conf/httpd.conf
        log_info "已启用SSL模块"
    else
        log_info "SSL模块已在其他配置中加载"
    fi
    if ! grep -q "LoadModule rewrite_module" /etc/httpd/conf/httpd.conf 2>/dev/null && \
       ! grep -rq "LoadModule rewrite_module" /etc/httpd/conf.d/ /etc/httpd/conf.modules.d/ 2>/dev/null; then
        echo "LoadModule rewrite_module modules/mod_rewrite.so" >> /etc/httpd/conf/httpd.conf
        log_info "已启用Rewrite模块"
    else
        log_info "Rewrite模块已在其他配置中加载"
    fi
...
main() {
    ...
    validity="${validity:-1825}"
        ...
        "enable-http-redirect")
            ...
            # 确保mod_rewrite模块已启用（检查 conf.modules.d 避免 AH01574 重复加载警告）
                    ...
                    if ! grep -q "SSLEngine" "$old_conf_file" 2>/dev/null && grep -q "<VirtualHost.*:80>" "$old_conf_file" 2>/dev/null; then
                        ...
                        if grep -q "ServerName.*${domain_item}" "$old_conf_file" 2>/dev/null || 
                           ...
                           grep -q "ServerAlias.*www\.${domain_item}" "$old_conf_file" 2>/dev/null; then
                            log_warning "发现旧格式HTTP配置文件: $old_conf_file，将备份到 _old 后删除以避免冲突"
                            install -d -m 0755 /etc/httpd/conf.d/_old
                            cp "$old_conf_file" "/etc/httpd/conf.d/_old/$(basename "$old_conf_file").backup.$(date +%Y%m%d_%H%M%S)" 2>/dev/null || true
                            rm -f "$old_conf_file"
                        ...
                        if ! grep -q "SSLEngine" "$base_conf_file" 2>/dev/null && grep -q "<VirtualHost.*:80>" "$base_conf_file" 2>/dev/null; then
                            if grep -q "ServerAlias.*www\.${base_domain}" "$base_conf_file" 2>/dev/null || 
                               grep -q "ServerName.*www\.${base_domain}" "$base_conf_file" 2>/dev/null; then
                                log_warning "发现旧格式HTTP配置文件（包含www别名）: $base_conf_file，将备份到 _old 后删除以避免冲突"
                                install -d -m 0755 /etc/httpd/conf.d/_old
                                cp "$base_conf_file" "/etc/httpd/conf.d/_old/$(basename "$base_conf_file").backup.$(date +%Y%m%d_%H%M%S)" 2>/dev/null || true
                                rm -f "$base_conf_file"
                    ...
                    if [[ -f "$www_conf_file" ]]; then
                        ...
                        if ! grep -q "SSLEngine" "$www_conf_file" 2>/dev/null && grep -q "<VirtualHost.*:80>" "$www_conf_file" 2>/dev/null; then
                            if grep -q "ServerAlias.*www\.${domain_item}" "$www_conf_file" 2>/dev/null; then
                                log_warning "发现旧格式HTTP配置文件（包含www别名）: $www_conf_file，将备份到 _old 后删除以避免冲突"
                                install -d -m 0755 /etc/httpd/conf.d/_old
                                cp "$www_conf_file" "/etc/httpd/conf.d/_old/$(basename "$www_conf_file").backup.$(date +%Y%m%d_%H%M%S)" 2>/dev/null || true
                                rm -f "$www_conf_file"
