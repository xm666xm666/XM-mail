...
#   backup.sh setup-backup-cron <间隔> <数据库> <配置> <邮件目录> <保留天数> [自定义时间] [时] [分] [秒] - 配置备份 cron 参数
...
#
# 备份目录:
#   - BACKUP_DIR=/var/backups/mail（可通过环境变量覆盖）
#
# 注意事项:
#   - 需要 root 权限执行
#   - 备份路径与保留策略由 BACKUP_DIR 或 setup-backup-cron 参数决定
# ============================================================================
...
backup_config() {
  ...
  [[ -d "$BASE_DIR/config" ]] && backup_items+=("$BASE_DIR/config")
...
# 备份间隔: ${interval}天
# 保留时间: ${retention}天
# 项目根目录（setup 时确定，若移动项目需重新执行 setup-backup-cron）
BASE_DIR="${BASE_DIR}"
...
if [[ "${config}" == "true" ]]; then
  ...
  [[ -f "/etc/systemd/system/mail-ops-dispatcher.service" ]] && backup_items+=("/etc/systemd/system/mail-ops-dispatcher.service")
  [[ -d "/var/log/mail-ops" ]] && backup_items+=("/var/log/mail-ops")
  [[ -n "\$BASE_DIR" && -d "\$BASE_DIR/config" ]] && backup_items+=("\$BASE_DIR/config")
