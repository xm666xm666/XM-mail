...
function checkSpamContent(subject, content, from, to, spamConfig) {
  ...
  // 检查是否为转发邮件（Subject以"Fwd:"或"转发:"开头）
  const isForwardEmail = subject && (subject.trim().toLowerCase().startsWith('fwd:') || subject.trim().startsWith('转发:'))
  ...
  // 答复邮件或转发邮件跳过内容规则检查（行数、大写比例、感叹号、特殊字符等）
  // 只检查关键词和域名黑名单
  if (isReplyEmail || isForwardEmail) {
    console.log(`[垃圾邮件检测] 检测到${isReplyEmail ? '答复' : '转发'}邮件，跳过内容规则检查: subject=${subject}`)
    return {
...
/** 只允许邮件文件夹白名单，防止非法 folder 参数
 *  系统文件夹：inbox, sent, drafts, trash, spam, all
 *  自定义文件夹：字母、数字、下划线、连字符（与 add-folder 命名规则一致）
 */
function safeMailFolder (folder) {
  const systemFolders = ['inbox', 'sent', 'drafts', 'trash', 'all', 'spam']
  const f = String(folder || 'inbox').trim()
  if (systemFolders.includes(f.toLowerCase())) return f.toLowerCase()
  // 自定义文件夹：仅允许字母、数字、下划线、连字符
  if (/^[a-zA-Z0-9_-]+$/.test(f) && f.length <= 100) return f
  return 'inbox'
}

/** 将参数限制为安全整数（用于 limit/offset 等） */
function safeInteger (val, defaultVal, max = 100000) {
...
/** 转义 shell 参数（单引号包裹，内部单引号用 '\'' 转义），防止命令注入 */
...
// 简单 CSRF 保护中间件：
// - 仅对需要登录的「修改类」接口启用（在具体路由上挂载）
// - 跳过 GET / HEAD / OPTIONS
// - 只要求请求头中存在 X-CSRF-Token，不做值校验（前后端在同一应用内生成）
function csrfProtection (req, res, next) {
  ...
  if (!token) {
    return res.status(403).json({
      ...
      message: '安全校验失败：请求缺少 X-CSRF-Token 头部'
    })
  }
  ...
  child.on('close', (code) => {
    ...
    if (scriptName === 'app_user.sh' && args[0] === 'query-users') {
      try {
        ...
        // 查找JSON输出行（app_user.sh query-users 输出格式为 [{"id":1,"username":"xm",...}]）
        for (const line of lines) {
          const trimmedLine = line.trim()
          // 排除日志行（如 [2026-02-26T09:38:09.390Z] [DEBUG] Args: [...]），只解析数组对象格式
...
app.post('/api/ops', auth, csrfProtection, (req, res) => {
  ...
  switch (action) {
    case 'check':
      scriptName = 'run_diagnosis.sh'; args = []; break
    case 'install':
    ...
    case 'setup-logs':
      scriptName = 'mail_service_logger.sh'; args = ['install']; break
    case 'full-backup':
...
// ==================== SSL 证书辅助函数 ====================
const CERT_STORE_DIR = '/etc/pki/tls'
const CERT_BACKUP_DIR = '/etc/pki/tls/backup'
const CERT_NAME_REGEX = /^[a-z0-9_-]+(\.[a-z0-9_-]+)*$/i

function isValidCertName(name) {
  return typeof name === 'string' && CERT_NAME_REGEX.test(name.trim())
}

function getDomainCertMap() {
  const domainCertConfigFile = path.join(ROOT_DIR, 'config', 'ssl-domain-cert.json')
  if (!fs.existsSync(domainCertConfigFile)) return {}
  try {
    return JSON.parse(fs.readFileSync(domainCertConfigFile, 'utf8')) || {}
  } catch (e) {
    console.warn('[证书] 读取 ssl-domain-cert.json 失败:', e.message)
    return {}
  }
}

function saveDomainCertMap(domainCertMap) {
  const domainCertConfigFile = path.join(ROOT_DIR, 'config', 'ssl-domain-cert.json')
...
function getDomainsUsingCert(certName) {
  const domainCertMap = getDomainCertMap()
  return Object.keys(domainCertMap).filter(domain => domainCertMap[domain] === certName)
}

function isApacheSslConfiguredForDomain(domain) {
  const sslConfFile = `/etc/httpd/conf.d/${domain}_ssl.conf`
  try {
    if (fs.existsSync(sslConfFile)) {
      const content = fs.readFileSync(sslConfFile, 'utf8')
      return content.includes('SSLEngine on')
    }
  } catch (_) {}
  try {
    const content = execSync(`sudo cat "${sslConfFile}" 2>/dev/null`, { timeout: 3000, encoding: 'utf8' })
    return content.includes('SSLEngine on')
  } catch (_) {
    return false
  }
...
function getActiveSslDomainsForCert(certName) {
  return getDomainsUsingCert(certName).filter(domain => isApacheSslConfiguredForDomain(domain))
}

function backupCertFiles(certName) {
  const files = [
    `${CERT_STORE_DIR}/${certName}.crt`,
    `${CERT_STORE_DIR}/${certName}.key`,
    `${CERT_STORE_DIR}/${certName}.chain.crt`
  ].filter(f => {
  ...
  const backupDir = path.join(CERT_BACKUP_DIR, `${certName}_${stamp}`)
  const backedUp = []

  try {
    execSync(`sudo mkdir -p "${backupDir}"`, { timeout: 5000 })
  } catch (_) {
    try { fs.mkdirSync(backupDir, { recursive: true }) } catch (e) {
      console.warn('[证书] 创建备份目录失败:', e.message)
      return []
    }
  }
  ...
  for (const filePath of files) {
    ...
    try {
      execSync(`sudo cp "${filePath}" "${dest}"`, { timeout: 5000 })
      backedUp.push(dest)
    } catch (_) {
      try {
        ...
        console.warn(`[证书] 备份失败 ${filePath}:`, e.message)
      }
    }
  }
  return backedUp
...
function installCertFile(srcPath, destPath, mode) {
  try {
    execSync(`sudo cp "${srcPath}" "${destPath}"`, { timeout: 5000 })
    execSync(`sudo chmod ${mode} "${destPath}"`, { timeout: 3000 })
  } catch (_) {
    fs.copyFileSync(srcPath, destPath)
    fs.chmodSync(destPath, mode === '600' ? 0o600 : 0o644)
  }
}

function reloadApacheGraceful() {
  try {
    execSync('sudo httpd -t', { timeout: 8000, encoding: 'utf8' })
  } catch (e1) {
    try {
      execSync('httpd -t', { timeout: 8000, encoding: 'utf8' })
    } catch (e2) {
      throw new Error('Apache 配置语法检查失败: ' + (e2.message || e1.message))
    }
  }
  try {
    execSync('sudo systemctl reload httpd', { timeout: 30000, encoding: 'utf8' })
  } catch (_) {
    try {
      execSync('systemctl reload httpd', { timeout: 30000, encoding: 'utf8' })
    } catch (e2) {
      try {
        execSync('sudo apachectl graceful', { timeout: 30000, encoding: 'utf8' })
      } catch (e3) {
        throw new Error('Apache 优雅重载失败: ' + (e3.message || e2.message))
      }
    }
  }
}

function removeApacheSslForDomain(domain) {
  const sslConfFile = `/etc/httpd/conf.d/${domain}_ssl.conf`
  ...
  try {
    ...
    deleted.push(sslConfFile, httpConfFile)
  } catch (_) {
    try {
      execSync(rmCmd, { timeout: 10000, encoding: 'utf8' })
      deleted.push(sslConfFile, httpConfFile)
    } catch (e2) {
      for (const f of [sslConfFile, httpConfFile]) {
        try {
          if (fs.existsSync(f)) {
            fs.unlinkSync(f)
            deleted.push(f)
...
function unlinkCertFromDomains(certName, domains) {
  const domainCertMap = getDomainCertMap()
  let changed = false
  ...
  if (changed) saveDomainCertMap(domainCertMap)
}

function validatePemCertAndKey(certContent, keyContent) {
...
app.post('/api/cert/domains', auth, csrfProtection, (req, res) => {
  try {
...
app.delete('/api/cert/domains/:domain', auth, csrfProtection, (req, res) => {
  try {
    ...
    // 删除SSL/HTTP配置文件（/etc/httpd/conf.d/ 需 root 权限，使用 sudo）
    const deleteApacheConf = (filePath) => {
      if (!fs.existsSync(filePath)) return
      try {
        execSync(`sudo rm -f "${filePath}"`, { timeout: 5000, encoding: 'utf8' })
        deletedFiles.push(filePath)
        console.log(`[删除域名] 已删除配置文件: ${filePath}`)
      } catch (e) {
        console.error(`[删除域名] 删除配置文件失败: ${filePath}`, e)
      }
    }
    deleteApacheConf(sslConfFile)
    deleteApacheConf(httpConfFile)
    
    // 删除旧格式的配置文件（如果存在且包含该域名，/etc/httpd/conf.d/ 需 root 权限）
    if (fs.existsSync(oldConfFile)) {
      try {
        ...
        if (content.includes(`ServerName ${domain}`) || content.includes(`ServerAlias ${domain}`) || 
          ...
          try {
            execSync(`sudo rm -f "${oldConfFile}"`, { timeout: 5000, encoding: 'utf8' })
            deletedFiles.push(oldConfFile)
            console.log(`[删除域名] 已删除旧格式配置文件: ${oldConfFile}`)
          } catch (rmErr) {
            console.error(`[删除域名] 检查/删除旧格式配置文件失败: ${oldConfFile}`, rmErr)
          }
        }
      } catch (e) {
...
// 更新域名证书关联API
app.put('/api/cert/domains/:domain/cert', auth, csrfProtection, (req, res) => {
  try {
    const domain = req.params.domain
...
// 启用域名的Apache SSL配置
app.post('/api/cert/domains/:domain/enable-ssl', auth, csrfProtection, (req, res) => {
  try {
    const domain = req.params.domain
    ...
    try {
      ...
      // 延迟重启调度层（先发送响应，避免ERR_CONNECTION_REFUSED；调度层需运行才能响应API）
      try {
        require('child_process').spawn('sh', ['-c', 'sleep 3 && systemctl restart mail-ops-dispatcher'], { detached: true, stdio: 'ignore' }).unref()
        console.log('[启用SSL] 已调度3秒后重启调度层服务')
      } catch (e) {
        console.warn('[启用SSL] 调度层重启调度失败:', e.message)
      }
      res.json({
        success: true,
        message: 'Apache SSL配置成功，服务已重启。若HTTPS页面出现 api/version、api/captcha 等 ERR_CONNECTION_REFUSED，请执行 systemctl restart mail-ops-dispatcher 并刷新页面。'
      })
    } catch (sudoError) {
      ...
      try {
        ...
        // 延迟重启调度层（先发送响应，避免ERR_CONNECTION_REFUSED）
        try {
          require('child_process').spawn('sh', ['-c', 'sleep 3 && systemctl restart mail-ops-dispatcher'], { detached: true, stdio: 'ignore' }).unref()
          console.log('[启用SSL] 已调度3秒后重启调度层服务')
        } catch (e) {
...
app.post('/api/cert/domains/:domain/disable-ssl', auth, csrfProtection, (req, res) => {
  try {
    const domain = req.params.domain
...
// 删除证书
// - 未关联域名：直接删除文件
// - 仅配置关联、未启用 Apache SSL：解除关联后删除（不影响站点）
// - 已启用 Apache SSL：默认拒绝；force=true 时先禁用相关域名 HTTPS，再删除
// - 续期请用同名上传覆盖，无需删除，可保持域名 HTTPS 继续运行
app.delete('/api/cert/:certName', auth, csrfProtection, (req, res) => {
  try {
    const certName = decodeURIComponent(req.params.certName || '').trim()
    ...
    const usedByDomains = getDomainsUsingCert(certName)
    const activeSslDomains = getActiveSslDomainsForCert(certName)

    if (activeSslDomains.length > 0 && !force) {
      return res.status(400).json({
        success: false,
        error: '证书正在为域名提供 HTTPS，无法直接删除',
        message: `证书正被以下域名的 Apache SSL 使用: ${activeSslDomains.join(', ')}。` +
          `如需续期，请用同名上传覆盖（域名可继续运行）；若确认删除，请使用强制删除（将禁用这些域名的 HTTPS）。`,
        usedByDomains,
        activeSslDomains,
        requireForce: true,
    ...
    const removedSslConfs = []
    if (force && activeSslDomains.length > 0) {
      console.log(`[删除证书] 强制删除，先禁用域名 HTTPS: ${activeSslDomains.join(', ')}`)
      for (const domain of activeSslDomains) {
        removedSslConfs.push(...removeApacheSslForDomain(domain))
      }
    }

    // 解除域名-证书关联（含仅映射、未启用 SSL 的情况）
    if (usedByDomains.length > 0) {
      unlinkCertFromDomains(certName, usedByDomains)
      console.log(`[删除证书] 已解除域名关联: ${usedByDomains.join(', ')}`)
    }

    const certFile = `${CERT_STORE_DIR}/${certName}.crt`
    const keyFile = `${CERT_STORE_DIR}/${certName}.key`
    const chainFile = `${CERT_STORE_DIR}/${certName}.chain.crt`
    const candidateFiles = [certFile, keyFile, chainFile]
    const filesToDelete = candidateFiles.filter(f => {
      try { return fs.existsSync(f) } catch (_) { return true }
    })

    // 即使 existsSync 因权限失败，也尝试 sudo rm
    let deletedFiles = []
    let errors = []
    const targets = filesToDelete.length > 0 ? filesToDelete : candidateFiles

    for (const filePath of targets) {
      try {
        execSync(`sudo rm -f "${filePath}"`, { timeout: 5000, encoding: 'utf8' })
        deletedFiles.push(filePath)
        console.log(`[删除证书] 已删除: ${filePath}`)
      } catch (e) {
        try {
          ...
          // 文件本来就不存在则忽略
          if (fs.existsSync(filePath)) {
            errors.push(`删除失败: ${filePath} - ${e2.message || e.message}`)
            console.error(`[删除证书] 删除失败: ${filePath}`, e2)
          }
        }
      }
    }

    if (deletedFiles.length === 0 && usedByDomains.length === 0 && removedSslConfs.length === 0) {
      return res.status(404).json({
        success: false,
        error: '证书文件不存在'
      })
    ...
    let reloaded = false
    if (removedSslConfs.length > 0) {
      try {
        reloadApacheGraceful()
        reloaded = true
      } catch (reloadErr) {
        console.warn('[删除证书] Apache 重载失败:', reloadErr.message)
        return res.status(200).json({
          success: true,
          message: '证书已删除，但 Apache 重载失败，请手动执行 systemctl reload httpd',
          deletedFiles,
          unboundDomains: usedByDomains,
          disabledSslDomains: activeSslDomains,
          warning: reloadErr.message
        })
      }
    }
    ...
    res.json({
      success: true,
      message: activeSslDomains.length > 0 && force
        ? `证书已强制删除，并已禁用域名 HTTPS: ${activeSslDomains.join(', ')}`
        : (usedByDomains.length > 0
          ? `证书已删除，并已解除域名关联: ${usedByDomains.join(', ')}`
          : '证书已删除'),
      deletedFiles,
      unboundDomains: usedByDomains,
      disabledSslDomains: force ? activeSslDomains : [],
      apacheReloaded: reloaded
    })
  } catch (error) {
...
app.post('/api/cert/disable-http-redirect', auth, csrfProtection, (req, res) => {
  try {
    const apacheConfDir = '/etc/httpd/conf.d'
    const configDir = path.join(ROOT_DIR, 'config')
    const httpRedirectStateFile = path.join(configDir, 'http-redirect-enabled.json')
...
// 证书上传API（使用multer处理文件上传；同名时覆盖续期，保持已配置域名 HTTPS 继续运行）
app.post('/api/cert/upload', auth, csrfProtection, upload.fields([
  { name: 'cert', maxCount: 1 },
  { name: 'key', maxCount: 1 }
]), async (req, res) => {
  try {
    const certNameRaw = req.body.certName
    
    if (!certNameRaw || typeof certNameRaw !== 'string' || !certNameRaw.trim()) {
      return res.status(400).json({
        success: false,
        error: '证书名称参数无效'
      })
    ...
    const certName = certNameRaw.trim()
    if (!isValidCertName(certName)) {
      return res.status(400).json({
        success: false,
        error: '证书名称格式无效，应为字母数字/连字符/下划线，或域名格式（如 example.com）'
      })
    }
    ...
    const targetCertPath = `${CERT_STORE_DIR}/${certName}.crt`
    const targetKeyPath = `${CERT_STORE_DIR}/${certName}.key`
    const existed = fs.existsSync(targetCertPath) || fs.existsSync(targetKeyPath)
    const usedByDomains = getDomainsUsingCert(certName)
    const activeSslDomains = getActiveSslDomainsForCert(certName)
    
    // 确保目标目录存在
    if (!fs.existsSync(CERT_STORE_DIR)) {
      try {
        execSync(`sudo mkdir -p "${CERT_STORE_DIR}"`, { timeout: 5000 })
      } catch (_) {
        fs.mkdirSync(CERT_STORE_DIR, { recursive: true })
      }
    }
    ...
    try {
      ...
      validatePemCertAndKey(certContent, keyContent)

      let backedUp = []
      if (existed) {
        backedUp = backupCertFiles(certName)
        console.log(`[证书上传] 同名覆盖续期: ${certName}，备份 ${backedUp.length} 个文件`)
      }

      installCertFile(certFile.path, targetCertPath, '644')
      installCertFile(keyFile.path, targetKeyPath, '600')

      // 清理临时文件
      try { fs.unlinkSync(certFile.path) } catch (_) {}
      try { fs.unlinkSync(keyFile.path) } catch (_) {}

      let apacheReloaded = false
      let reloadWarning = null
      // 已启用 HTTPS 的域名：原地覆盖后 graceful reload，不停站
      if (activeSslDomains.length > 0) {
        try {
          reloadApacheGraceful()
          apacheReloaded = true
          console.log(`[证书上传] 已优雅重载 Apache，域名继续 HTTPS: ${activeSslDomains.join(', ')}`)
        } catch (reloadErr) {
          reloadWarning = reloadErr.message
          console.warn('[证书上传] Apache 重载失败:', reloadErr.message)
        }
      }

      const message = existed
        ? (activeSslDomains.length > 0
          ? `证书 ${certName} 已覆盖续期，相关域名 HTTPS 继续运行${apacheReloaded ? '（已重载 Apache）' : ''}`
          : `证书 ${certName} 已覆盖更新`)
        : '证书上传成功'
      ...
      res.json({
        ...
        apacheReloaded,
        backedUp,
        warning: reloadWarning || undefined
      })
...
// 批量上传证书API
app.post('/api/cert/batch-upload', auth, csrfProtection, upload.any(), async (req, res) => {
  ...
  try {
    ...
    for (let i = 0; i < count; i++) {
      try {
        ...
        if (!isValidCertName(trimmedCertName)) {
        ...
        const targetCertPath = `${CERT_STORE_DIR}/${trimmedCertName}.crt`
        const targetKeyPath = `${CERT_STORE_DIR}/${trimmedCertName}.key`
        // chain证书保存为 certName.chain.crt 格式（如果上传了chain文件）
        const targetChainPath = chainFile ? `${CERT_STORE_DIR}/${trimmedCertName}.chain.crt` : null
        const existed = fs.existsSync(targetCertPath) || fs.existsSync(targetKeyPath)
        const activeSslDomainsForItem = getActiveSslDomainsForCert(trimmedCertName)

        // 同名已存在：备份后覆盖续期（不再拒绝），保持域名 HTTPS 运行
        if (existed) {
          const backedUp = backupCertFiles(trimmedCertName)
          console.log(`[批量上传] 证书 ${i}: 同名覆盖续期 ${trimmedCertName}，备份 ${backedUp.length} 个文件`)
        }
        ...
        try {
          ...
          let overwritten = existed
          if (isDomainFormat) {
            try {
              console.log(`[批量上传] 证书 ${i}: 检测到域名格式，处理 Apache/域名关联: ${trimmedCertName}`)
              ...
              if (!fs.existsSync(targetCertPath) || !fs.existsSync(targetKeyPath)) {
                ...
                saveDomainCertMap(domainCertMap)
                console.log(`[批量上传] 证书 ${i}: 域名-证书关联已保存: ${trimmedCertName} -> ${trimmedCertName}`)

                // 已启用 HTTPS：同名覆盖后仅优雅重载，不停站、不重建虚拟主机
                const sslAlreadyActive = activeSslDomainsForItem.length > 0 || isApacheSslConfiguredForDomain(trimmedCertName)
                if (sslAlreadyActive) {
                  try {
                    reloadApacheGraceful()
                    apacheConfigured = true
                    apacheConfigMessage = overwritten
                      ? '，证书已覆盖续期并优雅重载 Apache（域名 HTTPS 继续运行）'
                      : '，已优雅重载 Apache'
                    console.log(`[批量上传] 证书 ${i}: 已启用 SSL，覆盖后续期并 reload`)
                  } catch (reloadErr) {
                    apacheConfigMessage = '，证书已覆盖，但 Apache 重载失败，请手动 systemctl reload httpd'
                    console.warn(`[批量上传] 证书 ${i}: reload 失败:`, reloadErr.message)
                  }
                } else {
          ...
          } else if (existed && activeSslDomainsForItem.length > 0) {
            // 非域名命名但已被域名绑定使用：覆盖后续期并 reload
            try {
              reloadApacheGraceful()
              apacheConfigured = true
              apacheConfigMessage = '，证书已覆盖续期并优雅重载 Apache（关联域名 HTTPS 继续运行）'
            } catch (reloadErr) {
              apacheConfigMessage = '，证书已覆盖，但 Apache 重载失败，请手动 systemctl reload httpd'
              console.warn(`[批量上传] 证书 ${i}: reload 失败:`, reloadErr.message)
            }
          ...
          results.push({
            ...
            success: true,
            certName: trimmedCertName,
            message: (overwritten ? '证书覆盖续期成功' : '证书上传成功') + (chainFile ? '（包含证书链）' : '') + apacheConfigMessage,
    ...
    // 构建响应消息
    const overwriteCount = results.filter(r => r.success && r.overwritten).length
    let responseMessage = `成功上传 ${successCount} 个证书`
    if (overwriteCount > 0) {
      responseMessage += `（其中 ${overwriteCount} 个为同名覆盖续期）`
    }
    if (apacheConfiguredCount > 0) {
      responseMessage += `，其中 ${apacheConfiguredCount} 个已处理 Apache（覆盖续期为优雅重载，不停站；新建配置可能需重启）。请稍后验证 HTTPS。`
    } else if (successCount > 0) {
      // 检查是否有域名格式的证书但没有配置Apache
      const domainFormatCerts = results.filter(r => r.success && /^([a-z0-9]+(-[a-z0-9]+)*\.)+[a-z]{2,}$/i.test(r.certName || '') && !r.apacheConfigured)
      if (domainFormatCerts.length > 0) {
        responseMessage += '。注意：检测到域名格式证书，但Apache自动配置未成功，请手动配置SSL。'
        // 提取失败原因（message 中 "，" 后的内容，如 "Apache配置失败: xxx"）
        const reasons = domainFormatCerts
          .map(r => (r.message || '').replace(/^证书上传成功[^，]*，?/, '').replace(/，请手动.*$/, '').trim())
          .filter(r => r && !r.includes('已自动配置'))
        if (reasons.length > 0) {
          const uniqueReasons = [...new Set(reasons)].slice(0, 3) // 最多显示3条
          responseMessage += ` 失败原因：${uniqueReasons.join('；')}。`
        }
        responseMessage += ' 排查方法：查看调度层日志 journalctl -u mail-ops-dispatcher -n 100 --no-pager，或手动执行 sudo cert_setup.sh enable-ssl <域名> 配置。'
      }
    }
    if (failCount > 0) {
...
app.get('/api/version', (req, res) => {
  try {
    ...
    // 读取start.sh文件获取版本信息
    const startScriptPath = path.join(ROOT_DIR, 'start.sh')
    let version = '0.0.1' // 默认版本（获取失败时表示系统错误）
...
app.get('/api/system-status', auth, async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      // 兜底：只要有域名且 named 未运行，一律用公网 DNS 做健康检查，避免误判为全部缺失
      if (configuredDomain && services.named?.status !== 'running' && dnsResolver === '127.0.0.1') {
        dnsResolver = '8.8.8.8'
        inferredPublicDns = true
      }
...
// DNS配置API端点
// 配置DNS
app.post('/api/dns/configure', auth, csrfProtection, (req, res) => {
  try {
    const { 
      domain, 
      serverIp, 
      ...
      if (code === 0) {
        ...
        try {
          ...
          try {
            ...
            execSync(`${ROOT_DIR}/backend/scripts/mail_db.sh update-admin-email ${escapeShellArg(adminEmailForDomain)}`, { timeout: 5000 })
...
// 公网DNS配置API端点
app.post('/api/dns/public-configure', auth, csrfProtection, (req, res) => {
  try {
    ...
    child.on('close', (code) => {
      if (code === 0) {
        ...
        try {
          ...
          // 公网DNS配置成功后，与本地Bind逻辑完全一致：添加域名到邮件域管理 + 配置邮件系统
          const mailDbScript = path.join(__dirname, '..', 'scripts', 'mail_db.sh')
          const mailSetupScript = path.join(__dirname, '..', 'scripts', 'mail_setup.sh')
          if (fs.existsSync(mailDbScript)) {
            // 1. 先确保 localhost 域名存在（mail_db.sh add_domain 内部会检查，存在则跳过）
            // 使用 cd ROOT_DIR 确保脚本工作目录正确（与 app_user.sh、Bind 逻辑一致）
            exec(`cd "${ROOT_DIR}" && bash "${mailDbScript}" add_domain "localhost"`, (localhostError, localhostStdout) => {
              if (localhostError) {
                const localhostOutput = (localhostStdout || '').toString()
                if (localhostOutput.includes('已存在')) {
                  console.log('localhost 域名已存在于邮件域管理中')
                } else {
                  console.warn('自动添加 localhost 域名失败（不影响DNS配置）:', localhostError.message)
                }
              } else {
                const localhostOutput = (localhostStdout || '').toString()
                if (localhostOutput.includes('域名添加成功')) {
                  console.log('localhost 域名已自动添加到邮件域管理')
                } else if (localhostOutput.includes('已存在')) {
                  console.log('localhost 域名已存在于邮件域管理中')
                }
              }
              // 2. 添加公网域名（mail_db.sh add_domain 内部会检查，存在则跳过）
              exec(`cd "${ROOT_DIR}" && bash "${mailDbScript}" add_domain ${escapeShellArg(domain)}`, (domainError, domainStdout) => {
                if (domainError) {
                  console.warn('公网DNS域名添加到邮件域管理失败:', domainError.message)
                } else {
                  const domainOutput = (domainStdout || '').toString()
                  if (domainOutput.includes('域名添加成功')) {
                    console.log(`公网DNS域名 ${domain} 已自动添加到邮件域管理`)
                  } else if (domainOutput.includes('已存在')) {
                    console.log(`公网DNS域名 ${domain} 已存在于邮件域管理中`)
                  } else {
                    console.warn('公网DNS域名添加结果未知:', domainOutput)
                  }
                }
                // 3. DNS配置完成后，自动配置邮件系统（Postfix/Dovecot），与 Bind 逻辑一致
                if (fs.existsSync(mailSetupScript)) {
                  console.log('公网DNS配置完成，开始配置邮件系统:', domain)
                  exec(`cd "${ROOT_DIR}" && bash "${mailSetupScript}" configure ${escapeShellArg(domain)}`, (mailError, mailStdout) => {
                    if (mailError) {
                      console.warn('公网DNS配置完成后自动配置邮件系统失败:', mailError.message)
                    } else {
                      console.log('公网DNS邮件系统配置成功:', (mailStdout || '').toString().trim())
                    }
                  })
                }
              })
            ...
            console.warn('未找到 mail_db.sh，跳过公网DNS域名添加到邮件域管理，请手动在系统设置中添加域名:', domain)
          }
        } catch (settingsError) {
...
app.get('/api/user-email', auth, (req, res) => {
  try {
    ...
    try {
      const mailDbPass = getMailDbPassword()
      const userEmailResult = execSync(`mysql -u mailuser -p"${mailDbPass.replace(/"/g, '\\"')}" maildb -s -r -e "SELECT email FROM mail_users WHERE username='${escapeSqlString(user)}' LIMIT 1;"`, { encoding: 'utf8', timeout: 3000 }).trim()
...
// 保存系统设置
app.post('/api/system-settings', auth, csrfProtection, (req, res) => {
  try {
    const timestamp = new Date().toISOString()
    const clientIP = getRealClientIP(req)
    const user = req.headers.authorization ? Buffer.from(req.headers.authorization.split(' ')[1], 'base64').toString().split(':')[0] : 'unknown'
    ...
    if (settings.general && settings.general.adminEmail) {
      try {
        ...
        if (adminEmail && adminEmail.includes('@')) {
          // 更新数据库中的xm用户邮箱
          const mailDbPass = getMailDbPassword()
          execSync(`mysql -u mailuser -p"${mailDbPass.replace(/"/g, '\\"')}" maildb -e "UPDATE mail_users SET email='${escapeSqlString(adminEmail)}' WHERE username='xm';"`, { timeout: 5000 })
...
app.post('/api/upload-avatar', auth, csrfProtection, (req, res) => {
  try {
    const timestamp = new Date().toISOString()
    const clientIP = getRealClientIP(req)
    const user = req.headers.authorization ? Buffer.from(req.headers.authorization.split(' ')[1], 'base64').toString().split(':')[0] : 'unknown'
...
// 测试系统通知API
app.post('/api/notifications/test-system-alert', auth, csrfProtection, async (req, res) => {
  try {
    ...
    try {
      ...
      const domainCheck = execSync(
        `mysql -u mailuser -p"${getMailDbPassword().replace(/"/g, '\\"')}" maildb -s -r -e "SELECT id FROM virtual_domains WHERE name='${escapeSqlString(domain)}' LIMIT 1;" 2>/dev/null || echo ""`,
      ...
      if (!domainId) {
        ...
        execSync(
          `mysql -u mailuser -p"${getMailDbPassword().replace(/"/g, '\\"')}" maildb -e "INSERT INTO virtual_domains (name) VALUES ('${escapeSqlString(domain)}');" 2>/dev/null`,
          { timeout: 5000 }
        )
        domainId = execSync(
          `mysql -u mailuser -p"${getMailDbPassword().replace(/"/g, '\\"')}" maildb -s -r -e "SELECT id FROM virtual_domains WHERE name='${escapeSqlString(domain)}' LIMIT 1;" 2>/dev/null`,
          { encoding: 'utf8', timeout: 5000 }
        ).trim()
      ...
      const userCheck = execSync(
        `mysql -u mailuser -p"${getMailDbPassword().replace(/"/g, '\\"')}" maildb -s -r -e "SELECT id FROM virtual_users WHERE email='${escapeSqlString(recipientEmail)}' LIMIT 1;" 2>/dev/null || echo ""`,
        { encoding: 'utf8', timeout: 5000 }
      ).trim()
      ...
      if (!userCheck) {
        ...
        execSync(
          `mysql -u mailuser -p"${getMailDbPassword().replace(/"/g, '\\"')}" maildb -e "INSERT INTO virtual_users (domain_id, email, password, active) VALUES (${domainId}, '${escapeSqlString(recipientEmail)}', '${escapeSqlString(defaultPassword)}', 1);" 2>/dev/null`,
          { timeout: 5000 }
        )
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      try {
        ...
        // 检查用户是否存在
        const userCheck = execSync(
          `mysql -u mailuser -p"${getMailDbPassword().replace(/"/g, '\\"')}" maildb -s -r -e "SELECT id FROM virtual_users WHERE email='${escapeSqlString(toEmail)}' LIMIT 1;" 2>/dev/null || echo ""`,
          { encoding: 'utf8', timeout: 5000 }
        ).trim()
        ...
        if (!userCheck) {
          ...
          const defaultPassword = '$6$rounds=5000$defaultsalt$' + crypto.randomBytes(16).toString('hex')
          execSync(
            `mysql -u mailuser -p"${getMailDbPassword().replace(/"/g, '\\"')}" maildb -e "INSERT INTO virtual_users (domain_id, email, password, active) VALUES (${domainId}, '${escapeSqlString(toEmail)}', '${escapeSqlString(defaultPassword)}', 1);" 2>/dev/null`,
            { timeout: 5000 }
          )
...
// ========== 邮件附件分片上传（支持1GB大文件）==========
const MAIL_UPLOAD_DIR = '/tmp/mail-uploads'
const CHUNK_SIZE_LIMIT = 10 * 1024 * 1024 // 单分片最大10MB
const MAX_FILE_SIZE = 1024 * 1024 * 1024 // 单文件最大1GB
const MAX_TOTAL_ATTACHMENTS = 2 * 1024 * 1024 * 1024 // 总附件最大2GB

// 分片上传用multer（内存存储，单分片10MB）
const mailChunkStorage = multer.memoryStorage()
const mailChunkUpload = multer({
  storage: mailChunkStorage,
  limits: { fileSize: CHUNK_SIZE_LIMIT },
  fileFilter: (req, file, cb) => cb(null, true)
})

// 确保上传目录存在
if (!fs.existsSync(MAIL_UPLOAD_DIR)) {
  fs.mkdirSync(MAIL_UPLOAD_DIR, { recursive: true })
}

// 分片上传API
app.post('/api/mail/upload-chunk', auth, csrfProtection, mailChunkUpload.single('chunk'), async (req, res) => {
  try {
    ...
    if (!uploadId || chunkIndex === undefined || !totalChunks || !filename) {
      return res.status(400).json({
        success: false,
        error: '缺少参数',
        message: '需要 uploadId, chunkIndex, totalChunks, filename'
      })
    }

    const chunkIdx = parseInt(chunkIndex, 10)
    const total = parseInt(totalChunks, 10)
    const expectedSize = parseInt(fileSize, 10) || 0

    if (isNaN(chunkIdx) || isNaN(total) || chunkIdx < 0 || chunkIdx >= total) {
      return res.status(400).json({
        success: false,
        error: '参数无效',
        message: 'chunkIndex 或 totalChunks 无效'
      })
    }
    ...
    if (expectedSize > MAX_FILE_SIZE) {
      return res.status(400).json({
        success: false,
        error: '文件过大',
        message: `单文件最大支持 ${MAX_FILE_SIZE / 1024 / 1024 / 1024}GB`
      })
    }

    const uploadDir = path.join(MAIL_UPLOAD_DIR, uploadId)
    if (!fs.existsSync(uploadDir)) {
      fs.mkdirSync(uploadDir, { recursive: true })
    }

    const chunkPath = path.join(uploadDir, `part_${chunkIdx}`)
    fs.writeFileSync(chunkPath, chunkFile.buffer)

    const receivedCount = fs.readdirSync(uploadDir).filter(f => f.startsWith('part_')).length

    if (receivedCount === total) {
      // 所有分片已到齐，合并为最终文件
      const safeFilename = path.basename(filename).replace(/[^a-zA-Z0-9._-]/g, '_') || 'attachment'
      const finalPath = path.join(uploadDir, `final_${safeFilename}`)
      const writeStream = fs.createWriteStream(finalPath, { flags: 'w' })

      for (let i = 0; i < total; i++) {
        const partPath = path.join(uploadDir, `part_${i}`)
      ...
      console.log(`[MAIL_UPLOAD] 分片合并完成: uploadId=${uploadId}, filename=${safeFilename}, size=${(stats.size / 1024 / 1024).toFixed(2)}MB`)
    ...
    console.error('[MAIL_UPLOAD] 分片上传错误:', error)
    res.status(500).json({
      success: false,
      error: '上传失败',
      message: error.message
...
// 清理过期的分片上传（超过1小时未完成的）
function cleanupExpiredMailUploads() {
  try {
    const dirs = fs.readdirSync(MAIL_UPLOAD_DIR)
    const now = Date.now()
    const maxAge = 60 * 60 * 1000 // 1小时
    for (const d of dirs) {
      const dirPath = path.join(MAIL_UPLOAD_DIR, d)
      ...
      if (stat.isDirectory() && (now - stat.mtimeMs) > maxAge) {
        ...
        console.log(`[MAIL_UPLOAD] 已清理过期上传: ${d}`)
      }
    }
  } catch (e) {
    console.warn('[MAIL_UPLOAD] 清理失败:', e.message)
  }
}
setInterval(cleanupExpiredMailUploads, 15 * 60 * 1000) // 每15分钟清理一次
...
// 发送邮件（支持分片上传的大附件，单文件最大1GB）
app.post('/api/mail/send', auth, csrfProtection, async (req, res) => {
  try {
    const { to, cc, subject, body, from, attachments, folder, isForwarded } = req.body
    const timestamp = new Date().toISOString()
    const clientIP = getRealClientIP(req)
    const user = req.headers.authorization ? Buffer.from(req.headers.authorization.split(' ')[1], 'base64').toString().split(':')[0] : 'unknown'
    ...
    // 解析附件（支持 content 内联 与 uploadId 分片上传两种格式）
    let resolvedAttachments = []
    if (attachments && attachments.length > 0) {
      const MAX_SINGLE_FILE_SIZE = MAX_FILE_SIZE // 1GB
      const MAX_TOTAL_SIZE = MAX_TOTAL_ATTACHMENTS // 2GB
      let totalSize = 0
      ...
      for (const att of attachments) {
        ...
        if (att.uploadId) {
          // 分片上传：从 /tmp/mail-uploads/{uploadId}/final_{filename} 读取
          const safeName = (att.name || att.filename || 'attachment').replace(/[^a-zA-Z0-9._-]/g, '_')
          const uploadDir = path.join(MAIL_UPLOAD_DIR, att.uploadId)
          filePath = path.join(uploadDir, `final_${safeName}`)
          if (!fs.existsSync(filePath)) {
            return res.status(400).json({
              success: false,
              error: '附件未找到',
              message: `分片上传文件不存在或已过期: ${att.name || safeName}`
            })
          }
          const stat = fs.statSync(filePath)
          fileSize = stat.size
          resolvedAttachments.push({
            name: att.name || att.filename || safeName,
          ...
          fileSize = Math.floor(att.content.length * 0.75)
          resolvedAttachments.push({
            name: att.name || '未知文件',
            type: att.type || 'application/octet-stream',
            size: fileSize,
          ...
          fileSize = att.size
          resolvedAttachments.push({ ...att, path: null })
        }
      ...
      if (oversizedFiles.length > 0) {
        return res.status(400).json({
          success: false,
          error: '附件大小超限',
          message: `以下文件超过1GB限制: ${oversizedFiles.map(f => f.name).join(', ')}`,
          details: { oversizedFiles, maxSingleFileSize: '1GB' }
        })
      }
      if (totalSize > MAX_TOTAL_SIZE) {
        return res.status(400).json({
          success: false,
          error: '附件总大小超限',
          message: `附件总大小为 ${(totalSize / 1024 / 1024 / 1024).toFixed(2)}GB，超过2GB限制，请分批发送`,
          details: { totalSize: (totalSize / 1024 / 1024 / 1024).toFixed(2) + 'GB', maxTotalSize: '2GB' }
        })
      }
      console.log(`[MAIL_SEND_REQUEST] 附件验证通过: 总大小=${(totalSize / 1024 / 1024).toFixed(2)}MB, 文件数=${resolvedAttachments.length}`)
    }
    
    // 转发邮件：添加主题前缀 Fwd: 或 转发:（若尚未包含）
    let effectiveSubject = subject || ''
    if (isForwarded && effectiveSubject && !/^(Fwd:\s*|转发:\s*)/i.test(effectiveSubject.trim())) {
      effectiveSubject = `Fwd: ${effectiveSubject.trim()}`
    }

    // 验证邮件参数（如果是草稿，允许to为空）
    const isDraft = folder === 'drafts'
    if (!isDraft && (!to || !effectiveSubject || !body)) {
      console.error(`[MAIL_SEND_ERROR] Missing required fields - To: ${to ? 'provided' : 'missing'}, Subject: ${effectiveSubject ? 'provided' : 'missing'}, Body: ${body ? 'provided' : 'missing'}`)
      return res.status(400).json({
        success: false,
    ...
    if (isDraft && !effectiveSubject && !body) {
      return res.status(400).json({
        success: false,
    ...
    if (isDraft) {
      ...
      const userEmailResult = execSync(`mysql -u mailuser -p"${mailDbPass.replace(/"/g, '\\"')}" maildb -s -r -e "SELECT email FROM mail_users WHERE username='${escapeSqlString(user)}' LIMIT 1;"`, { encoding: 'utf8', timeout: 3000 }).trim()
        ...
        // 处理附件 - 支持 content 与 path（分片上传）
        const storageAttachments = (resolvedAttachments || []).map(a =>
          a.path ? { name: a.name, path: a.path, type: a.type, size: a.size } : { name: a.name, content: a.content, type: a.type, size: a.size }
        )
        if (storageAttachments.length > 0) {
          try {
            const attachmentsJson = JSON.stringify(storageAttachments)
        ...
        console.log('Saving draft:', { messageId, userEmail, to: to || '', subject: effectiveSubject || '(无主题)', targetFolder, hasAttachments: !!attachmentsFile })
        
        // 转义特殊字符，避免shell注入
        const safeMessageId = messageId.replace(/"/g, '\\"')
        const safeUserEmail = userEmail.replace(/"/g, '\\"')
        const safeTo = (to || '').replace(/"/g, '\\"')
        const safeSubject = (effectiveSubject || '(无主题)').replace(/"/g, '\\"').replace(/\$/g, '\\$')
    ...
    // 发送前检查收件人邮箱剩余空间：主收件人(to)任一满则整封拒发；抄送(cc)满则仅跳过该人，其他正常发送
    let effectiveTo = to || ''
    let effectiveCc = (cc && cc.trim()) ? cc.trim() : ''
    let storageRejectedCc = []
    try {
      ...
      const toAddrs = (to || '').split(',').map(extractEmail).filter(e => e && e.includes('@'))
      const ccAddrs = (cc || '').split(',').map(extractEmail).filter(e => e && e.includes('@'))
      if (toAddrs.length > 0 || ccAddrs.length > 0) {
        ...
        try {
          ...
          if (fs.existsSync(settingsFile)) {
            const settings = JSON.parse(fs.readFileSync(settingsFile, 'utf8'))
            if (settings.mail?.maxMailboxSize) limitMB = Number(settings.mail.maxMailboxSize) || 1024
          }
        } catch (e) { /* use default */ }
        const limitBytes = limitMB * 1024 * 1024
        const mailDbScript = path.join(ROOT_DIR, 'backend', 'scripts', 'mail_db.sh')
        const checkRecipient = (email) => {
          try {
            const statsOutput = execSync(`bash "${mailDbScript}" stats ${escapeShellArg(email)}`, { encoding: 'utf8', timeout: 10000 })
            const stats = JSON.parse(statsOutput)
            const usedBytes = Number(stats.deduplicated_size) >= 0 ? Number(stats.deduplicated_size) : [stats.inbox, stats.sent, stats.drafts, stats.trash, stats.spam]
              .filter(Boolean)
              .reduce((sum, f) => sum + (Number(f.size) || 0), 0)
            return usedBytes < limitBytes
          } catch (e) {
            console.warn(`收件人 ${email} 存储检查失败，视为有空间:`, e.message)
            return true
          }
        }
        const toAllowed = toAddrs.filter(e => checkRecipient(e))
        const toRejected = toAddrs.filter(e => !toAllowed.includes(e))
        if (toRejected.length > 0) {
          const storageLog = `[${timestamp}] [TO_RECIPIENT_STORAGE_FULL] Sender: ${user}, Rejected: ${toRejected.join(', ')}, IP: ${clientIP}\n`
          fs.appendFileSync(path.join(LOG_DIR, 'mail-operations.log'), storageLog)
          return res.status(400).json({
            success: false,
            error: 'storage_quota_exceeded',
            message: `主收件人 ${toRejected.join('、')} 的邮箱存储已满，无法接收邮件。若主收件人未收到，抄送人也不会收到。请通知对方清理或联系管理员扩容。`,
            stored: false
          })
        }
        const ccAllowed = ccAddrs.filter(e => checkRecipient(e))
        const ccRejected = ccAddrs.filter(e => !ccAllowed.includes(e))
        if (ccRejected.length > 0) {
          storageRejectedCc = ccRejected
          const storageLog = `[${timestamp}] [CC_RECIPIENT_STORAGE_SKIP] Sender: ${user}, Skipped: ${ccRejected.join(', ')}, IP: ${clientIP}\n`
          fs.appendFileSync(path.join(LOG_DIR, 'mail-operations.log'), storageLog)
        }
        effectiveTo = toAllowed.join(', ')
        effectiveCc = ccAllowed.join(', ')
      }
    } catch (storageCheckErr) {
      console.warn('收件人邮箱存储检查失败，继续发送:', storageCheckErr.message)
    }
    ...
    try {
      ...
      // 检查发件人域名
      const mailDbPass = getMailDbPassword()
      const fromDomainCheck = execSync(`mysql -u mailuser -p"${mailDbPass.replace(/"/g, '\\"')}" maildb -s -r -e "SELECT COUNT(*) FROM virtual_domains WHERE name='${escapeSqlString(fromDomain)}';"`, { encoding: 'utf8', timeout: 3000 }).trim()
      ...
      // 检查收件人域名（主收件人，支持多收件人时取第一个验证）
      const firstToAddr = effectiveTo.split(',')[0].trim()
      const firstToMatch = firstToAddr.match(/<([^>]+)>/)
      const firstToEmail = firstToMatch ? firstToMatch[1].trim() : firstToAddr
      const toDomain = firstToEmail.split('@')[1]
      if (!toDomain) {
        throw new Error(`无法从收件人地址提取域名: ${firstToEmail}`)
      }
      
      const toDomainCheck = execSync(`mysql -u mailuser -p"${getMailDbPassword().replace(/"/g, '\\"')}" maildb -s -r -e "SELECT COUNT(*) FROM virtual_domains WHERE name='${escapeSqlString(toDomain)}';"`, { encoding: 'utf8', timeout: 3000 }).trim()
      
      if (toDomainCheck === '0') {
        const domainLogLine = `[${timestamp}] [DOMAIN_REJECTED] User: ${user}, To: ${effectiveTo}, Domain: ${toDomain}, Reason: 收件人域名不在允许列表中, IP: ${clientIP}\n`
        fs.appendFileSync(path.join(LOG_DIR, 'mail-operations.log'), domainLogLine)
      ...
      // 检查抄送收件人域名（如果有，使用过滤后的抄送列表）
      if (effectiveCc && effectiveCc.trim()) {
        const extractEmailForDomain = (addr) => {
          const s = (addr || '').trim()
          const m = s.match(/<([^>]+)>/)
          return m ? m[1].trim() : s
        }
        const ccEmails = effectiveCc.split(',').map(extractEmailForDomain).filter(e => e && e.includes('@'))
        for (const ccEmail of ccEmails) {
          ...
          const ccDomainCheck = execSync(`mysql -u mailuser -p"${getMailDbPassword().replace(/"/g, '\\"')}" maildb -s -r -e "SELECT COUNT(*) FROM virtual_domains WHERE name='${escapeSqlString(ccDomain)}';"`, { encoding: 'utf8', timeout: 3000 }).trim()
    ...
    try {
      const firstToForVirtual = effectiveTo.split(',')[0].trim()
      const firstToMatchV = firstToForVirtual.match(/<([^>]+)>/)
      const firstToEmailV = firstToMatchV ? firstToMatchV[1].trim() : firstToForVirtual
      // 快速检查主收件人是否存在（同步，支持多收件人时检查第一个）
      const toUserCheck = execSync(
        `mysql -u mailuser -p"${getMailDbPassword().replace(/"/g, '\\"')}" maildb -s -r -e "SELECT id FROM virtual_users WHERE email='${escapeSqlString(firstToEmailV)}' LIMIT 1;" 2>/dev/null || echo ""`,
        { encoding: 'utf8', timeout: 2000 }
      ).trim()
      
      if (!toUserCheck) {
        // 主收件人不存在，快速创建（同步，但只执行必要的操作）
        const toEmailParts = firstToEmailV.split('@')
        const toEmailDomain = toEmailParts[1]
        const toEmailUsername = toEmailParts[0]
        
        // 获取或创建域名ID（合并查询）
        let toDomainId = execSync(
          `mysql -u mailuser -p"${getMailDbPassword().replace(/"/g, '\\"')}" maildb -s -r -e "SELECT id FROM virtual_domains WHERE name='${escapeSqlString(toEmailDomain)}' LIMIT 1;" 2>/dev/null || echo ""`,
          { encoding: 'utf8', timeout: 2000 }
        ).trim()
        
        if (!toDomainId) {
          execSync(
            `mysql -u mailuser -p"${getMailDbPassword().replace(/"/g, '\\"')}" maildb -e "INSERT INTO virtual_domains (name) VALUES ('${escapeSqlString(toEmailDomain)}'); SELECT id FROM virtual_domains WHERE name='${escapeSqlString(toEmailDomain)}' LIMIT 1;" 2>/dev/null`,
            { timeout: 2000 }
          )
          toDomainId = execSync(
            `mysql -u mailuser -p"${getMailDbPassword().replace(/"/g, '\\"')}" maildb -s -r -e "SELECT id FROM virtual_domains WHERE name='${escapeSqlString(toEmailDomain)}' LIMIT 1;" 2>/dev/null`,
            { encoding: 'utf8', timeout: 2000 }
          ).trim()
        }
        ...
        execSync(
          `mysql -u mailuser -p"${getMailDbPassword().replace(/"/g, '\\"')}" maildb -e "INSERT INTO virtual_users (domain_id, email, password, active) VALUES (${toDomainId}, '${escapeSqlString(firstToEmailV)}', '${escapeSqlString(userPassword)}', 1);" 2>/dev/null`,
          { timeout: 2000 }
        )
        console.log(`主收件人 ${firstToEmailV} 已快速创建`)
      ...
      // 抄送及额外主收件人异步检查（不阻塞请求，多主收件人时第2个起与抄送一并处理）
      const extractCcForVirtual = (addr) => {
        const s = (addr || '').trim()
        const m = s.match(/<([^>]+)>/)
        return m ? m[1].trim() : s
      }
      const allToEmails = effectiveTo.split(',').map(extractCcForVirtual).filter(e => e && e.includes('@'))
      const additionalToEmails = allToEmails.slice(1)
      const ccEmails = (effectiveCc && effectiveCc.trim()) ? effectiveCc.split(',').map(extractCcForVirtual).filter(e => e && e.includes('@')) : []
      const allRecipientsToEnsure = [...additionalToEmails, ...ccEmails]
      if (allRecipientsToEnsure.length > 0) {
        const mailDbPass = getMailDbPassword().replace(/'/g, "'\\''").replace(/\$/g, '\\$')
        // 异步处理额外主收件人和抄送人，不阻塞请求
        exec(`bash -c 'MAIL_DB_PASS="${mailDbPass}"
for email in ${allRecipientsToEnsure.map(e => `"${e}"`).join(' ')}; do
    ...
    // 发送前检查邮件大小限制（应用层强制拦截，确保与 Postfix 配置一致）
    let maxMessageSizeMB = 500
    try {
      const settingsFile = path.join(ROOT_DIR, 'config', 'system-settings.json')
      if (fs.existsSync(settingsFile)) {
        const settings = JSON.parse(fs.readFileSync(settingsFile, 'utf8'))
        if (settings.mail?.maxMessageSize) {
          maxMessageSizeMB = Number(settings.mail.maxMessageSize) || 500
        }
      }
    } catch (cfgErr) {
      console.warn('读取邮件大小限制失败，使用默认 500MB:', cfgErr.message)
    }
    const maxMessageSizeBytes = maxMessageSizeMB * 1024 * 1024
    const bodySizeBytes = Buffer.byteLength(body || '', 'utf8')
    const attachmentsSizeBytes = (resolvedAttachments || []).reduce((sum, att) => sum + (att.size || 0), 0)
    const totalMailSizeBytes = bodySizeBytes + attachmentsSizeBytes
    if (totalMailSizeBytes > maxMessageSizeBytes) {
      const sizeLog = `[${timestamp}] [MAIL_SIZE_EXCEEDED] User: ${user}, To: ${to}, Total: ${(totalMailSizeBytes / 1024 / 1024).toFixed(2)}MB, Limit: ${maxMessageSizeMB}MB, IP: ${clientIP}\n`
      fs.appendFileSync(path.join(LOG_DIR, 'mail-operations.log'), sizeLog)
      return res.status(400).json({
        success: false,
        error: 'message_size_exceeded',
        message: `邮件总大小 ${(totalMailSizeBytes / 1024 / 1024).toFixed(2)}MB 超过限制 ${maxMessageSizeMB}MB，请压缩附件或联系管理员调整「最大邮件大小」`,
        stored: false
      })
    }
    ...
    try {
      ...
      spamResult = checkSpamContent(effectiveSubject, body, from || user, to, spamConfig)
      
      if (spamResult.isSpam) {
        isSpamDetected = true
        // 记录垃圾邮件检测日志
        const spamLogLine = `[${timestamp}] [SPAM_DETECTED] User: ${user}, To: ${to}, Subject: ${effectiveSubject}, Violations: ${spamResult.violations.length}, Score: ${spamResult.spamScore}, IP: ${clientIP}\n`
    ...
    const attachmentInfo = resolvedAttachments && resolvedAttachments.length > 0 ? `, Attachments: ${resolvedAttachments.length}` : ''
    const logLine = `[${timestamp}] [MAIL_SEND] User: ${user}, To: ${to}${ccInfo}, Subject: ${effectiveSubject}, BodyLength: ${body.length}${attachmentInfo}, IP: ${clientIP}\n`
    fs.appendFileSync(path.join(LOG_DIR, 'mail-operations.log'), logLine)
    ...
    if (resolvedAttachments && resolvedAttachments.length > 0) {
      attachmentsArray = resolvedAttachments.map(att => {
        if (att.path && fs.existsSync(att.path)) {
          const stat = fs.statSync(att.path)
          console.log(`[附件] 使用文件路径: ${att.name}, size=${stat.size} bytes`)
          return { filename: att.name, content: fs.createReadStream(att.path), contentType: att.type }
        }
        const buf = Buffer.from(att.content || '', 'base64')
        console.log(`[附件] 使用base64内容: ${att.name}, contentLen=${(att.content || '').length}, bufferLen=${buf.length}`)
        return { filename: att.name, content: buf, contentType: att.type }
      })
      console.log(`[附件] 已准备 ${attachmentsArray.length} 个附件供 nodemailer 发送`)
    }
    ...
    if (hasAttachments) {
      // 计算总附件大小
      const totalSize = resolvedAttachments.reduce((sum, att) => sum + (att.size || 0), 0)
    ...
    const storageAttachmentsForMail = (resolvedAttachments || []).map(a =>
      a.path ? { name: a.name, path: a.path, type: a.type, size: a.size } : { name: a.name, content: a.content, type: a.type, size: a.size }
    )
    const attachmentsSize = (storageAttachmentsForMail || []).reduce((sum, att) => sum + (att.size || 0), 0)
    const totalMailSize = bodySize + attachmentsSize
    if (storageAttachmentsForMail.length > 0) {
      attachmentsJson = JSON.stringify(storageAttachmentsForMail)
      const baseTempFile = `/tmp/attachments_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`
      
      // 为收件箱创建临时文件（Python 从文件读取，避免大 base64 通过 heredoc 截断）
      attachmentsFileInbox = `${baseTempFile}_inbox.json`
      fs.writeFileSync(attachmentsFileInbox, attachmentsJson)
      console.log(`[存储邮件] 附件临时文件已创建: ${attachmentsFileInbox}, 大小=${attachmentsJson.length} 字节, 附件数=${storageAttachmentsForMail.length}`)
    ...
    // 存储邮件到数据库的辅助函数
    // onComplete: 可选回调，在收件箱和已发送存储都完成后调用，参数 { inboxSuccess, sentSuccess }
    // 仅当 isError 且传入 onComplete 时才会等待并调用；成功发送时不需要等待
    const storeMailToDatabase = (messageId, isError = false, errorObj = null, onComplete = null) => {
      console.log(`[存储邮件] 开始存储邮件到数据库: messageId=${messageId}, isError=${isError}, isSpam=${spamDetected}`)
      console.log(`[存储邮件] 参数: fromAddr=${fromAddr}, to=${to}, subject=${effectiveSubject}, bodySize=${bodySize}`)
      ...
      const startLog = `[${new Date().toISOString()}] [MAIL_STORE_START] messageId=${messageId}, fromAddr=${fromAddr}, to=${to}, subject=${effectiveSubject}, isError=${isError}${spamInfo}\n`
      ...
      if (isError && errorObj && errorObj.response && effectiveTo) {
        // 检查错误信息中是否包含主收件人地址，如果包含说明主收件人不存在
        const errorResponse = String(errorObj.response)
        if (errorResponse.includes(`<${effectiveTo}>`)) {
          shouldStoreToInbox = false
          console.log(`[存储邮件] 主收件人地址不存在，跳过收件箱存储: ${effectiveTo}`)
        }
      }
      
      // 用于 onComplete 时追踪收件箱和已发送的存储结果（仅 isError 且传入 onComplete 时使用）
      const storeResults = { inboxSuccess: !shouldStoreToInbox, sentSuccess: false }
      const totalToWait = (shouldStoreToInbox ? 1 : 0) + 1
      let completedCount = 0
      const maybeComplete = () => {
        completedCount++
        if (onComplete && completedCount >= totalToWait) {
          onComplete(storeResults)
        }
      }
      ...
      if (shouldStoreToInbox) {
        ...
        const safeTo = escapeShellArg(effectiveTo)
        const safeSubject = escapeShellArg(effectiveSubject)
        ...
        const isForwardedParam = isForwarded ? '1' : '0'
        const inboxCommand = `bash "${mailDbScript}" store ${safeInboxMessageId} ${safeFromAddr} ${safeTo} ${safeSubject} ${safeBody} ${safeHtmlBody} "${targetFolder}" "${totalMailSize}" ${safeCcValue} ${safeAttachmentsParamInbox} "{}" "0" "0" "${isForwardedParam}"`
        ...
        exec(inboxCommand, { maxBuffer: 50 * 1024 * 1024 }, (storeError, stdout, stderr) => {
          // 清理临时文件（如果存在）
          ...
          if (storeError) {
            ...
            storeResults.inboxSuccess = false
          } else {
            const folderName = spamDetected ? '垃圾邮箱' : '收件箱'
            console.log(`[存储邮件] ${folderName}存储成功: recipient=${effectiveTo}, messageId=${inboxMessageId}`)
            if (stdout) console.log(`[存储邮件] stdout: ${stdout}`)
            // 记录成功日志到文件
            const successLog = `[${new Date().toISOString()}] [MAIL_STORE_SUCCESS] Stored ${targetFolder} mail: recipient=${effectiveTo}, messageId=${inboxMessageId}, isSpam=${spamDetected}\n`
            fs.appendFileSync(path.join(LOG_DIR, 'mail-operations.log'), successLog)
            storeResults.inboxSuccess = true
          }
          maybeComplete()
        })
        ...
        if (ccValue) {
          ...
          ccAddresses.forEach((ccAddr, index) => {
            ...
            // 这样抄送收件人看到的邮件信息才是正确的
            const isForwardedParamCc = isForwarded ? '1' : '0'
            const ccCommand = `bash "${mailDbScript}" store ${safeCcMessageId} ${safeFromAddr} ${safeTo} ${safeSubject} ${ccSafeBody} ${ccSafeHtmlBody} "inbox" "${totalMailSize}" ${safeCcValue} ${safeAttachmentsParamCc} "{}" "0" "0" "${isForwardedParamCc}"`
            console.log(`[存储邮件] 执行抄送存储命令 (收件人: ${ccAddr}): ${ccCommand.substring(0, 200)}...`)
            
            exec(ccCommand, { maxBuffer: 50 * 1024 * 1024 }, (storeError, stdout, stderr) => {
              if (storeError) {
      ...
      const safeToSent = escapeShellArg(effectiveTo)
      const safeSubjectSent = escapeShellArg(effectiveSubject)
      ...
      // 存储到已发送文件夹
      const isForwardedParamSent = isForwarded ? '1' : '0'
      const sentCommand = `bash "${mailDbScript}" store ${safeSentMessageId} ${safeFromAddrSent} ${safeToSent} ${safeSubjectSent} ${safeBodySent} ${safeHtmlBodySent} "sent" "${totalMailSize}" ${safeCcValueSent} ${safeAttachmentsParamSent} "{}" "0" "0" "${isForwardedParamSent}"`
      ...
      if (spamDetected) {
        ...
        const spamCommand = `bash "${mailDbScript}" store ${safeSpamMessageId} ${safeFromAddrSent} ${safeToSent} ${safeSubjectSent} ${safeSpamBodySent} ${safeSpamHtmlBodySent} "spam" "${totalMailSize}" ${safeCcValueSent} ${safeAttachmentsParamSent} "{}" "0" "0" "${isForwardedParamSent}"`
        console.log(`[存储邮件] 执行发送人垃圾邮箱存储命令: ${spamCommand.substring(0, 200)}...`)
        
        exec(spamCommand, { maxBuffer: 50 * 1024 * 1024 }, (storeError, stdout, stderr) => {
          if (storeError) {
      ...
      exec(sentCommand, { maxBuffer: 50 * 1024 * 1024 }, (storeError, stdout, stderr) => {
        // 清理临时文件（如果存在）
        ...
        if (storeError) {
          ...
          fs.appendFileSync(path.join(LOG_DIR, 'mail-operations.log'), errorLog)
          storeResults.sentSuccess = false
        } else {
          console.log(`[存储邮件] 已发送文件夹存储成功: messageId=${sentMessageId}`)
          if (stdout) console.log(`[存储邮件] stdout: ${stdout}`)
          storeResults.sentSuccess = true
        }
        maybeComplete()
      })
    }
    ...
    transporter.sendMail(mailOptions, (error, info) => {
      ...
      if (error) {
        ...
        // 无论什么错误，都尝试存储到已发送和收件箱
        // 传入 onComplete 回调，等待存储完成后再返回，确保 stored 反映真实存储结果
        const sendErrorResponse = (stored) => {
          ...
          if (isRecipientError) {
            ...
            const is552SizeError = error.responseCode === 552 || 
              (error.message && (error.message.includes('552') || error.message.toLowerCase().includes('message file too big') || error.message.toLowerCase().includes('message size')))
            res.status(500).json({
              success: false,
              error: is552SizeError ? 'message_size_exceeded' : 'Failed to send email',
              message: error.message,
              stored: stored
            })
          }
        ...
        // 存储超时保护：大附件存储可能较慢，若超时则返回 stored: false
        const storeTimeout = setTimeout(() => {
          if (!responseSent) {
            console.error('[存储邮件] 存储超时，未能在限定时间内完成')
            sendErrorResponse(false)
          }
        }, 300000) // 5分钟
        
        storeMailToDatabase(messageId, true, error, (storeResults) => {
          clearTimeout(storeTimeout)
          // 收件箱跳过时 inboxSuccess 为 true；否则需两者都成功
          const stored = storeResults.inboxSuccess && storeResults.sentSuccess
          if (!stored) {
            console.error('[存储邮件] 存储未完全成功:', storeResults)
          }
          sendErrorResponse(stored)
        })
      } else {
        ...
        const sendLog = `[${new Date().toISOString()}] [MAIL_SEND_SUCCESS] User: ${user}, To: ${effectiveTo}, CC: ${ccValue || ''}, Subject: ${effectiveSubject}, MessageId: ${messageId}${spamInfo}, IP: ${clientIP}\n`
        ...
        console.log('Mail send completed successfully:', { messageId: info.messageId, fromAddr, to: effectiveTo, subject, isSpam: spamDetected })
        responseSent = true
        const successMsg = spamDetected ? '邮件已发送，但检测到垃圾邮件内容，已自动移动到垃圾邮箱' : 'Email sent successfully'
        const storageSkipMsg = storageRejectedCc.length > 0 ? `以下抄送人因邮箱存储已满未收到邮件: ${storageRejectedCc.join('、')}` : null
        res.json({
          success: true,
          messageId: info.messageId,
          message: storageSkipMsg ? `${successMsg}。${storageSkipMsg}` : successMsg,
          storageRejectedCc: storageRejectedCc.length > 0 ? storageRejectedCc : undefined,
...
app.get('/api/mail/list', auth, (req, res) => {
  try {
    ...
    const folderSafe = safeMailFolder(folder)
    const limitSafe = safeInteger(limit, 50, 500)
    const offsetSafe = safeInteger(offset, 0, 100000)
    const timestamp = new Date().toISOString()
    const clientIP = getRealClientIP(req)
    const username = req.headers.authorization ? Buffer.from(req.headers.authorization.split(' ')[1], 'base64').toString().split(':')[0] : 'unknown'
    const usernameEscaped = escapeSqlString(username)

    // 获取用户的真实邮箱地址
    let userEmail = username
    try {
      const mailDbPass = getMailDbPassword()
      const userEmailResult = execSync(`mysql -u mailuser -p"${mailDbPass.replace(/"/g, '\\"')}" maildb -s -r -e "SELECT email FROM mail_users WHERE username='${usernameEscaped}' LIMIT 1;"`, { encoding: 'utf8', timeout: 3000 }).trim()
      const lines = userEmailResult.split('\n').filter(line => line.trim() && line !== 'email')
      const email = lines[lines.length - 1]
      if (email && email.includes('@')) {
        userEmail = email
    ...
    // 记录邮件列表查询
    const logLine = `[${timestamp}] [MAIL_LIST] User: ${username}, Email: ${userEmail}, Folder: ${folderSafe}, Limit: ${limitSafe}, Offset: ${offsetSafe}, IP: ${clientIP}\n`
    fs.appendFileSync(path.join(LOG_DIR, 'mail-operations.log'), logLine)

    // 使用真实邮件数据库（参数经安全校验与 shell 转义）
    const mailDbScript = path.join(ROOT_DIR, 'backend', 'scripts', 'mail_db.sh')

    exec(`bash "${mailDbScript}" list ${escapeShellArg(userEmail)} ${escapeShellArg(folderSafe)} "${limitSafe}" "${offsetSafe}"`, (error, stdout, stderr) => {
      if (error) {
...
app.get('/api/mail/search', auth, (req, res) => {
  try {
    ...
    const folderSafe = safeMailFolder(folder)
    const limitSafe = safeInteger(limit, 100, 500)
    const offsetSafe = safeInteger(offset, 0, 100000)
    const timestamp = new Date().toISOString()
    const clientIP = getRealClientIP(req)
    const username = req.headers.authorization ? Buffer.from(req.headers.authorization.split(' ')[1], 'base64').toString().split(':')[0] : 'unknown'
    ...
    // 记录邮件搜索
    const logLine = `[${timestamp}] [MAIL_SEARCH] User: ${username}, Email: ${userEmail}, Query: ${q}, Folder: ${folderSafe}, Limit: ${limitSafe}, Offset: ${offsetSafe}, IP: ${clientIP}\n`
    fs.appendFileSync(path.join(LOG_DIR, 'mail-operations.log'), logLine)

    // 使用真实邮件数据库（参数经安全校验与 shell 转义）
    const mailDbScript = path.join(ROOT_DIR, 'backend', 'scripts', 'mail_db.sh')

    exec(`bash "${mailDbScript}" search ${escapeShellArg(userEmail)} ${escapeShellArg(String(q || ''))} ${escapeShellArg(folderSafe)} "${limitSafe}" "${offsetSafe}"`, (error, stdout, stderr) => {
      if (error) {
  ...
  try {
    ...
    const checkMailDatabase = () => {
      return new Promise((resolve) => {
        try {
          const mailDbScript = path.join(ROOT_DIR, 'backend', 'scripts', 'mail_db.sh')
          exec(`bash "${mailDbScript}" stats ${escapeShellArg(user)}`, (error, stdout) => {
...
// 获取邮件端口状态（25/587/993/995 各端口启用情况）
app.get('/api/mail/port-status', auth, (req, res) => {
  try {
    const scriptPath = path.join(ROOT_DIR, 'backend', 'scripts', 'mail_port_control.sh')
    // 通过 sed 去除 CRLF，避免 Windows 换行符导致脚本执行失败
    const out = execSync(`sed 's/\\r$//' "${scriptPath}" | sudo bash -s status`, { encoding: 'utf8', timeout: 5000 })
    let data
    try {
      data = JSON.parse(out.trim())
    } catch (e) {
      data = { postfix: { 25: true, 587: true }, dovecot: { 993: true, 995: true } }
    }
...
// 控制单个邮件端口启用/禁用
app.post('/api/mail/port-control', auth, csrfProtection, (req, res) => {
  try {
    const { port, enabled } = req.body || {}
    const validPorts = ['25', '587', '993', '995']
    if (!port || !validPorts.includes(String(port))) {
      return res.status(400).json({
        success: false,
        error: 'Invalid port',
        message: '端口需为 25、587、993 或 995'
      })
    }
    const action = enabled ? 'enable' : 'disable'
    const scriptPath = path.join(ROOT_DIR, 'backend', 'scripts', 'mail_port_control.sh')
    // 通过 sed 去除 CRLF，避免 Windows 换行符导致脚本执行失败
    execSync(`sed 's/\\r$//' "${scriptPath}" | sudo bash -s ${port} ${action}`, { encoding: 'utf8', timeout: 15000 })
    res.json({
      success: true,
      message: `端口 ${port} 已${enabled ? '启用' : '禁用'}`
    })
  } catch (error) {
    console.error('Port control error:', error)
    res.status(500).json({
      success: false,
      error: 'Failed to control port',
      message: error.message || '执行失败'
    })
  }
})

// 控制邮件服务（Postfix/Dovecot）启动或停止
app.post('/api/mail/service-control', auth, csrfProtection, (req, res) => {
  try {
    ...
    if (!service || !action) {
      return res.status(400).json({
        ...
        message: '需要提供 service 和 action 参数'
      })
    }
    if (!['postfix', 'dovecot'].includes(service)) {
      return res.status(400).json({
        success: false,
        error: 'Invalid service',
        message: '仅支持 postfix 或 dovecot'
      })
    }
    ...
    res.json({
      success: true,
      message: `${service} 已${action === 'start' ? '启动' : '停止'}`
    })
  } catch (error) {
    ...
    res.status(500).json({
      ...
      message: error.message || '执行失败，请确认 xm 用户具有 sudo systemctl 权限'
    })
  }
})
...
app.get('/api/mail/stats', auth, (req, res) => {
  try {
    ...
    try {
      const mailDbPass = getMailDbPassword()
      const userEmailResult = execSync(`mysql -u mailuser -p"${mailDbPass.replace(/"/g, '\\"')}" maildb -s -r -e "SELECT email FROM mail_users WHERE username='${escapeSqlString(username)}' LIMIT 1;"`, { encoding: 'utf8', timeout: 3000 }).trim()
      const lines = userEmailResult.split('\n').filter(line => line.trim() && line !== 'email')
      const email = lines[lines.length - 1]
      if (email && email.includes('@')) {
        userEmail = email
    ...
    exec(`bash "${mailDbScript}" stats ${escapeShellArg(userEmail)}`, (error, stdout, stderr) => {
      if (error) {
...
// 获取当前用户邮件存储使用情况（用于 Mail 页面可视化）
app.get('/api/mail/storage-usage', auth, (req, res) => {
  try {
    const username = req.headers.authorization ? Buffer.from(req.headers.authorization.split(' ')[1], 'base64').toString().split(':')[0] : 'unknown'
    let userEmail = username
    try {
      ...
      const lines = userEmailResult.split('\n').filter(line => line.trim() && line !== 'email')
      const email = lines[lines.length - 1]
      if (email && email.includes('@')) userEmail = email
    } catch (e) { /* use username */ }
    const mailDbScript = path.join(ROOT_DIR, 'backend', 'scripts', 'mail_db.sh')
    const statsOutput = execSync(`bash "${mailDbScript}" stats ${escapeShellArg(userEmail)}`, { encoding: 'utf8', timeout: 10000 })
    const stats = JSON.parse(statsOutput)
    // 单向收到计算：同一封邮件在收件箱+已发送等多处只计一次（使用 deduplicated_size）
    const usedBytes = Number(stats.deduplicated_size) >= 0 ? Number(stats.deduplicated_size) : [stats.inbox, stats.sent, stats.drafts, stats.trash, stats.spam]
      .filter(Boolean)
      .reduce((sum, f) => sum + (Number(f.size) || 0), 0)
    let limitMB = 1024
    ...
    res.json({
      ...
      byFolder: {
        inbox: stats.inbox?.size || 0,
        sent: stats.sent?.size || 0,
        drafts: stats.drafts?.size || 0,
        trash: stats.trash?.size || 0,
        spam: stats.spam?.size || 0
      }
    })
  } catch (error) {
...
// 获取所有用户邮件存储使用情况（仅管理员）
app.get('/api/mail/storage-usage/all', auth, (req, res) => {
  try {
    const username = req.headers.authorization ? Buffer.from(req.headers.authorization.split(' ')[1], 'base64').toString().split(':')[0] : ''
    if (!username || username.toLowerCase() !== 'xm') {
      return res.status(403).json({ success: false, error: '需要管理员权限' })
    }
    const mailDbScript = path.join(ROOT_DIR, 'backend', 'scripts', 'mail_db.sh')
    const listOutput = execSync(`bash "${mailDbScript}" list_users`, { encoding: 'utf8', timeout: 5000 })
    ...
    for (const u of users) {
      const email = u.email || `${u.username}@localhost`
      try {
        const statsOutput = execSync(`bash "${mailDbScript}" stats ${escapeShellArg(email)}`, { encoding: 'utf8', timeout: 5000 })
        const stats = JSON.parse(statsOutput)
        // 单向收到计算：同一封邮件只计一次
        const usedBytes = Number(stats.deduplicated_size) >= 0 ? Number(stats.deduplicated_size) : [stats.inbox, stats.sent, stats.drafts, stats.trash, stats.spam]
          .filter(Boolean)
        ...
        result.push({ username: u.username, email: u.email, displayName: u.display_name || u.username, usedBytes: 0, usedMB: 0, limitBytes, limitMB, percentage: 0, error: e.message })
      }
    }
    res.json({ success: true, users: result, limitMB })
  } catch (error) {
    console.error('Storage usage all error:', error)
    res.status(500).json({ success: false, error: error.message })
  }
})
...
app.get('/api/mail/:id', auth, (req, res) => {
  try {
    ...
    exec(`bash "${mailDbScript}" detail ${escapeShellArg(id)} ${escapeShellArg(userEmail)}`, (error, stdout, stderr) => {
      if (error) {
...
// 删除邮件
app.delete('/api/mail/:id', auth, csrfProtection, (req, res) => {
  try {
    const { id } = req.params
    const timestamp = new Date().toISOString()
    const clientIP = getRealClientIP(req)
    ...
    exec(`bash "${mailDbScript}" delete ${escapeShellArg(id)} ${escapeShellArg(user)}`, (error, stdout, stderr) => {
      if (error) {
...
// 还原邮件（从已删除文件夹恢复到原文件夹）
app.post('/api/mail/:id/restore', auth, csrfProtection, (req, res) => {
  try {
    const { id } = req.params
    const timestamp = new Date().toISOString()
    const clientIP = getRealClientIP(req)
    ...
    exec(`bash "${mailDbScript}" restore ${escapeShellArg(id)} ${escapeShellArg(user)}`, (error, stdout, stderr) => {
      if (error) {
...
// 彻底删除邮件（硬删除）
app.delete('/api/mail/:id/permanent', auth, csrfProtection, (req, res) => {
  try {
    const { id } = req.params
    const timestamp = new Date().toISOString()
    const clientIP = getRealClientIP(req)
    ...
    exec(`bash "${mailDbScript}" hard-delete ${escapeShellArg(id)} ${escapeShellArg(user)}`, (error, stdout, stderr) => {
      if (error) {
        ...
        try {
          ...
          if (folders.length > 0) {
            ...
            exec(debugQuery, (debugError, debugStdout, debugStderr) => {
              ...
              const userQuery = `mysql -u mailuser -p"${getMailDbPassword().replace(/"/g, '\\"')}" maildb -s -r -e "SELECT id, username, email FROM mail_users WHERE username='${escapeSqlString(username)}' OR email LIKE '%${escapeSqlString(username)}%' LIMIT 5;" 2>&1`
...
app.get('/api/mail/folders/:id/stats', auth, (req, res) => {
  try {
    ...
    exec(`bash "${mailDbScript}" folder-stats ${escapeShellArg(id)} ${escapeShellArg(user)}`, (error, stdout, stderr) => {
      if (error) {
...
app.get('/api/mail/stats/sending-trends', auth, (req, res) => {
  try {
    ...
    const periodSafe = ['hour', 'day', 'week'].includes(String(period)) ? period : 'day'
    const mailDbScript = path.join(ROOT_DIR, 'backend', 'scripts', 'mail_db.sh')
    
    exec(`bash "${mailDbScript}" sending-trends ${escapeShellArg(periodSafe)}`, (error, stdout, stderr) => {
      if (error) {
...
app.get('/api/mail/stats/frequency-analysis', auth, (req, res) => {
  try {
    ...
    const groupBySafe = ['user', 'day'].includes(String(group_by)) ? group_by : 'user'
    const mailDbScript = path.join(ROOT_DIR, 'backend', 'scripts', 'mail_db.sh')
    
    exec(`bash "${mailDbScript}" frequency-analysis ${escapeShellArg(groupBySafe)}`, (error, stdout, stderr) => {
      if (error) {
...
// 创建自定义文件夹
app.post('/api/mail/folders', auth, csrfProtection, (req, res) => {
  try {
    ...
    getMailUserIdFromUsername(username, (err, userId) => {
      ...
      exec(`bash "${mailDbScript}" add-folder ${escapeShellArg(name)} ${escapeShellArg(display_name || name)} "${userIdParam}"`, (error, stdout, stderr) => {
...
// 更新自定义文件夹
app.put('/api/mail/folders/:id', auth, csrfProtection, (req, res) => {
  try {
    const { id } = req.params
    const { name, display_name } = req.body
    ...
    getUserIdFromUsername(username, (err, userId) => {
      ...
      exec(`bash "${mailDbScript}" update-folder ${escapeShellArg(id)} ${escapeShellArg(nameParam)} ${escapeShellArg(displayNameParam)} "${userIdParam}"`, (error, stdout, stderr) => {
      if (error) {
...
// 删除自定义文件夹
app.delete('/api/mail/folders/:id', auth, csrfProtection, (req, res) => {
  try {
    const { id } = req.params
    const mailDbScript = path.join(ROOT_DIR, 'backend', 'scripts', 'mail_db.sh')
    const username = getUsernameFromRequest(req)
    ...
    getUserIdFromUsername(username, (err, userId) => {
      ...
      exec(`bash "${mailDbScript}" delete-folder ${escapeShellArg(id)} "${userIdParam}"`, (error, stdout, stderr) => {
      if (error) {
...
// 为邮件添加标签
app.post('/api/mail/:id/labels', auth, csrfProtection, (req, res) => {
  try {
    const { id } = req.params
    ...
    exec(`bash "${mailDbScript}" add-label ${escapeShellArg(id)} ${escapeShellArg(labelId)}`, (error, stdout, stderr) => {
      if (error) {
...
// 移除邮件标签
app.delete('/api/mail/:id/labels/:labelId', auth, csrfProtection, (req, res) => {
  try {
    const { id, labelId } = req.params
    const mailDbScript = path.join(ROOT_DIR, 'backend', 'scripts', 'mail_db.sh')
    
    exec(`bash "${mailDbScript}" remove-label ${escapeShellArg(id)} ${escapeShellArg(labelId)}`, (error, stdout, stderr) => {
      if (error) {
...
// 移动邮件到文件夹
app.put('/api/mail/:id/move', auth, csrfProtection, (req, res) => {
  try {
    const { id } = req.params
    ...
    exec(`bash "${mailDbScript}" move ${escapeShellArg(id)} ${escapeShellArg(folder)} ${escapeShellArg(user)}`, (error, stdout, stderr) => {
      if (error) {
...
// 批量移动邮件
app.post('/api/mail/bulk/move', auth, csrfProtection, (req, res) => {
  try {
    const { ids = [], folder } = req.body
    const user = req.headers.authorization ? Buffer.from(req.headers.authorization.split(' ')[1], 'base64').toString().split(':')[0] : 'unknown'
    if (!Array.isArray(ids) || ids.length === 0 || !folder) {
      return res.status(400).json({ success: false, error: 'ids (array) and folder are required' })
    }
    const mailDbScript = path.join(ROOT_DIR, 'backend', 'scripts', 'mail_db.sh')
    let completed = 0
    let failed = 0
    const run = (i) => {
      if (i >= ids.length) {
        return res.json({ success: true, message: `Moved ${completed} email(s)`, failed })
      }
      const id = String(ids[i])
      exec(`bash "${mailDbScript}" move ${escapeShellArg(id)} ${escapeShellArg(folder)} ${escapeShellArg(user)}`, (error) => {
        if (error) failed++
        else completed++
        run(i + 1)
      })
...
// 批量删除邮件（软删除）
app.post('/api/mail/bulk/delete', auth, csrfProtection, (req, res) => {
  try {
    const { ids = [] } = req.body
    const user = req.headers.authorization ? Buffer.from(req.headers.authorization.split(' ')[1], 'base64').toString().split(':')[0] : 'unknown'
    ...
    const run = (i) => {
      if (i >= ids.length) {
        return res.json({ success: true, message: `Deleted ${completed} email(s)`, failed })
      }
      const id = String(ids[i])
      exec(`bash "${mailDbScript}" delete ${escapeShellArg(id)} ${escapeShellArg(user)}`, (error) => {
        if (error) failed++
        else completed++
        run(i + 1)
      })
...
// 批量彻底删除（仅已删除文件夹）
app.post('/api/mail/bulk/permanent', auth, csrfProtection, (req, res) => {
  try {
    const { ids = [] } = req.body
    const user = req.headers.authorization ? Buffer.from(req.headers.authorization.split(' ')[1], 'base64').toString().split(':')[0] : 'unknown'
    if (!Array.isArray(ids) || ids.length === 0) {
    ...
    const run = (i) => {
      if (i >= ids.length) {
        return res.json({ success: true, message: `Permanently deleted ${completed} email(s)`, failed })
      }
      const id = String(ids[i])
      exec(`bash "${mailDbScript}" hard-delete ${escapeShellArg(id)} ${escapeShellArg(user)}`, (error) => {
        if (error) failed++
        else completed++
        run(i + 1)
      })
...
// 置顶/取消置顶邮件
app.put('/api/mail/:id/pin', auth, csrfProtection, (req, res) => {
  try {
    const { id } = req.params
    ...
    exec(`bash "${mailDbScript}" pin ${escapeShellArg(id)} ${escapeShellArg(pinVal)} ${escapeShellArg(user)}`, (error, stdout, stderr) => {
      if (error) {
        console.error('Pin email error:', error)
        res.status(500).json({
          success: false,
          error: 'Failed to pin email',
          message: error.message
        })
        return
      }
      ...
      res.json({
        success: true,
        message: pinned ? 'Email pinned' : 'Email unpinned'
      })
    })
  } catch (error) {
    console.error('Pin email error:', error)
...
// 标记邮件为已读
app.put('/api/mail/:id/read', auth, csrfProtection, (req, res) => {
  try {
    const { id } = req.params
    const timestamp = new Date().toISOString()
    const clientIP = getRealClientIP(req)
    ...
    const mailDbScript = path.join(ROOT_DIR, 'backend', 'scripts', 'mail_db.sh')
    exec(`bash "${mailDbScript}" mark_read ${escapeShellArg(id)} ${escapeShellArg(userEmail)}`, (error, stdout, stderr) => {
      if (error) {
...
// 终端命令执行API端点
app.post('/api/terminal', auth, csrfProtection, (req, res) => {
  const user = req.headers.authorization ? 
    Buffer.from(req.headers.authorization.split(' ')[1], 'base64').toString().split(':')[0] : 
    'unknown'
  const timestamp = new Date().toISOString()
...
// 添加域名
app.post('/api/system/domains', auth, csrfProtection, (req, res) => {
  try {
    ...
    // 调用mail_db.sh添加域名
    const mailDbScript = path.join(__dirname, '..', 'scripts', 'mail_db.sh')
    exec(`bash "${mailDbScript}" add_domain ${escapeShellArg(domain.trim())}`, (error, stdout, stderr) => {
      // 检查stdout中是否包含成功消息
      const stdoutStr = stdout ? stdout.toString() : ''
...
// 删除域名
app.delete('/api/system/domains/:id', auth, csrfProtection, (req, res) => {
  try {
    const { id } = req.params
    const timestamp = new Date().toISOString()
    const clientIP = getRealClientIP(req)
    ...
    exec(`bash "${mailDbScript}" list_domains`, (listError, listStdout) => {
      ...
      try {
        const domains = JSON.parse(listStdout.toString().trim())
        const domainToDelete = domains.find(d => String(d.id) === String(id))
        ...
        // 调用mail_db.sh删除域名
        exec(`bash "${mailDbScript}" delete_domain ${escapeShellArg(id)}`, (error, stdout, stderr) => {
          if (error) {
            const scriptOutput = (stdout || '').toString().trim() || (stderr || '').toString().trim()
            const errMsg = scriptOutput || error.message
            console.error('Failed to delete domain:', error, 'stdout:', stdout, 'stderr:', stderr)
            // 域名下有用户时返回 400，其他情况返回 500
            const statusCode = /该域名下还有|无法删除/.test(errMsg) ? 400 : 500
            res.status(statusCode).json({
              success: false,
              error: 'Failed to delete domain',
              message: errMsg || '删除域名失败'
            })
          } else {
...
// 更新垃圾邮件过滤配置API
app.post('/api/spam-filter/config', auth, csrfProtection, (req, res) => {
  try {
    const clientIP = getRealClientIP(req)
...
// 垃圾邮件检测API（从数据库加载配置）
app.post('/api/spam-filter/check', auth, csrfProtection, (req, res) => {
  try {
    const clientIP = getRealClientIP(req)
...
// 大附件下载（file_path 存储的附件，支持 1GB）
app.get('/api/mail/attachment/:emailId/:attachmentId', auth, (req, res) => {
  try {
    ...
    const mailDbScript = path.join(ROOT_DIR, 'backend', 'scripts', 'mail_db.sh')
    const detailOut = execSync(`bash "${mailDbScript}" detail ${escapeShellArg(emailId)} ${escapeShellArg(userEmail)}`, { encoding: 'utf8', timeout: 10000 })
    const jsonMatch = detailOut.match(/\[[\s\S]*\]/) || detailOut.match(/\{[\s\S]*\}/)
    if (!jsonMatch) {
      return res.status(404).json({ success: false, error: '邮件未找到' })
    }
    const data = JSON.parse(jsonMatch[0])
    const email = Array.isArray(data) ? data[0] : data
    if (!email || !email.id) return res.status(404).json({ success: false, error: '邮件未找到' })
    const attachments = (email.attachments && typeof email.attachments === 'string' ? (() => { try { return JSON.parse(email.attachments) } catch { return [] } })() : email.attachments) || []
    const att = attachments.find(a => String(a.id) === String(attachmentId))
    if (!att || !att.file_path || !fs.existsSync(att.file_path)) {
      return res.status(404).json({ success: false, error: '附件未找到或已过期' })
    }
    ...
    console.error('[MAIL_ATTACHMENT] 下载失败:', err)
    res.status(500).json({ success: false, error: err.message })
  }
})

// 获取邮件详情（放在最后以避免与具体路由冲突）
// 查询参数 prefetch=1 或 mark_read=0：预加载时不标记已读，仅用户实际点击打开时才标记
app.get('/api/mail/:id', auth, (req, res) => {
  try {
    const { id } = req.params
    const prefetch = req.query.prefetch === '1' || req.query.prefetch === 'true'
    const skipMarkRead = prefetch ? '1' : '0'
    const timestamp = new Date().toISOString()
    const clientIP = getRealClientIP(req)
    const username = req.headers.authorization ? Buffer.from(req.headers.authorization.split(' ')[1], 'base64').toString().split(':')[0] : 'unknown'
    ...
    exec(`bash "${mailDbScript}" detail ${escapeShellArg(id)} ${escapeShellArg(userEmail)} ${escapeShellArg(skipMarkRead)}`, (error, stdout, stderr) => {
      if (error) {
        console.error('Mail detail error:', error)
        console.error('Mail detail stderr:', stderr)
        console.error('Mail detail stdout:', stdout)
...
server.listen(PORT, () => {
  ...
  // 启动时同步 Postfix 邮件大小限制（确保配置与 system-settings 一致）
  setTimeout(() => {
    try {
      const settingsFile = path.join(ROOT_DIR, 'config', 'system-settings.json')
      if (fs.existsSync(settingsFile)) {
        const settings = JSON.parse(fs.readFileSync(settingsFile, 'utf8'))
        const maxMB = settings.mail?.maxMessageSize
        if (maxMB && Number(maxMB) > 0) {
          const limitBytes = Number(maxMB) * 1024 * 1024
          execSync(`sudo postconf -e "message_size_limit = ${limitBytes}"`, { timeout: 8000 })
          console.log(`[启动] Postfix message_size_limit 已同步为 ${maxMB}MB`)
        }
      }
    } catch (err) {
      console.warn('[启动] 同步 Postfix 邮件大小限制失败:', err.message)
    }
  }, 1000)
