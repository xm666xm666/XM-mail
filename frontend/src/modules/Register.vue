<template>
  <div class="register-page">
    ...
    <div class="register-container">
      <div class="register-card">
        <div class="card-header">
          ...
          <p class="card-subtitle">创建新的管理账户</p>
        </div>
    ...
    <CopyrightFooter variant="auth" :icp="icpSettings" :version="currentVersion" />
  </div>
</template>
  ...
  try {
    ...
    try {
      const settingsRes = await fetch('/api/system-settings', {
        headers: {
          // 未登录时 fallback 仅用于拉取注册页所需系统设置；部署后请修改 xm 密码
    ...
    // 注册操作需要认证（用于记录操作日志）
    // 安全说明：未登录访问注册页时使用 xm 默认凭证作为 fallback，仅用于调用注册 API。
    ...
    const res = await fetch('/api/ops', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Authorization': `Basic ${token}`, ...csrfHeader() },
      body: JSON.stringify({ 
        action: 'app-register', 
        params: { 
    ...
    if (!res.ok) {
      let errorMessage = `注册失败 (HTTP ${res.status})`
      try {
...
.register-card {
  ...
  box-shadow: 0 24px 64px rgba(0, 0, 0, 0.35), 0 0 0 1px rgba(6, 182, 212, 0.05);
