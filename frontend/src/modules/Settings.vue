  ...
  <Layout>
    <div class="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50 to-purple-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 py-4 sm:py-8">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <div class="space-y-4">
                <div>
                  ...
                  <input v-model.number="systemSettings.security.sessionTimeout" 
                         @blur="onSecurityFieldBlur('sessionTimeout')"
                         type="number" 
                         min="5" 
                         max="1440" 
                ...
                <div>
                  ...
                  <input v-model.number="systemSettings.security.maxLoginAttempts" 
                         @blur="onSecurityFieldBlur('maxLoginAttempts')"
                         type="number" 
                         min="3" 
                         max="20" 
                         class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent">
                ...
                <div>
                  ...
                  <input v-model.number="systemSettings.security.passwordMinLength" 
                         @blur="onSecurityFieldBlur('passwordMinLength')"
                         type="number" 
                         min="6" 
                         max="32" 
                         class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent">
            ...
            <div class="space-y-4">
              <div>
                ...
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                  ...
                  <span v-else>设置邮件列表每页显示的邮件数量（5-25 条）</span>
                </p>
              </div>
              ...
              <div>
                ...
                <div class="relative">
                  <input :value="systemSettings.mail.maxMailboxSize" @input="onMailNumericInput($event, 'maxMailboxSize')" @blur="onMailFieldBlur('maxMailboxSize')" :disabled="mailSettingsSaving" type="text" inputmode="numeric" pattern="[0-9]*" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent disabled:bg-gray-100 dark:disabled:bg-gray-600 disabled:cursor-not-allowed">
                  <div v-if="mailSettingsSaving" class="absolute right-3 top-1/2 transform -translate-y-1/2">
                    <svg class="animate-spin h-4 w-4 text-green-600 dark:text-green-400" fill="none" viewBox="0 0 24 24">
                      <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                      <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                ...
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                  ...
                  <span v-else>设置单个用户邮箱的最大存储空间（100-50000 MB）</span>
                </p>
              </div>
              ...
              <div>
                ...
                <div class="relative">
                  <input :value="systemSettings.mail.maxMessageSize" @input="onMailNumericInput($event, 'maxMessageSize')" @blur="onMailFieldBlur('maxMessageSize')" :disabled="mailSettingsSaving" type="text" inputmode="numeric" pattern="[0-9]*" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent disabled:bg-gray-100 dark:disabled:bg-gray-600 disabled:cursor-not-allowed">
                  <div v-if="mailSettingsSaving" class="absolute right-3 top-1/2 transform -translate-y-1/2">
                    <svg class="animate-spin h-4 w-4 text-green-600 dark:text-green-400" fill="none" viewBox="0 0 24 24">
                      <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                      <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                ...
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                  <span v-if="mailSettingsSaving" class="text-green-600 dark:text-green-400">正在应用设置...</span>
                  <span v-else>设置单封邮件的最大大小限制（1-1024 MB）</span>
                </p>
              </div>
        ...
        <!-- 删除域名确认弹窗 - Teleport 到 body 避免被父级 overflow 裁剪，高 z-index 避免被滚动条遮挡 -->
        <Teleport to="body">
          <div v-if="showDeleteConfirm" class="fixed inset-0 z-[9999] flex items-center justify-center p-4">
            <div class="fixed inset-0 bg-gray-500/75 dark:bg-gray-900/80 transition-opacity" @click="cancelDeleteDomain" aria-hidden="true"></div>
            <div class="relative z-10 inline-block w-full max-w-lg rounded-lg bg-white dark:bg-gray-800 text-left shadow-xl border border-transparent dark:border-gray-600">
...
// 安全设置数字字段：删除为空时恢复默认值
const SECURITY_DEFAULTS: Record<string, number> = { sessionTimeout: 30, maxLoginAttempts: 5, passwordMinLength: 8 }
const SECURITY_RANGES: Record<string, [number, number]> = { sessionTimeout: [5, 1440], maxLoginAttempts: [3, 20], passwordMinLength: [6, 32] }
const onSecurityFieldBlur = (field: string) => {
  const defaultVal = SECURITY_DEFAULTS[field]
  const [min, max] = SECURITY_RANGES[field] || [0, Infinity]
  const val = systemSettings.value.security[field as keyof typeof systemSettings.value.security]
  const numVal = typeof val === 'number' ? val : parseInt(String(val || '').trim(), 10)
  const isValid = val !== '' && val !== null && val !== undefined && !isNaN(numVal) && numVal >= min && numVal <= max
  if (!isValid) {
    systemSettings.value.security[field as keyof typeof systemSettings.value.security] = defaultVal as any
  } else {
    systemSettings.value.security[field as keyof typeof systemSettings.value.security] = numVal as any
  }
  saveSystemSettings()
}

// 邮件设置数字字段：删除为空时恢复默认值
const MAIL_DEFAULTS: Record<string, number> = { maxMailboxSize: 1024, maxMessageSize: 500 }
const MAIL_RANGES: Record<string, [number, number]> = { maxMailboxSize: [100, 50000], maxMessageSize: [1, 1024] }
// 邮件数字输入：仅允许数字，避免 type="number" 的输入限制
const onMailNumericInput = (e: Event, field: string) => {
  const raw = (e.target as HTMLInputElement).value
  const digits = raw.replace(/\D/g, '')
  const num = digits === '' ? '' : parseInt(digits, 10)
  ;(systemSettings.value.mail as any)[field] = num
}
const onMailFieldBlur = (field: string) => {
  const defaultVal = MAIL_DEFAULTS[field]
  const [min, max] = MAIL_RANGES[field] || [0, Infinity]
  const val = systemSettings.value.mail[field as keyof typeof systemSettings.value.mail]
  const numVal = typeof val === 'number' ? val : parseInt(String(val || '').trim(), 10)
  const isValid = val !== '' && val !== null && val !== undefined && !isNaN(numVal) && numVal >= min && numVal <= max
  if (!isValid) {
    systemSettings.value.mail[field as keyof typeof systemSettings.value.mail] = defaultVal as any
  } else {
    systemSettings.value.mail[field as keyof typeof systemSettings.value.mail] = numVal as any
  }
  saveMailSettings()
}
...
const executeSaveSystemSettings = async (silent: boolean = false) => {
  ...
  try {
    // 安全设置：删除为空时恢复默认值（保存前兜底）
    if (systemSettings.value.security) {
      for (const field of Object.keys(SECURITY_DEFAULTS)) {
        const val = systemSettings.value.security[field as keyof typeof systemSettings.value.security]
        const [min, max] = SECURITY_RANGES[field] || [0, Infinity]
        const numVal = typeof val === 'number' ? val : parseInt(String(val), 10)
        if (val === '' || val === null || val === undefined || isNaN(numVal) || numVal < min || numVal > max) {
          systemSettings.value.security[field as keyof typeof systemSettings.value.security] = SECURITY_DEFAULTS[field] as any
        }
      }
    }
    // 验证安全设置
    ...
    // 邮件设置：删除为空时恢复默认值（保存前兜底，保存设置按钮会保存完整配置）
    if (systemSettings.value.mail) {
      for (const field of Object.keys(MAIL_DEFAULTS)) {
        const val = systemSettings.value.mail[field as keyof typeof systemSettings.value.mail]
        const [min, max] = MAIL_RANGES[field] || [0, Infinity]
        const numVal = typeof val === 'number' ? val : parseInt(String(val), 10)
        if (val === '' || val === null || val === undefined || isNaN(numVal) || numVal < min || numVal > max) {
          systemSettings.value.mail[field as keyof typeof systemSettings.value.mail] = MAIL_DEFAULTS[field] as any
        }
      }
    }
    ...
    if (response.ok) {
      ...
      try {
        ...
        throw new Error('保存失败：服务器返回格式错误')
      }
      if (data.success) {
        // 保存分页设置到localStorage
      ...
      try {
        ...
        errMsg = response.status === 401 ? '未登录或会话已过期，请重新登录' : `保存失败 (${response.status})`
      }
  ...
  try {
    ...
    if (response.ok) {
      ...
      try {
        ...
        throw new Error('获取设置失败：服务器返回格式错误')
      }
      if (data.success) {
        ...
        systemSettings.value = {
          ...
          mail: {
            ...
            maxMessageSize: backendSettings.mail?.maxMessageSize ?? 500,
