<template>
  <div class="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-200">
    <!-- 左侧导航栏 - 仅管理员可见 -->
    <div v-if="isAdmin" class="fixed inset-y-0 left-0 z-50 w-full sm:w-64 flex flex-col bg-white/95 dark:bg-gray-800/95 backdrop-blur-xl border-r border-gray-200/80 dark:border-gray-700/80 shadow-xl shadow-gray-200/50 dark:shadow-black/30 transform transition-all duration-300 ease-out" :class="{ '-translate-x-full': !sidebarOpen, 'translate-x-0': sidebarOpen }" :style="{ opacity: isInitialized ? 1 : 0 }">
      <!-- 导航栏头部 -->
      <div class="relative flex items-center justify-between h-14 sm:h-16 px-4 sm:px-5 shrink-0 overflow-hidden border-b border-gray-200/80 dark:border-gray-700/80">
        <!-- 背景装饰 -->
        <div class="absolute inset-0 bg-gradient-to-br from-slate-50 via-blue-50/40 to-indigo-50/30 dark:from-gray-800 dark:via-blue-950/20 dark:to-indigo-950/10"></div>
        <div class="absolute inset-0 bg-[radial-gradient(ellipse_80%_50%_at_0%_50%,rgba(99,102,241,0.08),transparent)] dark:bg-[radial-gradient(ellipse_80%_50%_at_0%_50%,rgba(99,102,241,0.12),transparent)]"></div>
        <div class="absolute top-0 right-0 w-24 h-24 bg-gradient-to-bl from-blue-400/10 to-transparent rounded-bl-full dark:from-indigo-500/10"></div>
        <div class="relative flex items-center justify-between w-full min-w-0">
          <div class="flex items-center gap-2.5 min-w-0">
            <!-- Logo 图标 -->
            <div class="relative shrink-0 group/logo">
              <div class="absolute -inset-px rounded-lg bg-gradient-to-br from-blue-500 via-indigo-500 to-violet-600 opacity-60 blur-[2px] group-hover/logo:opacity-80 transition-opacity"></div>
              <div class="relative h-8 w-8 sm:h-9 sm:w-9 rounded-lg flex items-center justify-center bg-gradient-to-br from-blue-500 via-indigo-500 to-violet-600 shadow shadow-indigo-500/15 ring-1 ring-white/30 dark:ring-white/10 overflow-hidden">
                <img src="/favicon.ico" alt="XM邮件系统" class="h-7 w-7 sm:h-8 sm:w-8 object-contain drop-shadow-sm" />
              </div>
            </div>
            <!-- 品牌名称 -->
            <div class="min-w-0">
              <span class="block text-base sm:text-lg font-bold tracking-tight text-gray-900 dark:text-white truncate drop-shadow-sm">XM邮件系统</span>
              <span class="block text-[10px] sm:text-xs font-medium text-blue-600/80 dark:text-blue-400/80 tracking-wider truncate">邮件管理平台</span>
            </div>
          </div>
          <!-- 收起按钮 -->
          <button @click="toggleSidebar" class="relative p-2 rounded-xl text-gray-500 hover:text-gray-800 hover:bg-white/80 dark:text-gray-400 dark:hover:text-gray-100 dark:hover:bg-gray-700/80 transition-all duration-200 shrink-0 hover:scale-105 active:scale-95 shadow-sm hover:shadow" aria-label="收起侧栏">
      ...
      <!-- 导航菜单 -->
      <nav class="flex-1 overflow-y-auto overflow-x-hidden py-4 px-3 pb-48 scrollbar-thin scrollbar-thumb-gray-300/80 dark:scrollbar-thumb-gray-600 scrollbar-track-transparent">
        <div class="space-y-1">
          <!-- 仪表板 -->
          <router-link 
            to="/dashboard" 
            @click="() => { userLogger.logButtonClick('仪表板', '左侧导航'); onNavLinkClick() }"
            class="group flex items-center gap-3 px-3 py-2.5 text-sm font-medium rounded-xl transition-all duration-200"
            :class="$route.path === '/dashboard' ? 'bg-gradient-to-r from-blue-500 to-indigo-600 text-white shadow-lg shadow-blue-500/30' : 'text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700/80'"
          >
            <div class="flex-shrink-0 w-9 h-9 rounded-lg flex items-center justify-center transition-colors" :class="$route.path === '/dashboard' ? 'bg-white/20' : 'bg-gray-100 dark:bg-gray-900/50 group-hover:bg-blue-100 dark:group-hover:bg-blue-900/30'">
              <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2H5a2 2 0 00-2-2z"></path>
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 5a2 2 0 012-2h4a2 2 0 012 2v6H8V5z"></path>
              </svg>
            </div>
            <span class="flex-1 min-w-0">仪表板</span>
            <div v-if="$route.path === '/dashboard'" class="w-2 h-2 bg-white rounded-full animate-pulse shrink-0"></div>
          </router-link>

          <!-- 邮件管理 -->
          <router-link 
            to="/mail" 
            @click="() => { userLogger.logButtonClick('邮件管理', '左侧导航'); onNavLinkClick() }"
            class="group flex items-center gap-3 px-3 py-2.5 text-sm font-medium rounded-xl transition-all duration-200"
            :class="$route.path === '/mail' ? 'bg-gradient-to-r from-blue-500 to-indigo-600 text-white shadow-lg shadow-blue-500/30' : 'text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700/80'"
          >
            <div class="flex-shrink-0 w-9 h-9 rounded-lg flex items-center justify-center transition-colors" :class="$route.path === '/mail' ? 'bg-white/20' : 'bg-gray-100 dark:bg-gray-900/50 group-hover:bg-blue-100 dark:group-hover:bg-blue-900/30'">
              <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 4.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path>
              </svg>
            </div>
            <span class="flex-1 min-w-0">邮件管理</span>
            <div v-if="$route.path === '/mail'" class="w-2 h-2 bg-white rounded-full animate-pulse shrink-0"></div>
          </router-link>

          <!-- 系统管理 -->
          <div class="pt-6 mt-4 border-t border-gray-200/80 dark:border-gray-700/80">
            <div class="flex items-center gap-2 px-3 mb-3">
              <div class="w-1 h-4 rounded-full bg-gradient-to-b from-blue-500 to-indigo-500"></div>
              <h3 class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-widest">系统管理</h3>
            </div>
            <div class="space-y-1">
              <button 
                @click="() => { userLogger.logButtonClick('系统状态', '左侧导航'); showSystemStatus(); onNavItemClickAfterOpen() }"
                class="group flex items-center gap-3 w-full px-3 py-2.5 text-sm font-medium rounded-xl text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700/80 transition-all duration-200"
                title="系统状态"
              >
                <div class="flex-shrink-0 w-9 h-9 rounded-lg flex items-center justify-center bg-gray-100 dark:bg-gray-900/50 group-hover:bg-blue-100 dark:group-hover:bg-blue-900/30 text-gray-600 dark:text-gray-400 group-hover:text-blue-600 dark:group-hover:text-blue-400">
                  <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"></path>
                  </svg>
                </div>
                <span class="flex-1 text-left">系统状态</span>
              </button>

              <button 
                @click="() => { userLogger.logButtonClick('日志查看', '左侧导航'); showLogViewer(); onNavItemClickAfterOpen() }"
                class="group flex items-center gap-3 w-full px-3 py-2.5 text-sm font-medium rounded-xl text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700/80 transition-all duration-200"
                title="日志查看"
              >
                <div class="flex-shrink-0 w-9 h-9 rounded-lg flex items-center justify-center bg-gray-100 dark:bg-gray-900/50 group-hover:bg-emerald-100 dark:group-hover:bg-emerald-900/30 text-gray-600 dark:text-gray-400 group-hover:text-emerald-600 dark:group-hover:text-emerald-400">
                  <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path>
                  </svg>
                </div>
                <span class="flex-1 text-left">日志查看</span>
              </button>

              <button 
                @click="() => { userLogger.logButtonClick('邮件存储', '左侧导航'); openMailStorageDialog(); onNavItemClickAfterOpen() }"
                class="group flex items-center gap-3 w-full px-3 py-2.5 text-sm font-medium rounded-xl text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700/80 transition-all duration-200"
                title="各用户邮件存储使用情况"
              >
                <div class="flex-shrink-0 w-9 h-9 rounded-lg flex items-center justify-center bg-gray-100 dark:bg-gray-900/50 group-hover:bg-indigo-100 dark:group-hover:bg-indigo-900/30 text-gray-600 dark:text-gray-400 group-hover:text-indigo-600 dark:group-hover:text-indigo-400">
                  <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 7v10c0 2.21 3.582 4 8 4s8-1.79 8-4V7M4 7c0 2.21 3.582 4 8 4s8-1.79 8-4M4 7c0-2.21 3.582-4 8-4s8 1.79 8 4m0 5c0 2.21-3.582 4-8 4s-8-1.79-8-4"></path>
                  </svg>
                </div>
                <span class="flex-1 text-left">邮件存储</span>
              </button>

              <button 
                @click="() => { userLogger.logButtonClick('命令终端', '左侧导航'); showCommandTerminal(); onNavItemClickAfterOpen() }"
                class="group flex items-center gap-3 w-full px-3 py-2.5 text-sm font-medium rounded-xl text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700/80 transition-all duration-200"
                title="命令终端"
              >
                <div class="flex-shrink-0 w-9 h-9 rounded-lg flex items-center justify-center bg-gray-100 dark:bg-gray-900/50 group-hover:bg-amber-100 dark:group-hover:bg-amber-900/30 text-gray-600 dark:text-gray-400 group-hover:text-amber-600 dark:group-hover:text-amber-400">
                  <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 9l3 3-3 3m5 0h3M5 20h14a2 2 0 002-2V6a2 2 0 00-2-2H5a2 2 0 00-2 2v14a2 2 0 002 2z"></path>
                  </svg>
                </div>
                <span class="flex-1 text-left">命令终端</span>
              </button>
            </div>
          </div>
          ...
          <div class="pt-6 mt-4 border-t border-gray-200/80 dark:border-gray-700/80">
            <div class="flex items-center gap-2 px-3 mb-3">
              <div class="w-1 h-4 rounded-full bg-gradient-to-b from-violet-500 to-fuchsia-500"></div>
              <h3 class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-widest">设置</h3>
            </div>
            <router-link 
              to="/settings" 
              @click="() => { userLogger.logButtonClick('系统设置', '左侧导航'); onNavLinkClick() }"
              class="group flex items-center gap-3 px-3 py-2.5 text-sm font-medium rounded-xl transition-all duration-200"
              :class="$route.path === '/settings' ? 'bg-gradient-to-r from-violet-500 to-fuchsia-600 text-white shadow-lg shadow-violet-500/30' : 'text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700/80'"
            >
              <div class="flex-shrink-0 w-9 h-9 rounded-lg flex items-center justify-center transition-colors" :class="$route.path === '/settings' ? 'bg-white/20' : 'bg-gray-100 dark:bg-gray-900/50 group-hover:bg-violet-100 dark:group-hover:bg-violet-900/30'">
                <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <span class="flex-1 min-w-0">系统设置</span>
              <div v-if="$route.path === '/settings'" class="w-2 h-2 bg-white rounded-full animate-pulse shrink-0"></div>
            </router-link>
          </div>
        </div>
      </nav>
      ...
      <!-- 底部用户信息 -->
      <div class="absolute bottom-0 left-0 right-0 p-3 sm:p-4 shrink-0 border-t border-gray-200/80 dark:border-gray-700/80 bg-gradient-to-br from-slate-50/95 to-blue-50/80 dark:from-gray-800/95 dark:to-gray-800/80 backdrop-blur-sm">
        <div class="flex items-center">
          <div class="h-9 w-9 sm:h-10 sm:w-10 rounded-full overflow-hidden bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center shadow-md ring-2 ring-white/60 dark:ring-gray-600/60 shrink-0">
            <img 
              v-if="userAvatar" 
              :src="userAvatar" 
              alt="用户头像" 
        ...
        <!-- 邮件统计信息 -->
        <div class="mt-2 sm:mt-3 pt-2 sm:pt-3 border-t border-gray-200/80 dark:border-gray-600/80">
          <div class="flex items-center justify-between text-xs">
            <div class="flex items-center text-gray-600 dark:text-gray-300 gap-1.5">
              <svg class="h-3.5 w-3.5 text-gray-500 dark:text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 4.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path>
              </svg>
              <span>未读邮件</span>
            </div>
            <span class="bg-red-500/90 text-white px-2 py-0.5 rounded-lg font-semibold text-xs shadow-sm">{{ unreadEmailCount }}</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="flex-1 flex flex-col transition-all duration-300 ease-in-out" :class="{ 'sm:ml-64': sidebarOpen && isAdmin, 'ml-0': !sidebarOpen || !isAdmin }">
      ...
      <!-- 页面内容 - min-h-0 为 flex 子项必需，否则 overflow-auto 无法正确裁剪/滚动 -->
      <main class="flex-1 min-h-0 overflow-auto bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 transition-colors duration-200">
        <slot></slot>
      </main>
    </div>

    <CopyrightFooter variant="dashboard" :icp="icpSettings" :version="currentVersion" />
    ...
    <!-- 系统状态对话框 - 使用 Teleport 挂载到 body，避免父级 overflow 裁剪导致画面残缺 -->
    <Teleport to="body">
    <div v-if="showSystemStatusDialog" class="fixed inset-0 z-[9999] overflow-y-auto">
      <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        <div class="fixed inset-0 bg-gray-500/75 dark:bg-gray-900/75 transition-opacity" @click="closeSystemStatusDialog" aria-hidden="true"></div>
        <div class="relative z-10 inline-block align-bottom bg-white dark:bg-gray-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-4xl sm:w-full border border-gray-200 dark:border-gray-600">
          ...
          <div class="bg-gray-50 dark:bg-gray-700/50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse border-t border-gray-200 dark:border-gray-600">
            <button @click="closeSystemStatusDialog" class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-blue-600 text-base font-medium text-white hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 dark:focus:ring-offset-gray-800 sm:ml-3 sm:w-auto sm:text-sm">
              关闭
            </button>
          </div>
        </div>
    ...
    <div v-if="showSpamFilterDialog" class="fixed inset-0 z-50 overflow-y-auto">
      <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        <div class="fixed inset-0 bg-gray-500/75 dark:bg-gray-900/75 transition-opacity" @click="closeSpamFilterDialog"></div>
    ...
    <!-- 日志查看对话框 - 使用 Teleport 挂载到 body，避免父级 overflow 裁剪导致画面残缺 -->
    <Teleport to="body">
    <div v-if="showLogViewerDialog" class="fixed inset-0 z-[9999] overflow-y-auto">
      <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        <div class="fixed inset-0 bg-gray-500/75 dark:bg-gray-900/80 transition-opacity" @click="closeLogViewerDialog" aria-hidden="true"></div>
        <div class="relative z-10 inline-block align-bottom bg-white dark:bg-gray-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-6xl sm:w-full border border-gray-200 dark:border-gray-600">
          <div class="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
            <div class="sm:flex sm:items-start">
          ...
          <div class="bg-gray-50 dark:bg-gray-700/50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse border-t border-gray-200 dark:border-gray-600">
            <button @click="closeLogViewerDialog" class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-green-600 dark:bg-green-500 text-base font-medium text-white hover:bg-green-700 dark:hover:bg-green-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 dark:focus:ring-offset-gray-700 sm:ml-3 sm:w-auto sm:text-sm">
              关闭
            </button>
          </div>
        </div>
    ...
    <!-- 邮件存储使用情况（管理员） -->
    <Teleport to="body">
    <div v-if="showMailStorageDialog" class="fixed inset-0 z-[9999] overflow-y-auto">
      <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        <div class="fixed inset-0 bg-gray-900/60 dark:bg-black/70 backdrop-blur-sm transition-opacity" @click="closeMailStorageDialog" aria-hidden="true"></div>
        <div class="relative z-10 inline-block align-bottom bg-white dark:bg-gray-800 rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full border border-gray-200/50 dark:border-gray-600">
          <!-- 头部 -->
          <div class="px-6 pt-6 pb-4 bg-gradient-to-br from-indigo-50 via-purple-50/80 to-pink-50/60 dark:from-indigo-900/30 dark:via-purple-900/20 dark:to-pink-900/20 border-b border-gray-200/50 dark:border-gray-600">
            <div class="flex items-center justify-between">
              <div class="flex items-center gap-3">
                ...
                <div>
                  <h3 class="text-lg font-bold text-gray-900 dark:text-gray-100">邮件存储</h3>
                  <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">各用户邮箱容量使用情况</p>
                </div>
              </div>
              <button @click="closeMailStorageDialog" class="p-2 rounded-lg text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
              </button>
            </div>
            <p class="mt-3 text-xs text-gray-500 dark:text-gray-400">单用户限制：<span class="font-medium text-indigo-600 dark:text-indigo-400">{{ mailStorageAllUsers?.limitMB ?? 1024 }} MB</span>（可在系统设置-邮件设置中调整）</p>
          </div>
          <!-- 内容 -->
          <div class="px-6 py-4">
            <div v-if="mailStorageAllUsersLoading" class="flex flex-col items-center justify-center py-16">
              <svg class="animate-spin h-12 w-12 text-indigo-500 dark:text-indigo-400 mb-4" fill="none" viewBox="0 0 24 24">
                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
              </svg>
              <p class="text-sm text-gray-500 dark:text-gray-400">加载中...</p>
            </div>
            <div v-else-if="mailStorageAllUsers?.users?.length" class="space-y-3 max-h-96 overflow-y-auto">
              <div v-for="u in mailStorageAllUsers.users" :key="u.email" class="group flex items-center gap-4 p-4 rounded-xl border transition-all duration-200" :class="u.warning === 'critical' ? 'border-red-200 dark:border-red-800 bg-red-50/60 dark:bg-red-900/25' : u.warning === 'warning' ? 'border-amber-200 dark:border-amber-800 bg-amber-50/60 dark:bg-amber-900/25' : 'border-gray-200 dark:border-gray-600 hover:border-indigo-200 dark:hover:border-indigo-700 hover:bg-indigo-50/30 dark:hover:bg-indigo-900/10'">
                <div class="flex-shrink-0 w-10 h-10 rounded-xl flex items-center justify-center bg-gradient-to-br from-indigo-100 to-purple-100 dark:from-indigo-900/50 dark:to-purple-900/50">
                  <span class="text-sm font-bold text-indigo-600 dark:text-indigo-400">{{ (u.displayName || u.username || '?').charAt(0).toUpperCase() }}</span>
                </div>
                <div class="flex-1 min-w-0">
                  <div class="font-semibold text-gray-900 dark:text-gray-100 truncate">{{ u.displayName || u.username }}</div>
                  <div class="text-xs text-gray-500 dark:text-gray-400 truncate mt-0.5">{{ u.email }}</div>
                  <div class="mt-2 h-2 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
                    <div class="h-full rounded-full transition-all duration-500" :class="u.percentage >= 95 ? 'bg-red-500' : u.percentage >= 80 ? 'bg-amber-500' : 'bg-gradient-to-r from-indigo-500 to-purple-500'" :style="{ width: Math.min(u.percentage, 100) + '%' }"></div>
                  </div>
                </div>
                <div class="flex-shrink-0 text-right">
                  <div class="font-semibold" :class="u.percentage >= 95 ? 'text-red-600 dark:text-red-400' : u.percentage >= 80 ? 'text-amber-600 dark:text-amber-400' : 'text-gray-700 dark:text-gray-300'">{{ u.usedMB }} / {{ u.limitMB }} MB</div>
                  <div class="text-xs font-medium mt-0.5" :class="u.percentage >= 95 ? 'text-red-500' : u.percentage >= 80 ? 'text-amber-500' : 'text-gray-500 dark:text-gray-400'">{{ u.percentage }}%</div>
                </div>
              </div>
            </div>
            <div v-else class="py-12 text-center">
              <div class="inline-flex p-4 rounded-2xl bg-gray-100 dark:bg-gray-700/50 mb-4">
          ...
          <!-- 底部 -->
          <div class="px-6 py-4 bg-gray-50/80 dark:bg-gray-800/50 border-t border-gray-200 dark:border-gray-600">
            <button @click="closeMailStorageDialog" class="w-full sm:w-auto inline-flex justify-center items-center rounded-xl px-5 py-2.5 text-sm font-semibold text-white bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 shadow-lg shadow-indigo-500/25 transition-all duration-200">
              关闭
            </button>
          </div>
        </div>
...
const showMailStorageDialog = ref(false)

// 关闭对话框（供模板调用，避免在模板中直接赋值 ref 导致解包错误）
function closeSystemStatusDialog() {
  showSystemStatusDialog.value = false
  stopSystemStatusPolling()
}
function closeLogViewerDialog() {
  showLogViewerDialog.value = false
}
function closeSpamFilterDialog() {
  showSpamFilterDialog.value = false
}
const mailStorageAllUsers = ref<{ users: Array<{ username: string; email: string; displayName: string; usedBytes: number; usedMB: number; limitBytes: number; limitMB: number; percentage: number; warning?: string }>; limitMB: number } | null>(null)
const mailStorageAllUsersLoading = ref(false)
function closeMailStorageDialog() {
  showMailStorageDialog.value = false
}
async function openMailStorageDialog() {
  showMailStorageDialog.value = true
  mailStorageAllUsersLoading.value = true
  mailStorageAllUsers.value = null
  try {
    const auth = sessionStorage.getItem('apiAuth')
    const response = await fetch('/api/mail/storage-usage/all', { headers: { 'Authorization': `Basic ${auth}` } })
    if (response.ok) {
      const data = await response.json()
      if (data.success) mailStorageAllUsers.value = data
    }
  } catch (e) {
    console.error('Load mail storage all failed:', e)
  } finally {
    mailStorageAllUsersLoading.value = false
  }
}
...
// 关闭侧栏（供模板调用，避免在模板中直接写 sidebarOpen.value 导致 ref 解包错误）
function closeSidebar() {
  sidebarOpen.value = false
}

// 点击导航项后的侧栏行为：仅当「菜单折叠」开启时才关闭，否则保持展开（尊重布局设置）
function onNavItemClickAfterOpen() {
  if (sidebarCollapsed.value) closeSidebar()
}

// 点击路由链接后的侧栏行为：同上，仅折叠模式下才关闭
...
// 对日志中的文本做 HTML 转义，防止通过 innerHTML 注入 XSS
...
const updateLogDisplay = (logs) => {
  ...
  if (logContainer) {
    ...
    logContainer.innerHTML = logs.map(log => {
      ...
      let detailsHtml = ''
      if (parsed.email) {
        detailsHtml += `<span class="text-cyan-400">📧 ${escapeHtml(parsed.email)}</span>`
      }
      if (parsed.folder) {
        detailsHtml += `<span class="text-blue-400">📁 ${escapeHtml(parsed.folder)}</span>`
      }
      if (parsed.limit) {
        detailsHtml += `<span class="text-yellow-400">📊 Limit: ${escapeHtml(parsed.limit)}</span>`
      }
      if (parsed.offset) {
        detailsHtml += `<span class="text-yellow-400">📍 Offset: ${escapeHtml(parsed.offset)}</span>`
      }
      
      const safeOperationType = escapeHtml(operationType)
      const safeSourceName = escapeHtml(sourceName)
      const safeUser = escapeHtml(log.user || 'unknown')
      const safeIp = escapeHtml(log.ip || 'unknown')
        ...
        <div class="flex items-start space-x-3">
          ...
          <div class="flex-1 min-w-0">
            <div class="flex items-center space-x-2 mb-2 flex-wrap">
              ...
              <span class="text-green-400 text-sm font-bold">[${safeOperationType}]</span>
            ...
            <div class="flex items-center space-x-4 mb-2 flex-wrap text-xs">
              <span class="text-yellow-400">📁 ${safeSourceName}</span>
              <span class="text-purple-400">👤 ${safeUser}</span>
              <span class="text-green-400">⚡ ${safeOperationType}</span>
              <span class="text-cyan-400">🌐 ${safeIp}</span>
        </div>
